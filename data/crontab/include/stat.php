...
class statControl {
    ...
    public function memberstat(){
        ...
        for ($stime = $start_time; $stime < time(); $stime = $stime+86400){
			...
			//统计的销售业绩
            $field = ' sc_memberid,sc_membername,SUM(IF(sc_points>=0,sc_points,0)) as salescreditincrease, SUM(IF(sc_points<=0,sc_points,0)) as salescreditreduce ';
            $where = array();
            $where['sc_addtime'] = array('between',array($stime,$etime));
            $salescredit_tmp = $model->statBySalescreditlog($where, $field, 0, 0, '', 'sc_memberid');
            $points_list = array();
            foreach ((array)$salescredit_tmp as $k=>$v){
                $salescredit_list[$v['sc_memberid']] = $v;
            }
