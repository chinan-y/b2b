...
// 手机版商品评论后的弹框
function showComment( tips, height, time ){
    var windowWidth = document.documentElement.clientWidth;
    var tipsDiv = '<div class="tipsClass">' + tips + '</div>';
    $( 'body' ).append( tipsDiv );
    $( 'div.tipsClass' ).css({
...
function login_dialog(){
    CUR_DIALOG = ajax_form('login','登录',SITEURL+'/index.php?gct=login&inajax=1',360,1);
}
...
function collect_store(fav_id,jstype,jsobj){
    $.get('index.php?gct=index&gp=login', function(result){
        if(result=='0'){
            login_dialog();
        }else{
            var url = 'index.php?gct=member_favorites&gp=favoritesstore';
...
function collect_goods(fav_id,jstype,jsobj){
    $.get('index.php?gct=index&gp=login', function(result){
        if(result=='0'){
            ...
            var url = 'index.php?gct=member_favorites&gp=favoritesgoods';
            $.getJSON(url, {'fid':fav_id}, function(data){
                if (data.done)
            {
				$("#collectlist").load('index.php?gct=member_favorites&gp=fglist&type=html');
                showDialog(data.msg, 'succ','','','','','','','','',2);
				setInterval(function(){window.location.href=window.location.href;},1800);
                if(jstype == 'count'){ 
                    $('[nctype="'+jsobj+'"]').each(function(){
                        $(this).html(parseInt($(this).text())+1);
                    });
...
function del_collect_goods(fav_id){
    $.getJSON(SITEURL+'/index.php?gct=member_favorites&gp=delfavorites&type=goods&ty=html&fav_id='+fav_id, function(result){
        if(result.state){
            $("#collectlist").load('index.php?gct=member_favorites&gp=fglist&type=html');
...
function load_cart_information(){
	$.getJSON(SITEURL+'/index.php?gct=cart&gp=ajax_load&callback=?', function(result){
...
function drop_topcart_item(cart_id,goods_id){
    $.getJSON(SITEURL+'/index.php?gct=cart&gp=del&cart_id='+cart_id+'&goods_id='+goods_id+'&callback=?', function(result){
        if(result.state){
            var obj = $('.head-user-menu .my-cart');
            //删除成功
...
function load_history_information(){
    $.getJSON(SITEURL+'/index.php?gct=index&gp=viewed_info', function(result){
...
(function($) {
    $.show_nc_login = function(options) {
        var settings = $.extend({}, {action:'/index.php?gct=login&gp=login&inajax=1', nchash:'', formhash:'' ,anchor:''}, options);
        ...
        login_dialog_html.append('<form class="bg" method="post" id="ajax_login" action="'+APP_SITE_URL+settings.action+'"></form>').find('form')
        	...
        	.append('<dl><dt>验证码</dt><dd><input type="text" size="10" maxlength="4" class="text fl w60" name="captcha"><img border="0" onclick="this.src=\''+APP_SITE_URL+'/index.php?gct=seccode&amp;gp=makecode&amp;nchash='+settings.nchash+'&amp;t=\' + Math.random()" name="codeimage" title="看不清，换一张" src="'+APP_SITE_URL+'/index.php?gct=seccode&amp;gp=makecode&amp;nchash='+settings.nchash+'" class="fl ml10"><span>不区分大小写</span></dd></dl>')
        	.append('<ul><li>›&nbsp;如果您没有登录帐号，请先<a class="register" href="'+SHOP_SITE_URL+'/index.php?gct=login&amp;gp=register">注册会员</a>然后登录</li><li>›&nbsp;如果您<a class="forget" href="'+SHOP_SITE_URL+'/index.php?gct=login&amp;gp=forget_password">忘记密码</a>？，申请找回密码</li></ul>')
...
(function($){
	$.fn.membershipCard = function(options){
		var defaults = {
				type:''			// params  gcshop/circle/cms/micorshop
			};
		options = $.extend(defaults,options);
		return this.each(function(){
			var $this = $(this);
			var data_str = $(this).attr('data-param');eval('data_str = '+data_str);
			var _uri = SITEURL+'/index.php?gct=member_card&callback=?&uid='+data_str.id+'&from='+options.type;
			$this.qtip({
	            content: {
	                text: 'Loading...',
	                ajax: {
		                ...
		                success: function(data) {
		                	if(data){
		                		...
		                		$('<dt class="member-id"></dt>').append('<i class="sex'+data.sex+'"></i>')
		                			/*.append('<a href="'+SHOP_SITE_URL+'/index.php?gct=member_snshome&mid='+data.id+'" target="_blank">'+data.name+'</a>'+(data.truename != ''?'('+data.truename+')':''))*/
									.append('<a >'+data.name+'</a>')
		                			.appendTo(_dl);
		                		// avatar
		                		$('<dd class="avatar"><a href="'+SHOP_SITE_URL+'/index.php?gct=member_snshome&mid='+data.id+'" target="_blank"><img src="'+data.avatar+'" /></a><dd>')
		                			.appendTo(_dl);
		                		// info
		                		var _info = '';
		                		...
		                		// ajax info
		                		if(data.url != ''){
			                		$.getJSON(data.url+'/index.php?gct=member_card&gp=mcard_info&uid='+data.id, function(d){
			                			if(d){
		                		...
		                		if(data.follow != 2){
		                			...
		                			$('<div class="send-msg"> <a href="'+SHOP_SITE_URL+'/index.php?gct=member_message&gp=sendmsg&member_id='+data.id+'" target="_blank"><i></i>站内信</a> </div>').appendTo(_bottom);
		                		}
		...
		function follow(o){
			...
			$.getJSON(SHOP_SITE_URL+'/index.php?gct=member_snsfriend&gp=addfollow&callback=?&mid='+data_str.mid, function(data){
				if(data){
		...
		function onfollow(o){
			var data_str = o.parent().attr('data-param');
			eval( "data_str = "+data_str);
			$.getJSON(SHOP_SITE_URL+'/index.php?gct=member_snsfriend&gp=delfollow&callback=?&mid='+data_str.mid, function(data){
				if(data){
		...
		function circle_function(d){
			...
			$.each(d,function(i, n){
				rs.append('<div class="rank-div" title="'+n.circle_name+'圈等级'+n.cm_level+'，经验值'+n.cm_exp+'"><a href="'+CIRCLE_SITE_URL+'/index.php?gct=group&c_id='+n.circle_id+'" target="_blank">'+n.circle_name+'</a><i></i><em class="rank-em rank-'+n.cm_level+'">'+n.cm_level+'</em></div>');
			});
			return rs;
		}
...
/* 加入购物车 */
function addcart(store_id,goods_id,quantity,callbackfunc) {
    var url = 'index.php?gct=cart&gp=add';
    quantity = parseInt(quantity);
    $.getJSON(url, {store_id:store_id,'goods_id':goods_id, 'quantity':quantity}, function(data) {
        if (data != null) {
            if (data.state) {
            	if(callbackfunc){
...
//浏览商品加积分滑动字体提醒
function animatenTop(img, thisTop, thisLeft, topLength, leftLength, width, height, color, size) {
	
    var CopyDiv = '<em id="img" style="color:'+color+'; font-size:'+size+'px; text-align:center; line-height:'+height+'px; top:' + thisTop + "px;left:" + thisLeft + 'px;">'+img+'</em>' ;
...
//     if ($(document).height() - $(this).scrollTop() - $(this).height() > 7500) {
// 		$("#ads").hide();
//     }
