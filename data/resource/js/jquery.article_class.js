$(document).ready(function(){
	...
	$('img[nc_type="flex"]').click(function(){
		...
		if(status == 'open'){
			...
			//ajax
			$.ajax({
				url: 'index.php?gct=article_class&gp=article_class&ajax=1&ac_parent_id='+id,
				dataType: 'json',
				success: function(data){
					...
					for(var i = 0; i < data.length; i++){
						...
						//新增下级
						if(data[i].deep < 2){
							src += "<a class='btn-add-nofloat marginleft' href='index.php?gct=article_class&gp=article_class_add&ac_parent_id="+data[i].ac_id+"'><span>新增下级</span></a></span>";
						}
						src += "</td>";
						
						//操作
						src += "<td class='w84'>";
						src += "<span><a href='index.php?gct=article_class&gp=article_class_edit&ac_id="+data[i].ac_id+"'>编辑</a>";
						src += " | <a href=\"javascript:if(confirm('删除该分类将会同时删除该分类的所有下级分类，您确定要删除吗'))window.location = 'index.php?gct=article_class&gp=article_class_del&ac_id="+data[i].ac_id+"';\">删除</a>";
						src += "</td>";
						src += "</tr>";
					}
					//插入
