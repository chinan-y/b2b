$(document).ready(function(){
	...
	$('img[nc_type="flex"]').click(function(){
		...
		if(status == 'open'){
			...
			//ajax
			$.ajax({
				url: 'index.php?gct=sales_area&gp=sales_area&ajax=1&sc_parent_id='+id,
				dataType: 'json',
				success: function(data){
					...
					for(var i = 0; i < data.length; i++){
						...
						//排序
						src += "<span title='可编辑' ajax_branch='sales_area_sort' datatype='number' fieldid='"+data[i].sc_id+"' fieldname='sc_sort' nc_type='inline_edit' class='editable'>"+data[i].sc_sort+"</span>";
						//名称
						src += "<td class='name'>";
						...
						src += "<span title='可编辑' required='1' fieldid='"+data[i].sc_id+"' ajax_branch='sales_area_name' fieldname='sc_name' nc_type='inline_edit' class='node_name editable'>"+data[i].sc_name+"</span>";
						//新增下级
						if(data[i].deep < 2){
							src += "<a  class='btn-add-nofloat marginleft' href='index.php?gct=sales_area&gp=sales_area_add&sc_parent_id="+data[i].sc_id+"'><span>新增下级<span></a></span>";
						}
						src += "</td>";
						
						//操作
						src += "<td class='w84'>";
						src += "<span><a href='index.php?gct=sales_area&gp=sales_area_edit&sc_id="+data[i].sc_id+"'>编辑</a>";
						src += " | <a href=\"javascript:if(confirm('删除该分类将会同时删除该分类的所有下级分类，您确定要删除吗'))window.location = 'index.php?gct=sales_area&gp=sales_area_del&sc_id="+data[i].sc_id+"';\">删除</a>";
						src += "</td>";
						src += "</tr>";
					}
					//插入
					...
					$.getScript(RESOURCE_SITE_URL+"/js/jquery.sales_area.js");
