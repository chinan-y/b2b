$(document).ready(function(){
    ...
    $("#picture_image_upload").fileupload({
        dataType: 'json',
            url: "index.php?gct=cms_special&gp=special_image_upload",
            add: function(e,data) {
    ...
    $("[nctype='btn_drop_special_image']").live("click",function(){
        ...
        $.getJSON("index.php?gct=cms_special&gp=special_image_drop", { image_name: image_name }, function(result){
                if(result.status == "success") {
                    image_object.remove();
                } else {
    ...
    $("[nctype='btn_special_goods']").live('click', function(){
        ...
        $.getJSON("index.php?gct=cms_special&gp=goods_info_by_url", { url: url}, function(data){
            if(data.result == "true") {
				...
				temp += '<dd class="price">销售价：<em>'+data.price+'</em></dd>';
				}
				if(data.promotion_type>"0")
				{
				temp += '<dd class="price">促销价：<em>'+data.promotion_price+'</em></dd>';
				}
    ...
    $('#mb_special_goods_list').on('click', '[nctype="btn_add_goods"]', function() {
		...
		var goods_list = $("<div />").append($(this).parents("table").find("[nctype='_special_goods_list']").find("[id='_special_goods_list_li_"+item.goods_id+"']").clone());
		goods_list.find("i").remove();
		goods_list.find("[nctype='btn_special_goods_delete']").remove();
		goods_list.find("[nctype='btn_add_goods']").remove();
		goods_list.find("#goods_pagination").remove();
