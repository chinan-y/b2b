...
$(document).ready(function(){
    ...
    $("#btn_image_upload").fileupload({
        dataType: 'json',
            url: "index.php?gct=cms_index&gp=image_upload",
            add: function(e,data) {
                data.submit();
            },
    ...
    $("#btn_article_search").click(function(){
        ...
        if(search_keyword !== "") {
            $("#div_article_select_list").load("index.php?gct=cms_base&gp=get_article_list&" + $.param({search_type: search_type,search_keyword: search_keyword}));
        }
    });
    //文章选择翻页
    ...
    $("#btn_goods_search").live('click', function(){
        ...
        if($("#goods_selected_list li").length < limit_count || limit_count === 0) {
            if(link != '') {
                ...
                $.getJSON('index.php?gct=cms_base&gp=goods_info_by_url', { url: url}, function(data){
                    if(data.result == "true") {
    ...
    $("[nctype='btn_module_brand_edit']").click(function(){
        ...
        if($("#div_brand_select_list").html() === '') {
            $("#div_brand_select_list").load("index.php?gct=cms_base&gp=get_brand_list");
        }
    ...
    $("[nctype='btn_module_goods_class_edit']").click(function(){
        ...
        if($("#select_goods_class_list option").length < 1) {
            ...
            $.getJSON("index.php?gct=cms_base&gp=get_goods_class_list_json", { }, function(result){
    ...
    $("#select_goods_class_list").change(function(){
        $.get("index.php?gct=cms_base&gp=get_goods_class_detail", {class_id: $(this).val() }, function(result){
    ...
    $("#btn_module_micro_edit").live('click', function(){
        ...
        var count = 0;
        api_url = micro_url+'/index.php?gct=api&gp=get_micro_name&callback=?';
        $.getJSON(api_url, {data_type: 'html'}, function(result){
            micro_name = result;
            write_html();
        });
        api_url = micro_url+'/index.php?gct=api&gp=get_personal_class&callback=?';
        $.getJSON(api_url, {data_type: 'html'}, function(result){
            micro_personal_class = result;
            write_html();
        });
        api_url = micro_url+'/index.php?gct=api&gp=get_personal_commend&callback=?';
        $.getJSON(api_url, {data_type: 'html', data_count: 8}, function(result){
            micro_personal_commend = result;
            write_html();
        });
        api_url = micro_url+'/index.php?gct=api&gp=get_store_commend&callback=?';
    ...
    $("#btn_module_circle_edit").live('click', function(){
        ...
        var count = 0;
        api_url = circle_url+'/index.php?gct=api&gp=get_theme_list&callback=?';
        $.getJSON(api_url, {data_type: 'html'}, function(result){
            circle_theme_list = result;
            write_html();
        });
        api_url = circle_url+'/index.php?gct=api&gp=get_reply_themelist&callback=?';
        $.getJSON(api_url, {data_type: 'html'}, function(result){
            circle_reply_themelist = result;
            write_html();
        });
        api_url = circle_url+'/index.php?gct=api&gp=get_more_membertheme&callback=?';
    ...
    $("#btn_store_search").click(function(){
        ...
        if(search_keyword !== "") {
            $("#div_store_select_list").load("index.php?gct=cms_base&gp=get_store_list&" + $.param({search_keyword: search_keyword}));
        }
    });
    //店铺选择翻页
    ...
    $("#btn_member_search").click(function(){
        ...
        if(search_keyword !== "") {
            $("#div_member_select_list").load("index.php?gct=cms_base&gp=get_member_list&" + $.param({search_keyword: search_keyword}));
        }
    });
    //用户选择翻页
