...
//	        window.location.href=uri + '&' + name + '=' + items;
	...
	/* 批量查看按钮 */
	$('a[nc_type="batchbutton1"]').click(function(){
		/* 是否有选择 */
        if($('.checkitem:checked').length == 0){ //没有选择
        	showDialog('请选择需要标为已读的记录');
            return false;
        }
		var _uri = $(this).attr('uri');
        var _name = $(this).attr('name');
		...
		handleResult(_uri,_name);
		setInterval(function(){window.location.href=window.location.href;},300);
	});	
    ...
    $('*[nc_type="gselector"]').focus(function(){
        ...
        var gp = $(this).attr('gs_op');
        var store_id = $(this).attr('gs_store_id');
        var title = $(this).attr('gs_title') ? $(this).attr('gs_title') : '';
        var width = $(this).attr('gs_width');
        ajax_form(id, title, SITEURL + '/index.php?gct=store_groupbuy&gp=' + gp + '&dialog=1&title=' + title + '&store_id=' + store_id+ '&id=' + id + '&name=' + name + '&callback=' + callback, width);
        return false;
    });
