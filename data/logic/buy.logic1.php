...
class buyLogic {
    ...
    public function getBuyStep1Data($member_id, $data) {
        ...
        list($store_cart_list,$store_goods_total,$store_taxes_total) = $this->_logic_buy_1->calcCartList($store_cart_list);
    ...
    private function _createOrderStep3() {
        ...
        //商品金额计算(分别对每个商品/优惠套装小计、每个店铺小计)
        list($store_cart_list,$store_goods_total,$store_taxes_total) = $this->_logic_buy_1->calcCartList($store_cart_list);//增加返利
    ...
    private function _createOrderStep4() {
		...
		$true_idnum = $input_address_info['true_idnum'];
		//得到会员购买商品得返利金额，根据商品返利和区域返利
		$member = Model('member');
		$member_info = $member -> where(array('member_id'=>$member_id))->select();
		$goods_rebate_rate = array();
		$sum = array();
		$member_ref_id = $member -> where(array('member_id' => $member_info[0]['refer_id']))->select();
		if($member_info[0]['refer_id']){
			foreach($goods_goods_taxes as $is => $v){
				if($member_ref_id[0]['is_rebate'] == 1){
					$sum[$is] = $v['goods_rebate_rate'] * $v['goods_price'] * $v['goods_num'];
					$goods_rebate_rate['order_distinguish'] = 1 ;
				} else if($member_ref_id[0]['is_member_rebate'] == 1){
					$sum[$is] = $member_ref_id[0]['member_rebate_rate'] * $v['goods_price'] * $v['goods_num'] * $v['tune_parameter'];
					$goods_rebate_rate['order_distinguish'] = 0 ;
				} else {
					$goods_rebate_rate['goods_rebate_amount'] = 0;
					$goods_rebate_rate['order_distinguish'] = 2 ;
				}
				
			}
			$goods_rebate_rate['goods_rebate_amount'] = array_sum($sum);
        ...
        foreach ($store_cart_list as $store_id => $goods_list) {
			...
			$order['order_distinguish'] = $goods_rebate_rate['order_distinguish'];
			$order['goods_rebate_amount'] = $goods_rebate_rate['goods_rebate_amount'];//订单总返利金额没计算出来，数据库采用事务
			//修改货到付款bug BY MING
            ...
            $order_common['order_name'] = $order_name;
			$order_common['reciver_idnum'] = $reciver_idnum;
            ...
            foreach ($goods_list as $goods_info) {
                ...
                if (!intval($goods_info['bl_id'])) {
					...
					if($member_info[0]['refer_id']){
							if($member_ref_id[0]['is_rebate'] == 1){
								$order_goods[$i]['goods_rebate_rate'] =$goods_info['goods_rebate_rate'];
								$order_goods[$i]['goods_rebate_amount'] = $goods_info['goods_rebate_rate'] * $goods_info['goods_price'] * $goods_info['goods_num'];
							} else if($member_ref_id[0]['is_member_rebate'] == 1){
								$order_goods[$i]['goods_rebate_rate'] = $member_ref_id[0]['member_rebate_rate'];
								$order_goods[$i]['goods_rebate_amount'] = $member_ref_id[0]['member_rebate_rate'] * $goods_info['goods_price'] * $goods_info['goods_num'] * $goods_info['tune_parameter'];
							} else {
								$order_goods[$i]['goods_rebate_rate']=0.000;
								$order_goods[$i]['goods_rebate_amount']=0.00;
							}
					}else{
						$order_goods[$i]['goods_rebate_rate']=0.000;
						$order_goods[$i]['goods_rebate_amount']=0.00;
                    ...
                    foreach ($goods_info['bl_goods_list'] as $bl_goods_info) {
						...
						//$goods_taxes = $goods_info['goods_taxes'] * $goods_info['goods_num'];
                        //计算本件商品优惠金额
                        $promotion_value = floor($goods_total*($promotion_rate));
                        $order_goods[$i]['goods_pay_price'] = $goods_total - $promotion_value;
                        $promotion_sum += $promotion_value;
    ...
    private function _getGoodsList($cart_list) {
        ...
        foreach ($cart_list as $key => $cart) {
            ...
            if (!intval($cart['bl_id'])) {
				...
				$goods_list[$i]['tune_parameter'] = $cart['tune_parameter'];
                $i++;
            } else {
                //如果是优惠套装商品
