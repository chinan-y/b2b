...
class queueLogic {
     ...
     * @param unknown $member_info
     */
    public function addSalescredit($member_info) {
        $salescredit_model = Model('salescredit');
        $salescredit_model->saveSalescreditLog('login',array('sc_memberid'=>$member_info['member_id'],'sc_membername'=>$member_info['member_name']),true);
        return callback(true);
    }
    ...
    public function cancelOrderUpdateStorage($goods_buy_quantity) {
        ...
        foreach ($goods_buy_quantity as $goods_id => $quantity) {
			...
			//有抢购的商品更新抢购商品的库存销量
			$data1 = array();
			$data1['goods_storage'] = array('exp','goods_storage+'.$quantity);
			$data1['buyer_count'] 	= array('exp','buyer_count-'.$quantity);
			$data1['buy_quantity'] 	= array('exp','buy_quantity-'.$quantity);
			Model('groupbuy')->editGroupbuy($data1, array('goods_id'=>$goods_id));
        }
        if (!$result) {
            return callback(false,'变更商品库存与销量失败');
        } else {
    ...
    public function saveDeliveryOrder($param) {
        ...
        foreach ($param['order_sn_list'] as $order_id => $v) {
			...
			$data['reciver_idnum'] = $param['reciver_idnum'];
     ...
     * 发送站内消息
     */
    public function sendMessageMsg($param) {
        $email = new Email();
        $result = $email->send_sys_email($param['acount'],$param['subject'],$param['message']);
        if (!$result) {
            return callback(false,'发送推广邮件失败member_id:'.$param['member_id'].'模版id:'.$param['$template_id']);
        } else {
            return callback(true);
        }
    }
     ...
     * 发送短信消息
     */
    public function sendSmsMsg($param) {
        $sms = new Sms();
        $result = $sms->send($param['acount'],$param['message']);
        if (!$result) {
            return callback(false,'发送推广短信消息失败member_id:'.$param['member_id'].'模版id:'.$param['$template_id']);
        } else {
            return callback(true);
        }
    }
     ...
     * 发送邮件消息
     */
    public function sendEmailMsg($param) {
        $email = new Email();
        $result = $email->send_sys_email($param['acount'],$param['subject'],$param['message']);
        if (!$result) {
            return callback(false,'发送推广邮件失败member_id:'.$param['member_id'].'模版id:'.$param['$template_id']);
