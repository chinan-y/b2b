...
class buy_1Logic {
     ...
     */
    public function getGoodsCartList($cart_list,$third="") {

        $cart_list = $this->_getOnlineCartList($cart_list,$third);
     ...
     * @param unknown $store_cart_list 以店铺ID分组的购物车商品信息  One：1，进口保税。two：2海外直邮。three：3、8、9普通商品。
     * @return array
     */
    public function calcCartList($store_cart_list) {
        ...
        foreach ($store_cart_list as $store_id => $store_cart) {
            foreach ($store_cart as $key => $cart_info) {
                $mess_store_cart_list[$store_id]['mess_price_amount'] += ncPriceFormat($cart_info['goods_price'] * $cart_info['goods_num']);
                $mess_store_cart_list[$store_id]['mess_num_amount'] += $cart_info['goods_num'];
                $mess_store_cart_list[$store_id]['mess_weight_amount'] += $cart_info['goods_weight']*$cart_info['goods_num'];
            }
			$mess_store_cart_list[$store_id]['mess_transport_id'] = $store_cart[0]['transport_id'];
			$mess_store_cart_list[$store_id]['shipping_fee'] = $store_cart[0]['shipping_fee'];
			$mess_store_cart_list[$store_id]['mess_goods_count'] = count($store_cart);
        }
		
		//海关计算默认运费-初始化运费收货地址信息
        $member_id = $_SESSION['member_id'];
        if($this->_order_data['input_city_id']) {
            $mess_city_id = $this->_order_data['input_city_id'];
            $mess_area_id = $this->_order_data['input_area_id'];
        }else{
            $mess_address_info = Model('address')->getDefaultAddressInfo(array('member_id'=>$member_id));
            $mess_city_id = $mess_address_info['city_id'];
            $mess_area_id = $mess_address_info['area_id'];
        }
		
        //海关计算默认运费-如果商品金额未达到免运费设置下线，则需要计算运费
        $condition = array('store_id' => array('in',array_keys($store_cart_list)));
        $store_list = Model('store')->getStoreOnlineList($condition,null,'','store_id,store_free_price');

        //海关计算默认运费-如果有商品使用的运费模板，先计算这些商品的运费总金额
        $model_transport = Model('transport');
		$transports = $model_transport->getTransports();
        foreach ($store_list as $store_info) {
            $store_id = $store_info['store_id'];
			$mess_transport_id = $mess_store_cart_list[$store_id]['mess_transport_id'];
            $limit_price = floatval($store_info['store_free_price']);
            if ($limit_price == 0 || $limit_price > $mess_store_cart_list[$store_id]['mess_price_amount']) {
				//优惠套装商品的运费单独计算好结算税金
				if($mess_store_cart_list[$store_id]['shipping_fee']){
					$mess_store_cart_list[$store_id]['mess_shipping_fee'] = $mess_store_cart_list[$store_id]['shipping_fee'];
				}else{
					if($transports[$mess_transport_id]['type'] == 0){
						$mess_store_cart_list[$store_id]['mess_shipping_fee'] = $model_transport->calc_transport($mess_transport_id,$mess_store_cart_list[$store_id]['mess_weight_amount'],$mess_city_id);
					}else{
						$mess_store_cart_list[$store_id]['mess_shipping_fee'] = $model_transport->calc_transport($mess_transport_id,$mess_store_cart_list[$store_id]['mess_num_amount'],$mess_city_id);
					}
				}
					
            }else{
				$mess_store_cart_list[$store_id]['mess_shipping_fee'] = 0;
			}
        }

        //存放每个店铺的商品总金额	其中进口保税，直邮的商品不能在同一店铺
        $store_goods_total = array();
		$store_taxes_total = array();
        foreach ($store_cart_list as $store_id => $store_cart) {
            $tmp_amount = 0;
			$tmp_taxes = 0;
			
			foreach ($store_cart as $key => $cart_info) {
            	if($cart_info['bl_goods_list']){
					$store_cart = $cart_info['bl_goods_list'];
				}
			}
            foreach ($store_cart as $key => $cart_info) {
            	
                if($cart_info['store_from'] == 1 || $cart_info['store_from'] == 8){//保税进口方式和三方报税供货
                    if($mess_store_cart_list[$store_id]['mess_shipping_fee'] > 0){
                        $cart_info['shipping_fee'] = ncPriceFormat($mess_store_cart_list[$store_id]['mess_shipping_fee'] / $mess_store_cart_list[$store_id]['mess_num_amount']);
                    }else{
                    ...
                    $store_cart[$key]['taxes_total'] = ncPriceFormat($this->_logic_tax_1->single_times_allow_2000($cart_info) * $cart_info['goods_num']);
                    $store_cart[$key]['shipping_total'] = $cart_info['shipping_fee']*$cart_info['goods_num'];
                }else{//其他不收税方式
            		$store_cart[$key]['goods_image_url'] = cthumb($store_cart[$key]['goods_image']);
            		$store_cart[$key]['goods_total'] = ncPriceFormat($cart_info['goods_price'] * $cart_info['goods_num']);
                    $store_cart[$key]['taxes_total'] = 0;
            	}
				
                $tmp_amount += $store_cart[$key]['goods_total'];
				$tmp_taxes += $store_cart[$key]['taxes_total'];
            }
            $store_cart_list[$store_id] = $store_cart;
            $store_goods_total[$store_id] = ncPriceFormat($tmp_amount);
			$store_taxes_total[$store_id] = ncPriceFormat($tmp_taxes);

        }
        return array($store_cart_list,$store_goods_total,$store_taxes_total);
    }
     ...
     * 
     */
    public function calcCartList_taxes($store_cart_list) {
        ...
        foreach ($store_cart_list as $store_id => $store_cart) {
			...
			$store_taxes_total[$store_id] = $store_cart;

        }
        return $store_taxes_total;
    }
	...
	public function reCalcGoodsTotal($store_goods_total, $preferential_array, $preferential_type) {
	    ...
	    switch ($preferential_type) {
			...
			case 'points':
	    	    foreach ($preferential_array as $store_id => $points_total) {
	    	        $store_goods_total[$store_id] += '-'.number_format($points_total/ C('exchange_rate'),2);
	    	    }
	    	    break;
	    }
	...
	public function getStoreAvailableVoucherList($store_goods_total, $member_id) {
	    ...
	    foreach ($store_goods_total as $store_id => $goods_total) {
	        $condition = array();
	        $condition['voucher_store_id'] = array('in','0,'.$store_id);//0为虚拟的通用平台店铺ID,用户可以使用平台通用代金券
	...
	public function reParseVoucherList($input_voucher_list = array(), $store_goods_total = array(), $member_id) {
	    ...
	    foreach ($input_voucher_list as $store_id => $voucher) {
	        ...
	        if (is_array($tmp) && isset($tmp[$voucher['voucher_t_id']])) {
	            ...
	            $input_voucher_list[$store_id]['voucher_design_sku'] = $tmp[$voucher['voucher_t_id']]['voucher_design_sku'];//增加商品对应SKU
	        } else {
    ...
    public function getStoreFreightList($buy_list = array(), $free_freight_sid_list) {
        ...
        foreach ($buy_list as $goods_info) {
            $freight_list[$goods_info['store_id']][$goods_info['transport_id']] += ($goods_info['goods_num'] * $goods_info['goods_weight']) ;
        }
...
//         if (empty($pd_payment_info)) return;
        ...
        foreach ($order_list as $order_info) {
            ...
            if ($available_pd_amount >= $order_amount) {
                ...
                $data_order['payment_time'] = TIMESTAMP;
                // $data_order['payment_code'] = 'predeposit';
                $data_order['payment_code'] = 'tonglian'; //站内余额支付使用通联支付报关推单
                ...
                if (!$result) {
					...
					if($is_ems_no){
						$data1 = array();
						$data1['shipping_express_id'] = 8;
						$data1['shipping_time'] = TIMESTAMP ;
						$condition = array();
						$condition['order_id'] = $order_info['order_id'];
						$condition['store_id'] = $order_info['store_id'];
						$model_order->editOrderCommon($data1,$condition);
						
						$bill_no = Logic('order')->getEmsBillno(1, 1);
						if(!$bill_no){
							$bill_no = Logic('order')->getEmsBillno(1, 1);
						}
						$update_order = array();
						...
						$update_order['shipping_code'] = $bill_no;
						$update_order['delay_time'] = TIMESTAMP;
						$re = $model_order->editOrder($update_order,array('order_id'=>$order_info['order_id'],'order_state'=>ORDER_STATE_PAY));
						if(!$re){
							$sms = new Sms();
    ...
    public function getFreeFreightActiveList($store_id_array) {
        ...
        foreach ($store_list as $store_info) {
            $limit_price = floatval($store_info['store_free_price']);
            if ($limit_price > 0) {
                $store_free_freight_active[$store_info['store_id']] = sprintf('满%s元包邮',$limit_price);
            }
        }
    ...
    public function getReciverAddr($address_info = array()) {
        if (intval($address_info['dlyp_id'])) {
			...
			$reciver_idnum = $address_info['dlyp_address_idnum'];
			$order_name = $address_info['dlyp_order_name'];
        } else {
			...
			$reciver_idnum = $address_info['true_idnum'];
			$order_name = $address_info['order_name'];
        }
        return array($reciver_info, $reciver_name, $reciver_idnum,$order_name);
    }
    ...
    private function _getGoodsOnlineInfo($goods_id,$quantity) {
		...
		$goods_adjust_rebate_rate =Model('goods')->getGood_Sadjust_rebate_rate($goods_commonid);
        ...
        $new_array['goods_id'] = strval($goods_id);
        $new_array['goods_commonid'] = $goods_info['goods_commonid'];
        $new_array['goods_hscode'] = $goods_info['goods_hscode'];
        $new_array['gc_id'] = $goods_info['gc_id'];
        $new_array['store_id'] = $goods_info['store_id'];
        $new_array['store_from'] = $goods_info['store_from'];
        $new_array['goods_name'] = $goods_info['goods_name'];
        $new_array['goods_price'] = $goods_info['goods_price'];
		$new_array['goods_taxes'] = $goods_info['goods_taxes'];
		$new_array['goods_weight'] = $goods_info['goods_weight'];
		$new_array['goods_rebate_rate'] = $goods_info['goods_rebate_rate'];//商品返利率 by liu
		$new_array['goods_rebate_amount'] = $goods_info['goods_rebate_amount'];//商品返利金额 by liu
		...
		$new_array['adjust_rebate_rate'] = $goods_adjust_rebate_rate['adjust_rebate_rate'];
		$new_array['mess_country_code'] = $goods_info['mess_country_code'];
    ...
    public function getGroupbuyInfo(& $goods_info = array()) {
		...
		$goods_info['goods_taxes'] = $groupbuy_info['groupbuy_taxes'];
     ...
     * @return array
     */
    private function _getOnlineCartList($cart_list,$third='') {
        ...
        foreach ((array)$cart_list as $key => $cart_info) {
            ...
            if (in_array($cart_info['goods_id'],array_keys($goods_online_array))) {
				...
				$cart_list[$key]['goods_hscode'] = $goods_online_info['goods_hscode'];
                $cart_list[$key]['goods_commonid'] = $goods_online_info['goods_commonid'];
                $cart_list[$key]['goods_name'] = $goods_online_info['goods_name'];/*.$goods_online_info['goods_jingle'];在商品后面增加商品的型号信息*/
				...
				$cart_list[$key]['goods_taxes'] = $goods_online_info['goods_taxes'];
				$cart_list[$key]['store_from'] = $goods_online_info['store_from'];
				$cart_list[$key]['goods_weight'] = $goods_online_info['goods_weight'];
				$cart_list[$key]['goods_rebate_rate'] = $goods_online_info['goods_rebate_rate'];//增加商品返利率
				$cart_list[$key]['goods_rebate_amount'] = $goods_online_info['goods_rebate_amount'];//增加商品返利金额
				...
				$cart_list[$key]['adjust_rebate_rate'] = $goods_online_info['adjust_rebate_rate'];
				$cart_list[$key]['is_use_adjustrate'] = $goods_online_info['is_use_adjustrate'];
                $cart_list[$key]['groupbuy_info'] = $goods_online_info['groupbuy_info'];
                $cart_list[$key]['xianshi_info'] = $goods_online_info['xianshi_info'];
                $cart_list[$key]['mess_country_code'] = $goods_online_info['mess_country_code'];
            } else {
                //如果商品下架
    ...
    private function _getBundlingCartList(& $cart_list) {
        ...
        foreach ($cart_list as $key => $cart_info) {
			...
			$cart_list[$key]['goods_commonid'] = $goods_list[0]['goods_commonid'];
            ...
            foreach ($cart_list[$key]['bl_goods_list'] as $k => $goods_info) {
				$cart_list[$key]['bl_goods_list'][$k]['state'] = true;
				$cart_list[$key]['bl_goods_list'][$k]['storage_state'] = true;
                if (array_key_exists($goods_info['goods_id'],$goods_online_list)) {
                    $goods_online_info = $goods_online_list[$goods_info['goods_id']];
                    //如果库存不足，标识false
                    if ($cart_info['goods_num'] > $goods_online_info['goods_storage']) {
                        $cart_list[$key]['storage_state'] = false;
                        $cart_list[$key]['bl_goods_list'][$k]['storage_state'] = false;
                    }
					 if (empty($bl_info) || !intval($bl_info['bl_state'])) {
						$cart_list[$key]['bl_goods_list'][$k]['state'] = false;
					}
					
                    $cart_list[$key]['bl_goods_list'][$k]['cart_id'] = $cart_info['cart_id'];
					...
					$cart_list[$key]['bl_goods_list'][$k]['shipping_fee'] = $cart_info['shipping_fee'];
                    $cart_list[$key]['bl_goods_list'][$k]['goods_hscode'] = $goods_online_info['goods_hscode'];
                    $cart_list[$key]['bl_goods_list'][$k]['goods_price'] = $goods_info['bl_goods_price'];
                    $cart_list[$key]['bl_goods_list'][$k]['store_from'] = $cart_info['store_from'];
                    $cart_list[$key]['bl_goods_list'][$k]['goods_num'] = $cart_info['goods_num'];
                    $cart_list[$key]['bl_goods_list'][$k]['store_id'] = $goods_online_info['store_id'];
                    ...
                    $cart_list[$key]['bl_goods_list'][$k]['goods_rebate_rate'] = $goods_online_info['goods_rebate_rate'];
                    $cart_list[$key]['bl_goods_list'][$k]['adjust_rebate_rate'] = $goods_online_info['adjust_rebate_rate'];
                    $cart_list[$key]['bl_goods_list'][$k]['is_use_adjustrate'] = $goods_online_info['is_use_adjustrate'];
                    $cart_list[$key]['bl_goods_list'][$k]['mess_country_code'] = $goods_online_info['mess_country_code'];
                    //每个商品直降多少
