...
class paymentLogic {
	...
	//贵州银行支付打折
	public function returnGzBank($pay_sn){
		...
		//取得订单数据
		$order_info=$order_model->getOrderList(array('pay_sn'=>$pay_sn));
		//取得折扣信息
		$list_setting = $model_setting->getListSetting();
		if($list_setting['GZ_RETURN']==1 && $list_setting['GZ_RETURN_discount']!=0){
		$discount=$list_setting['GZ_RETURN_discount'];
		$returnArray['amount']=$order_info[0]['goods_amount']*(1-$discount/100);
		}
		$returnArray['pay_sn']=$pay_sn;
		return $returnArray;
