...
class buyModel {
    ...
    public function getStoreFreightList($buy_list = array(), $free_freight_sid_list) {
        ...
        //最后再计算使用运费模板的信息(店铺ID，运费模板ID，购买数量),使用相同运费模板的商品数量累加
        $freight_list = array();
        ...
        if(is_array($store_premiums_list)) {
            foreach ($store_premiums_list as $store_id => $goods_list) {
                foreach ($goods_list as $goods_info) {
					...
					$new_data['goods_weight'] = $goods_info['goods_weight'];
        ...
        foreach ($store_cart_list as $store_cart) {
            foreach ($store_cart as $cart_info) {
                if (!intval($cart_info['bl_id'])) {
                    //正常商品
                    $goods_storage_quangitygoods_storage_quangity[$cart_info['goods_id']] = $cart_info['goods_storage'];
    ...
    public function buyStep1($cart_id, $ifcart, $invalid_cart, $member_id, $store_id) {
        ...
        list($store_cart_list,$store_goods_total,$store_taxes_total) = $model_cart->calcCartList($store_cart_list);
    ...
    public function buyStep2($post, $member_id, $member_name, $member_email) {
        ...
        //计算店铺最终订单实际支付金额(加上运费 和 税金)
        $store_final_order_total = $model_cart->reCalcGoodsTotal($store_final_goods_total,$store_freight_total,'freight',$store_taxes_total);
