...
class messageModel {
	...
	public function countNewMessage($member_id){
		$member = Model('member')->getMemberInfo(array('member_id'=>$member_id),'member_time');
		$condition_arr = array();
	 ...
	 * 更新站内信(一键查看)
	 */
	 public function updateMultitermMessage($condition,$to_member_id){
		//得到条件语句
		$condition_str = $this->getCondition($condition);
		//查询站内信列表
		$message_list	= array();
		...
		foreach ($message_list as $k=>$v){
			...
			if (!empty($v['read_member_id'])){
				$tmp_lock_arr = explode(',',$v['read_member_id']);
				if (!in_array($to_member_id,$tmp_lock_arr)){
					$tmp_lock_arr[] = $to_member_id;
				}
				foreach ($tmp_lock_arr as $delid_k=>$delid_v){
					if ($delid_v == ''){
						unset($tmp_lock_arr[$delid_k]);
					}
				}
				$tmp_lock_arr = array_unique ($tmp_lock_arr);//去除相同
				sort($tmp_lock_arr);//排序·
				$tmp_lock_str = ",".implode(',',$tmp_lock_arr).",";
			}else {
				$tmp_lock_str = ",{$to_member_id},";
			}
			
			if($v['message_type']==2 ||  $v['message_type']==0 ){
				$tmp_lock_str = '1' ; 
				Db::update('message',array('message_open'=>$tmp_lock_str)," message_id = '{$v['message_id']}' ");
			}else{
				Db::update('message',array('read_member_id'=>$tmp_lock_str)," message_id = '{$v['message_id']}' ");
			}
		}
		return true; 
	}
	...
	private function getCondition($condition_array){
		...
		if($condition_array['to_member_id'] != ''){
			$condition_sql	.= "and message.message_update_time >'{$condition_array['member_time']}' and (message.to_member_id ='all' or (message.message_ismore=0 and message.to_member_id='{$condition_array['to_member_id']}') or (message.message_ismore=1 and message.to_member_id like '%,{$condition_array['to_member_id']},%'))";
		}
		//发信人
	 ...
	 *  生成订单，发送站内信给用户
	 */
	public function message_mail($conditions){
		Model()->table('message')->insert($conditions);
	}	
	
	public function del_message($del_message){
		$uu = Model()->table('message')->where($del_message)->select();
		var_dump($uu);die;
	}
