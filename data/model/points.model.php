...
class pointsModel {
	/**
	 * 操作积分
	 * @author gcwebshop Develop Team
	 * @param  string $stage 操作阶段regist(注册),sign(签到),browse(浏览商品),login(登录),comments(评论),order(下单),system(系统),other(其他),pointorder(积分礼品兑换),app(同步积分兑换),cash(订单积分抵现)
	...
	function savePointsLog($stage,$insertarr,$if_repeat = true){
		...
		switch ($stage){
			...
			case 'sign':
				...
				$insertarr['pl_points'] = intval(C('points_sign'));
				break;
			case 'browse':
				if (!$insertarr['pl_desc']){
					$insertarr['pl_desc'] = '浏览商品';
				}
				$insertarr['pl_points'] = intval(C('browse_goods'));
				break;
			case 'login':
				if (!$insertarr['pl_desc']){
			...
			case 'cash':
				if (!$insertarr['pl_desc']){
					$insertarr['pl_desc'] = '订单'.$insertarr['order_sn'].'抵现支付';
				}
				$insertarr['pl_points'] = '-'.$insertarr['member_points'];
				break;
		}
		$save_sign = true;
		if ($if_repeat == false){
		...
		if ($result){
			...
			if($stage == 'sign'){
				$upmember_array['member_sign_time'] = TIMESTAMP ;
			}
			if($stage == 'browse'){
				$upmember_array['browse_goods_up'] = array('exp','browse_goods_up+'.$insertarr['pl_points']);
			}
