...
class p_xianshi_goodsModel extends Model{
    ...
    public function getXianshiGoodsExtendInfo($xianshi_info) {
		$commonid = Model('goods')->getGoodsIn(array('goods_id' => $xianshi_info['goods_id']), 'goods_commonid');
        $xianshi_info['goods_url'] = urlShop('goods', 'index', array('goods_id' => $xianshi_info['goods_id']));
        $xianshi_info['image_url'] = cthumb($xianshi_info['goods_image'], 60, $xianshi_info['store_id'], $commonid[0]['goods_commonid']);
		$xianshi_info['goods_commonid'] = $commonid[0]['goods_commonid'];
     ...
     *
     */
    public function getIndexXianshiGoodsCommendList($count = 1000) {
        $prefix = 'home_index_xianshi_goods_commend';

        $item_list = rcache($count, $prefix);
        //缓存有效
        if(!empty($item_list)) {
            return unserialize($item_list['home_index_xianshi_goods_commend']);
        }

        //缓存无效查库并缓存
        $item_list = $this->getXianshiGoodsCommendList($count);

        $cache = array('home_index_xianshi_goods_commend' => serialize($item_list));
