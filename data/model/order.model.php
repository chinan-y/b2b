...
class orderModel extends Model {
    ...
    public function userNormalOrderList($condition, $pagesize = '', $field = '*', $order, $limit = '', $extend = array()){
        $condition['delete_state'] = 0;
        return $this->userOrderList($condition, $pagesize, $field, $order, $limit, $extend);
    }
    /**
     * 取得订单列表(所有)
     * @param unknown $condition
    ...
    public function getOrderList($condition, $pagesize = '', $field = '*', $order = 'order_id desc', $limit = '', $extend = array(), $master = false){
        ...
        foreach ($list as $k =>$order) {
			...
			$refer_id = $this->table('member')->field('refer_id')->where(array('member_id'=>$order['buyer_id']))->select();
			$ref_name = $this->table('member')->field('member_name')->where(array('member_id'=>$refer_id[0]['refer_id']))->select();
			$order['ref_name'] = $ref_name[0]['member_name'];
			$list[$k]['ref_name'] = $order['ref_name'];
        }
        ...
        if (in_array('order_goods',$extend)) {
            ...
            if (!empty($order_goods_list)) {
				foreach ($order_goods_list as $value) {
					...
					$value = array_merge($goods_info, $value);
    ...
    public function userOrderList($condition, $pagesize = '', $field = '*', $order, $limit = '', $extend = array(), $master = false){
        $list = $this->table('order')->field($field)->where($order)->page($pagesize)->limit($limit)->master($master)->select();
        if (empty($list)) return array();
        $order_list = array();
     ...
     * @param string $key
     */
    public function getOrderGoodsList($condition = array(), $fields = '*', $limit = null, $page = null, $order = 'rec_id asc', $group = null, $key = null) {
	 ...
	 * @param unknown_type $condition
	 */
	public function editOrderTrade($data,$condition) {
	...
	/**
	 * 取消订单得返利(订单付款增加返利后，这个取消仅限于取消平台和区域合作方的返利)
	 */
	public function reason($reason){
		...
		$order['one_rebate'] = NULL;
		$order['two_rebate'] = NULL;
		$order['three_rebate'] = NULL;
		$order['platform_rebate'] = NULL;
		$order['area_rebate'] = NULL;
		$result = Model()->table('order')->where(array('order_id'=>$order_id))->update($order);
