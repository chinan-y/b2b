...
class addressModel extends Model {
	...
	public function addAddress($param){
		/**
		 *
		 * 增加提交订单时候，把时间增加到member_mobile字段里
		 */
		 
		$mob_phone = $param['mob_phone'];
		$member_id = $param['member_id'];
		$is_mobile = Model()->table('member')->where(array('member_mobile' => $mob_phone))->find();
		$member_mobile = Model()->table('member')->where(array('member_id' => $member_id))->find();
		if($member_mobile['member_mobile'] == '' && !$is_mobile){
			$data = array('member_mobile' => $mob_phone,'member_mobile_bind' => 1);
			Model()->table('member')->where(array('member_id' => $member_id)) -> update($data);
		}
