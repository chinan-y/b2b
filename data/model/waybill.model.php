...
class waybillModel extends Model{
    ...
    private function _waybill_image_upload() {
		...
		if(C(OSS_IS_STORAGE) == 1){
			...
			if(!empty($_FILES['waybill_image']['name'])){
				$extend 	= pathinfo($_FILES['waybill_image']['name']); 
				$extend 	= strtolower($extend["extension"]); 
				$imgname	= 'wb_'.date('YmdHis').rand(0,99);
				$imgfullname= $imgname . '.' . $extend;
				$objectname = DIR_UPLOAD.DS.ATTACH_WAYBILL.DS.$imgfullname;
				$pathname 	= $_FILES['waybill_image']['tmp_name'];
				if($_FILES['waybill_image']['type'] =='image/gif' || $_FILES['waybill_image']['type'] =='image/jpeg' || $_FILES['waybill_image']['type'] =='image/png'){
					$result = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
					if ($result){
						return $imgfullname;
					}else{
						return array('error' => 'Waybill upload error!');
					}
				}
			}
		}
    ...
    public function delWaybillImage($image_name) {
		...
		if(C(OSS_IS_STORAGE) == 1){
			$oss = Logic('oss');
			$image = DIR_UPLOAD . DS . ATTACH_WAYBILL . DS . $image_name;
			$oss ->deleteObject(IMAGE_BUCKET_NAME, $image);
		}
    }
