...
class groupbuyModel extends Model{
	...
	public function userGroupbuyList($condition, $page = null, $order = 'state asc', $field = '*', $limit = 0) {
	    return $this->field($field)->where($condition)->page($page)->order("end_time asc")->limit($limit)->select();
	}
	...
	public function userGroupbuyExtendList($condition, $page = null, $order, $field = '*', $limit = 0) {
	    
	    $groupbuy_list = $this->where($condition)->order($order)->limit($limit)->select();
     ...
     *
     */
    public function getGroupbuyOnlineList($condition, $page = null, $order="", $field = '*') {
    ...
    public function userGroupbuyOnlineList($condition, $page = null, $order="", $field = '*', $limit = 100) {
        $where = array();
		if($condition['be_doing'] == 1){
			$where['start_time'] = array('lt', TIMESTAMP);
			$where['end_time'] = array('gt', TIMESTAMP);
		}else if($condition['time_num']){
			$where['time_num'] = $condition['time_num'];
		}
		
        return $this->userGroupbuyExtendList($where, $page, $order, $field, $limit);
    }
        ...
        switch ($groupbuy_info['state']) {
            ...
            case self::GROUPBUY_STATE_NORMAL:
                if($groupbuy_info['start_time'] > TIMESTAMP) {
                    ...
                    $groupbuy_info['state_flag'] = 'close';
                    $groupbuy_info['button_text'] = '已结束';
					$groupbuy_info['count_down_text'] = '本场已结束';
                } else {
                    $groupbuy_info['state_flag'] = 'buy-now';
                    $groupbuy_info['button_text'] = '抢购中';
     ...
     *
     */
    public function getIndexGroupbuyCommendedList($count) {
        $prefix = 'home_index_groupbuy';

        $item_list = rcache($count, $prefix);
        //缓存有效
        if(!empty($item_list)) {
            return unserialize($item_list['groupbuy']);
        }
        ...
        $cache = array('groupbuy' => serialize($item_list));
