...
class web_configModel extends Model{
	 ...
	 *
	 */
	public function getIndexWebHtml($web_page = 'index') {
		$prefix = 'home_index_web_html';

		$item_list = rcache($web_page, $prefix);
		//缓存有效
		if(!empty($item_list)) {
			return unserialize($item_list['home_index_web_html']);
		}

		//缓存无效查库并缓存
		$item_list = $this->getWebHtml($web_page);

		$cache = array('home_index_web_html' => serialize($item_list));
		wcache($web_page, $cache, $prefix);
