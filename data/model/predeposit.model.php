...
class predepositModel extends Model {
     ...
     * @param array $data
     */
    public function addPdRebate($data) {
        return $this->table('pd_recharge')->insert($data);
    }
    ...
    public function getPdLogList($condition = array(), $pagesize = '', $fields = '*', $order = '', $limit = '') {
		if($condition['lg_desc']){
			$condition	= " and `pd_log`.lg_desc like '%{$condition['lg_desc']}%'";
			$param	= array();
			$param['table']	= 'pd_log';
			$param['where']	= $condition;
			$param['field'] = $fields;
			$param['order'] = $order ? $order : 'pd_log.lg_id desc';
			$param['limit'] = $limit;
			return Db::select($param,$pagesize);
		} 
    ...
    public function changePd($change_type,$data = array()) {
        ...
        switch ($change_type){
            case 'order_pay':
                ...
                $data_log['lg_desc'] = '下单，支付预存款，订单号: '.$data['order_sn'].'，描述信息：'.$data['lg_desc'];
            ...
            case 'order_freeze':
                ...
                $data_log['lg_desc'] = '下单，冻结预存款，订单号: '.$data['order_sn'].'，描述信息：'.$data['lg_desc'];
				$data_log['lg_admin_name'] = $data['admin_name'];
            ...
            case 'order_cancel':
                $data_log['lg_av_amount'] = $data['amount'];
                $data_log['lg_freeze_amount'] = -$data['amount'];
                $data_log['lg_desc'] = '取消订单，解冻预存款，订单号: '.$data['order_sn'].'，描述信息：'.$data['lg_desc'];
				$data_log['lg_admin_name'] = $data['admin_name'];
                $data_pd['freeze_predeposit'] = array('exp','freeze_predeposit-'.$data['amount']);
                $data_pd['available_predeposit'] = array('exp','available_predeposit+'.$data['amount']);
            ...
            case 'order_comb_pay':
                $data_log['lg_freeze_amount'] = -$data['amount'];
                $data_log['lg_desc'] = '下单，支付被冻结的预存款，订单号: '.$data['order_sn'].'，描述信息：'.$data['lg_desc'];
				$data_log['lg_admin_name'] = $data['admin_name'];
                $data_pd['freeze_predeposit'] = array('exp','freeze_predeposit-'.$data['amount']);
            ...
            case 'recharge':
                $data_log['lg_av_amount'] = $data['amount'];
                $data_log['lg_desc'] = '充值，充值单号: '.$data['pdr_sn'].'，描述信息：'.$data['lg_desc'];
                $data_log['lg_admin_name'] = $data['admin_name'];
                $data_pd['available_predeposit'] = array('exp','available_predeposit+'.$data['amount']);
			...
			case 'salerebate':
                $data_log['lg_av_amount'] = $data['amount'];
                $data_log['lg_desc'] = '销售返利提现，订单号: '.$data['order_sn'].'，描述信息：'.$data['lg_desc'];
				$data_log['lg_admin_name'] = $data['admin_name'];
                $data_pd['available_predeposit'] = array('exp','available_predeposit+'.$data['amount']);
            ...
            case 'refund':
                $data_log['lg_av_amount'] = $data['amount'];
                $data_log['lg_desc'] = '确认退款，订单号: '.$data['order_sn'].'，描述信息：'.$data['lg_desc'];
				$data_log['lg_admin_name'] = $data['admin_name'];
                $data_pd['available_predeposit'] = array('exp','available_predeposit+'.$data['amount']);
            ...
            case 'vr_refund':
                $data_log['lg_av_amount'] = $data['amount'];
                $data_log['lg_desc'] = '虚拟兑码退款成功，订单号: '.$data['order_sn'].'，描述信息：'.$data['lg_desc'];
				$data_log['lg_admin_name'] = $data['admin_name'];
                $data_pd['available_predeposit'] = array('exp','available_predeposit+'.$data['amount']);
            ...
            case 'cash_apply':
                $data_log['lg_av_amount'] = -$data['amount'];
                $data_log['lg_freeze_amount'] = $data['amount'];
                $data_log['lg_desc'] = '申请提现，冻结预存款，提现单号: '.$data['order_sn'].'，描述信息：'.$data['lg_desc'];
				$data_log['lg_admin_name'] = $data['admin_name'];
                $data_pd['available_predeposit'] = array('exp','available_predeposit-'.$data['amount']);
                $data_pd['freeze_predeposit'] = array('exp','freeze_predeposit+'.$data['amount']);
            ...
            case 'cash_pay':
                $data_log['lg_freeze_amount'] = -$data['amount'];
                $data_log['lg_desc'] = '提现成功，提现单号: '.$data['order_sn'].'，描述信息：'.$data['lg_desc'];
                $data_log['lg_admin_name'] = $data['admin_name'];
                $data_pd['freeze_predeposit'] = array('exp','freeze_predeposit-'.$data['amount']);
            ...
            case 'cash_del':
                $data_log['lg_av_amount'] = $data['amount'];
                $data_log['lg_freeze_amount'] = -$data['amount'];
                $data_log['lg_desc'] = '取消提现申请，解冻预存款，提现单号: '.$data['order_sn'].'，描述信息：'.$data['lg_desc'];
                $data_log['lg_admin_name'] = $data['admin_name'];
                $data_pd['available_predeposit'] = array('exp','available_predeposit+'.$data['amount']);
                $data_pd['freeze_predeposit'] = array('exp','freeze_predeposit-'.$data['amount']);
		...
		//增加管理员调整预存款	BY MING
				case 'sys_add_money':
                $data_log['lg_av_amount'] = $data['amount'];
                $data_log['lg_desc'] = '管理员调节预存款【增加】，充值单号: '.$data['pdr_sn'].'，描述信息：'.$data['lg_desc'];
                $data_log['lg_admin_name'] = $data['admin_name'];
                $data_pd['available_predeposit'] = array('exp','available_predeposit+'.$data['amount']);
				...
				case 'sys_del_money':
                $data_log['lg_av_amount'] = -$data['amount'];
                $data_log['lg_desc'] = '管理员调节预存款【减少】，充值单号: '.$data['pdr_sn'].'，描述信息：'.$data['lg_desc'];
				$data_log['lg_admin_name'] = $data['admin_name'];
                $data_pd['available_predeposit'] = array('exp','available_predeposit-'.$data['amount']);
                ...
                $data_log['lg_av_amount'] = -$data['amount'];
                $data_log['lg_freeze_amount'] = $data['amount'];
				$data_log['lg_desc'] = '管理员调节预存款【冻结】，充值单号: '.$data['pdr_sn'].'，描述信息：'.$data['lg_desc'];
				$data_log['lg_admin_name'] = $data['admin_name'];
                $data_pd['available_predeposit'] = array('exp','available_predeposit-'.$data['amount']);
                $data_pd['freeze_predeposit'] = array('exp','freeze_predeposit+'.$data['amount']);
                ...
                $data_log['lg_av_amount'] = $data['amount'];
                $data_log['lg_freeze_amount'] = -$data['amount'];
                $data_log['lg_desc'] = '管理员调节预存款【解冻】，充值单号: '.$data['pdr_sn'].'，描述信息：'.$data['lg_desc'];
                $data_log['lg_admin_name'] = $data['admin_name'];
                $data_pd['available_predeposit'] = array('exp','available_predeposit+'.$data['amount']);
                $data_pd['freeze_predeposit'] = array('exp','freeze_predeposit-'.$data['amount']);
        ...
        if($tpl_info['mmt_wechat_switch']) {
            $member_id = $data['member_id'];
            $member_info = Model('member')->getMemberInfo(array('member_id'=>$member_id),array('member_wechatopenid','available_predeposit','member_name'));
            $param['openid'] = $member_info['member_wechatopenid'];
            // 判断对应的openid是否存在
            if($param['openid']) {
                $param['member_name'] = $member_info['member_name'];
                $param['available_predeposit'] = ncPriceFormat($member_info['available_predeposit']); 
                $param['av_amount'] = ncPriceFormat($data_msg['av_amount']);
                $param['freeze_amount'] = ncPriceFormat($data_msg['freeze_amount']);
                $param['desc'] = $data_log['lg_desc'];
                $param['payment_time'] = TIMESTAMP;
