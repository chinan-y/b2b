...
class voucherModel extends Model {
	...
	public function getCurrentAvailableVoucher($condition = array(), $goods_total = 0) {
	   ...
	   foreach ($voucher_list as $key => $voucher) {
	       if ($goods_total < $voucher['voucher_limit']) {
			   ...
			   if($voucher['voucher_design_sku']<=0){
			   $voucher_list[$key]['desc1'] = sprintf('面额%s元　至%s',$voucher['voucher_price'],date('Y-m-d',$voucher['voucher_end_date']));
			   }else
			   {
			   $voucher_list[$key]['desc2'] = sprintf('面额%s元　至%s',$voucher['voucher_price'],date('Y-m-d',$voucher['voucher_end_date']));
			   $voucher_list[$key]['sku']=$voucher['voucher_design_sku'];
			   }
	       }
	   }
	   return $voucher_list;
	...
	public function getCanChangeTemplateInfo($vid,$member_id,$store_id = 0){
	    ...
	    //查询代金券对应的店铺信息 0为平台通用代金券店铺ID
		if($template_info['voucher_t_store_id']==0){
			$store_info = Model('store')->getStoreInfoByID(0);
			}
		else {
	...
	/**
	 * 查询可兑换代金券模板详细信息，一键领取全部免费代金券
	 */
	public function getFreeVoucherInfo($vid,$member_id,$store_id = 0){
	    if ($vid <= 0 || $member_id <= 0 ){
	        return array('state'=>false,'msg'=>'参数错误');
	    }
	    //查询可用代金券模板
	    $where = array();
	    $where['voucher_t_id'] = array('in',$vid['voucher_t_id'][1]);
	    $where['voucher_t_state'] = $this->templatestate_arr['usable'][0];
	    $where['voucher_t_end_date'] = array('gt',time());
	    $template_info = $this->table('voucher_template')->field('*')->where($where)->select();
		foreach($template_info as $val){
			if (empty($val) || $val['voucher_t_total']<=$val['voucher_t_giveout']){//代金券不存在或者已兑换完
				return array('state'=>false,'msg'=>'代金券信息错误');
			}
			//验证是否为店铺自己
			if ($store_id > 0 && $store_id == $val['voucher_t_store_id']){
				return array('state'=>false,'msg'=>'不可以兑换自己店铺的代金券');
			}
			//查询代金券对应的店铺信息 0为平台通用代金券店铺ID
			if($template_info['voucher_t_store_id']==0){
			...
			if (!empty($val['voucher_t_customimg'])){
				$val['voucher_t_customimg'] = UPLOAD_SITE_URL.DS.ATTACH_VOUCHER.DS.$val['voucher_t_store_id'].DS.$val['voucher_t_customimg'];
			}else{
				$val['voucher_t_customimg'] = UPLOAD_SITE_URL.DS.defaultGoodsImage(240);
			}
		}
	    return array('state'=>true,'info'=>$template_info);
	}
    ...
    public function getMemberVoucherList($member_id, $voucher_state, $page = null) {
		...
		$field = 'voucher_id,voucher_code,voucher_title,voucher_desc,voucher_start_date,voucher_end_date,voucher_price,voucher_limit,voucher_state,voucher_design_sku,voucher_order_id,voucher_store_id,voucher_give_id,store_name,store_id,store_domain,voucher_t_customimg';
		...
		$list = $this->table('voucher,store,voucher_template')->field($field)->join('left,left')->on($on)->where($where)->order('voucher_id desc')->page($page)->select();

		if(!empty($list) && is_array($list)){
			foreach ($list as $key=>$val){
                //代金券图片
				if (empty($val['voucher_t_customimg']) || !file_exists(BASE_UPLOAD_PATH.DS.ATTACH_VOUCHER.DS.$val['voucher_store_id'].DS.$val['voucher_t_customimg'])){
					$list[$key]['voucher_t_customimg'] = UPLOAD_SITE_URL.DS.defaultGoodsImage(60);
				}else{
					$list[$key]['voucher_t_customimg'] = UPLOAD_SITE_URL.DS.ATTACH_VOUCHER.DS.$val['voucher_store_id'].DS.$val['voucher_t_customimg'];
				}
                //代金券状态文字
     ...
     * @param int $num 查询条数
     */
	public function getRegisterTemplate($num){
        //查询店铺分类
        $store_class = rkcache('store_class', true);

        //查询推荐的热门代金券列表
        $where = array();
        $where['voucher_t_give'] = '1';
        $where['voucher_t_points'] = '0';
        $where['voucher_t_state'] = $this->templatestate_arr['usable'][0];
        $where['voucher_t_end_date'] = array('gt',time());
        $recommend_voucher = $this->getVoucherTemplateList($where, $field = '*', $num, 0, 'voucher_t_id desc');
        if ($recommend_voucher){
    ...
    public function exchangeVoucher($template_info, $member_id, $member_name = ''){
		...
		$insert_arr['voucher_design_sku'] = $template_info['voucher_t_sku'];//增加代金券券指定商品sku
     ...
     * 一键领取全部免费代金券
     */
	public function exFreeVoucher($template_info, $member_id, $member_name = ''){
        if (intval($member_id) <= 0 || empty($template_info)){
            return array('state'=>false,'msg'=>'参数错误');
        }
        //查询会员信息
		...
		foreach($template_info as $val){
			$re = $this->table('voucher')->where(array('voucher_t_id'=>$val['voucher_t_id'],'voucher_owner_id'=>$member_id))->select();
			if(!$re){
				$insert_arr = array();
				$insert_arr['voucher_code'] = $this->get_voucher_code();
				$insert_arr['voucher_t_id'] = $val['voucher_t_id'];
				$insert_arr['voucher_title'] = $val['voucher_t_title'];
				$insert_arr['voucher_desc'] = $val['voucher_t_desc'];
				$insert_arr['voucher_start_date'] = time();
				$insert_arr['voucher_end_date'] = $val['voucher_t_end_date'];
				$insert_arr['voucher_price'] = $val['voucher_t_price'];
				$insert_arr['voucher_limit'] = $val['voucher_t_limit'];
				$insert_arr['voucher_store_id'] = $val['voucher_t_store_id'];
				$insert_arr['voucher_design_sku'] = $val['voucher_t_sku'];//增加代金券券指定商品sku
				$insert_arr['voucher_state'] = 1;
				$insert_arr['voucher_active_date'] = time();
				$insert_arr['voucher_owner_id'] = $member_id;
				$insert_arr['voucher_owner_name'] = $member_name;
				...
				$this->editVoucherTemplate(array('voucher_t_id'=>$val['voucher_t_id']), array('voucher_t_giveout'=>array('exp','voucher_t_giveout+1')));
				wcache($member_id, array('voucher_count' => array('exp','voucher_count+1')), 'm_voucher');
			}
		}
		if ($result){
	 ...
	 /**
     * 查询当前用户是否拥有该代金券
     * @param int $member_id      当前用户
     * @param int $voucher_t_id   代金券id
     */
	 public function getOwner($member_id,$voucher_t_id){
		 $owner=Model()->table('voucher')->where(array('voucher_t_id'=>$voucher_t_id,'voucher_owner_id'=>$member_id))->find();
