...
class refund_returnModel extends Model{
	...
	public function editRefundReturn($condition, $data) {
		...
		if (is_array($data)) {
			$result = $this->table('refund_return')->where(array('refund_id'=>$condition['refund_id']))->update($data);
			if($condition['is_repeal']){
			$member_model = Model('member');
			$re = $this->getRefundReturnInfo(array('refund_id'=>$condition['refund_id']), 'order_id,buyer_id');
			$member_id = $re['buyer_id'];
			$member = $member_model->getMemberSuperior(array('member_id'=> $member_id));
			$member_info = $member_model->getMemberInfo(array('member_id' => $member_id));
			$order_info = Model('order')->getOrderInfo(array('order_id'=>$re['order_id']));
			...
			if($member_info['is_seller'] == 1 && $member_info['is_rebate'] == 1 && $is_rebate == 1){
				//只有一级返利
				if($one_rebate>0 && $two_rebate==0 && $three_rebate==0){
					if($member['one_id'] == NULL){
						$rebate_data = array();
						...
						Model('salescredit')->saveSalescreditLog('refund',$rebate_data);
						...
						$one_data = array();
						$one_data['sc_memberid'] = $member['one_id'];
						$one_name = $member_model->getMemberInfo(array('member_id' => $member['one_id']),'member_name');
						...
						Model('salescredit')->saveSalescreditLog('refund',$one_data);
					}
				//一级返利和二级返利
				}else if($one_rebate>0 && $two_rebate>0 && $three_rebate==0){
					if($member['one_id'] == NULL && $member['two_id'] == NULL){
						$rebate_data = array();
						$rebate_data['sc_memberid'] = $member_id;
						$rebate_data['sc_membername'] = $order_info['buyer_name'];
						$rebate_data['buyer_name'] = $order_info['buyer_name'];
					...
					}else if($member['one_id']>0 && $member['two_id'] == NULL){
						$rebate_data = array();
						$rebate_data['sc_memberid'] = $member_id;
						$rebate_data['sc_membername'] = $order_info['buyer_name'];
						$rebate_data['buyer_name'] = $order_info['buyer_name'];
					...
					}else if($member['one_id']>0 && $member['two_id']>0){
						$rebate_data = array();
						$rebate_data['sc_memberid'] = $member_id;
						$rebate_data['sc_membername'] = $order_info['buyer_name'];
						$rebate_data['buyer_name'] = $order_info['buyer_name'];
						...
						$two_data = array();
						$two_data['sc_memberid'] = $member['two_id'];
						$two_name = $member_model->getMemberInfo(array('member_id' => $member['two_id']),'member_name');
						...
						Model('salescredit')->saveSalescreditLog('refund',$two_data);
					}
				//三级返利都有
				}else if($one_rebate>0 && $two_rebate>0 && $three_rebate>0){
					if($member['one_id'] == NULL && $member['two_id'] == NULL && $member['three_id'] == NULL){
						$rebate_data = array();
						$rebate_data['sc_memberid'] = $member_id;
						$rebate_data['sc_membername'] = $order_info['buyer_name'];
						$rebate_data['buyer_name'] = $order_info['buyer_name'];
					...
					}else if($member['one_id']>0 && $member['two_id'] == NULL && $member['three_id'] == NULL){
						$rebate_data = array();
						$rebate_data['sc_memberid'] = $member_id;
						$rebate_data['sc_membername'] = $order_info['buyer_name'];
						$rebate_data['buyer_name'] = $order_info['buyer_name'];
					...
					}else if($member['one_id']>0 && $member['two_id']>0 && $member['three_id'] == NULL){
						$rebate_data = array();
						$rebate_data['sc_memberid'] = $member_id;
						$rebate_data['sc_membername'] = $order_info['buyer_name'];
						$rebate_data['buyer_name'] = $order_info['buyer_name'];
					...
					}else if($member['one_id']>0 && $member['two_id']>0 && $member['three_id']>0){
						$rebate_data = array();
						$rebate_data['sc_memberid'] = $member_id;
						$rebate_data['sc_membername'] = $order_info['buyer_name'];
						$rebate_data['buyer_name'] = $order_info['buyer_name'];
						...
						$three_data = array();
						$three_data['sc_memberid'] = $member['three_id'];
						$three_name = $member_model->getMemberInfo(array('member_id' => $member['three_id']),'member_name');
						...
						Model('salescredit')->saveSalescreditLog('refund',$three_data);
					}
				}
				//合作方返利
				if($order_info['platform_member_id']>0 && $platform_rebate>0){
					...
					$member_name = $member_model->getMemberInfo(array('member_id' => $order_info['platform_member_id']),'member_name');
					$rebate_data['sc_membername'] = $member_name['member_name'];
					$rebate_data['buyer_name'] = $order_info['buyer_name'];
					$rebate_data['order_sn'] = $order_info['order_sn'];
					$rebate_data['rebate_amount'] = $platform_rebate ;
					Model('salescredit')->saveSalescreditLog('refund',$rebate_data);
				}
				//区域方返利
				if($order_info['area_member_id']>0 && $area_rebate>0){
					...
					$member_name = $member_model->getMemberInfo(array('member_id' => $order_info['area_member_id']),'member_name');
					$rebate_data['sc_membername'] = $member_name['member_name'];
					$rebate_data['buyer_name'] = $order_info['buyer_name'];
					$rebate_data['order_sn'] = $order_info['order_sn'];
			...
			}else if($member_info['is_seller'] == 1 && $member_info['is_member_rebate'] == 1 && $is_rebate == 1){
				$rebate_data = array();
				$rebate_data['sc_memberid'] = $member_id;
				$rebate_data['sc_membername'] = $order_info['buyer_name'];
				$rebate_data['buyer_name'] = $order_info['buyer_name'];
