...
class albumModel extends Model{
			...
			foreach($pic_list as $v){
			    foreach ($image_ext as $ext) {
					...
					if(C(OSS_IS_STORAGE) == 1){
						$oss = Logic('oss');
						$oss->deleteObject(IMAGE_BUCKET_NAME, DIR_UPLOAD.DS.ATTACH_GOODS.DS.$id.DS.$v['apic_cover']);
					}
			    }
			}
		}
	 ...
	 * @return bool
	 */
	public function delPic($id, $store_id, $goods_commonid){
        ...
        if(!empty($pic_list) && is_array($pic_list)){
			...
			foreach($pic_list as $v){
				if(C(OSS_IS_STORAGE) == 0){
					if (!file_exists(BASE_UPLOAD_PATH.DS.ATTACH_GOODS.DS.$store_id.DS.$goods_commonid.DS.$v['apic_cover'])){
						@unlink(BASE_UPLOAD_PATH.DS.ATTACH_GOODS.DS.$store_id.DS.$v['apic_cover']);
					}else{
						@unlink(BASE_UPLOAD_PATH.DS.ATTACH_GOODS.DS.$store_id.DS.$goods_commonid.DS.$v['apic_cover']);
					}
					foreach ($image_ext as $ext) {
						$file = str_ireplace('.', $ext . '.', $v['apic_cover']);
						if (!file_exists(BASE_UPLOAD_PATH.DS.ATTACH_GOODS.DS.$store_id.DS.$goods_commonid.DS.$file)){
							@unlink(BASE_UPLOAD_PATH.DS.ATTACH_GOODS.DS.$store_id.DS.$file);
						}else{
							 @unlink(BASE_UPLOAD_PATH.DS.ATTACH_GOODS.DS.$store_id.DS.$goods_commonid.DS.$file);
						}
					}
				}
				if(C(OSS_IS_STORAGE) == 1){
					$oss =Logic('oss');
					$oss->deleteObject(IMAGE_BUCKET_NAME, DIR_UPLOAD.DS.ATTACH_GOODS.DS.$store_id.DS.$goods_commonid.DS.$v['apic_cover']);
				}
			}
		}
		if(!empty($id)) {
