...
class cartModel extends Model {
	...
	private function _addCartDb($goods_info = array(),$quantity) {
		...
		$array['store_from']	= $goods_info['store_from']; //by  liu
		$array['goods_id']	= $goods_info['goods_id'];
		$array['goods_name'] = $goods_info['goods_name'];
		$array['goods_price'] = $goods_info['goods_price'];
		$array['goods_weight'] = $goods_info['goods_weight'];
		$array['goods_taxes'] = $goods_info['goods_taxes'];
	...
	private function _addCartCookie($goods_info = array(), $quantity = null) {
		...
		$cart_array[$goods_info['goods_id']] = array(
		  ...
		  'store_from' => $goods_info['store_from'],//by liu
	...
	public function editCart($data,$condition) {
		if($condition['buyer_id']){
			...
			$cart_str = base64_decode(decrypt($cart_str));
			$cart_array = @unserialize($cart_str);
			if (key_exists($data['goods_id'],(array)$cart_array)) {
				$cart_array[$data['goods_id']]['goods_num'] = $data['goods_num'];
			}
			setNcCookie('cart',encrypt(base64_encode(serialize($cart_array))),24*3600);
			$result = true;
		}
	...
	public function listCart($type, $condition = array(), $limit = '') {
		...
		if(is_array($cart_list)) {
			foreach ($cart_list as $val) {
				...
				$cart_all_taxes += $val['goods_taxes'] * $val['goods_num'];
			}
		}
        $this->cart_all_price = ncPriceFormat($cart_all_price);
		$this->cart_all_taxes = ncPriceFormat($cart_all_taxes);
	    ...
	    if ($type == 'db') {
		  ...
		  $this->cart_all_taxes = ncPriceFormat($cart_all_taxes);
    		...
    		foreach ($cart_array as $v){
				...
				$cart_all_taxes += floatval($v['goods_taxes'])*intval($v['goods_num']);
    		}
    		$this->cart_all_price = $cart_all_price;
			$this->cart_all_taxes = $cart_all_taxes;
	    }
	 ...
	 * @return array
	 */
	public function cartSplit($store_cart_list){
		$split = false;
		$new_cart_list = array();
		...
		foreach($store_cart_list as $store_id=>$goods_list){
			foreach($goods_list as $goods_info){
				$new_cart_list[$store_id][$goods_info['transport_id']][] = $goods_info;
			}
			if(count($new_cart_list[$store_id])>1){
				$split = true;
			}
		}
		return array($split, $new_cart_list, $regions);
	}
}
