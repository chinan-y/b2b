...
class goodsModel extends Model{
    ...
    const PROMOTION_TYPE_NULL = 0;    // 未促销
    const PROMOTION_TYPE_GROUNP_BUY = 1;    // 团购促销
    const PROMOTION_TYPE_PROMOTION_XIANSHI = 2;    // 限时促销
     ...
     * @return array
     */
    public function getTaxNoList($condition, $field = '*') {
    ...
    public function editProducesLockUp($update, $condition) {
        ...
        if ($return) {
            // 商品违规下架发送店铺消息
            $common_list = $this->getGoodsCommonList($condition, 'goods_commonid,goods_serial,store_id,goods_stateremark', 0);
            foreach ($common_list as $val) {
                $param = array();
                $param['remark'] = $val['goods_stateremark'];
                $param['goods_serial'] = $val['goods_serial'];
	...
	public function getGoodsIn($condition, $field = '*') {
        return $this->table('goods')->field($field)->where($condition)->select();
    }
    /**
     * 获取单条商品SKU信息及其促销信息
     *
    ...
    public function getGoodsOnlineListAndPromotionByIdArray($goodsid_array) {
        ...
        foreach ($goodsid_array as $goods_id) {
            $goods_info = $this->getGoodsOnlineInfoAndPromotionById($goods_id);
			if($goods_info){
				$common_info = $this->getGoodeCommonInfoByID($goods_info['goods_commonid']);
				$goods_info['adjust_rebate_rate'] = $common_info['adjust_rebate_rate'];
			}
     ...
     * 
     */
	public function getGoodsAppointSatedate($goods_commonid){
		return  $this->table('goods_common')->where($goods_commonid)->field('appoint_satedate,goods_commonid,store_id')->select();
	}
	...
	public function getGoodsInfoID($goods_id, $fields = '*') {
        $goods_info = $this->_rGoodsCache($goods_id, $fields);
        if (empty($goods_info)) {
            $goods_info = $this->getGoodsIn(array('goods_id'=>$goods_id));
            $this->_wGoodsCache($goods_id, $goods_info);
        }
        return $goods_info;
    }
    ...
    public function getGoodsDetail($goods_id) {
		...
		$post_tax_no = $this ->post_tax_no($goods_info);
		$goods_info['post_tax_rate'] =$post_tax_no[0]['POST_TAX_RATE'] * 100 . '%';
...
//		$uu[] = array('name' => '发货仓库','1' => $goods_info['address_name']);
        ...
        foreach ($spec_array as $key => $value) {
            ...
            $tpl_spec['url'] = urlShop('goods', 'index', array('goods_id' => $value['goods_id'],'ref' => $_SESSION['member_id']));//增加refid
            $spec_list[] = $tpl_spec;
            $spec_list_mobile[$spec_sign] = $value['goods_id'];
			$commonid = $this->getGoodsIn(array('goods_id' => $value['goods_id']), 'goods_commonid');
		...
		if(C(OSS_IS_STORAGE) == 0){
			if (!empty($image_more)) {
				foreach ($image_more as $val) {
					$goods_image[] = array(cthumb($val['goods_image'], 240, $goods_info['store_id'], $goods_info['goods_commonid']),cthumb($val['goods_image'], 360, $goods_info['store_id'], $goods_info['goods_commonid']),cthumb($val['goods_image'], 1280, $goods_info['store_id'], $goods_info['goods_commonid']));
					$goods_image_mobile[] = cthumb($val['goods_image'], 360, $goods_info['store_id'], $goods_info['goods_commonid']);
				}
			} else {
	 ...
	 * 查询商品的税率
	 */
	public function post_tax_no($goods_info){
		$post_tax_no = Model()->table('goods_post_tax')->where(array('POST_TAX_NO' => $goods_info['post_tax_no']))->select();
		return $post_tax_no;
	}
	 ...
	 * 查询商品的原产国
	 */
	public function getGoodsMessCountry($country_code){
		$mess_country = Model()->table('mess_country_code')->where(array('code' => $country_code))->select();
		return $mess_country;
	}
     ...
     *
     */
    public function getIndexGoodsCommendList($curpage) {
        $prefix = 'home_index_commend';

        $item_list = rcache($curpage, $prefix);
        //缓存有效
        if(!empty($item_list)) {
            return unserialize($item_list['commend']);
        }
        ...
        $condition['is_fcode'] = '0';
        $condition['is_presell'] = '0';
        $condition['goods_promotion_type'] = array('neq', '1');

        $condition['goods_state']   = self::STATE1;
        $condition['goods_verify']  = self::VERIFY1;
        $item_list = $this->where($condition)->field('goods_id,goods_name,store_id,store_name,store_from,goods_image,goods_price,goods_promotion_price,goods_salenum,evaluation_count,goods_promotion_type,evaluation_good_star')->order('goods_edittime DESC')->page(15)->limit()->select();
        if(!empty($item_list)) {
            foreach ($item_list as $key=>$value) {
                //处理图片
                $item_data[$key]['href'] = urlShop('goods', 'index', array('goods_id' => $value['goods_id'],'ref'=>$_SESSION['member_id']));
                $item_data[$key]['goods_image'] = cthumb($value['goods_image']);
            }
        }
        $cache = array('commend' => serialize($item_list));
        wcache($curpage, $cache, $prefix);
        return $item_list;
    }
	 ...
	 * 
	 */
	public function getGood_Sadjust_rebate_rate($goods_commonid){
		$good_adjust_rebate_rate = Model()->table('goods_common')->where(array('goods_commonid' => $goods_commonid))->field('adjust_rebate_rate')->find();
		return $good_adjust_rebate_rate;
	}
     ...
     * @param string $table 表名
     */
    public function addSalesstaffGoods($insert) {
        $re= Model()->table('salesstaff_goods')->insert($insert);
        return $re;
    }
	 ...
	 * 查询二维码销售员已上架商品(防重复上架)
	 */
	public function getSalesstaffGoods($sale_goods){
		$goods_list = Model()->table('salesstaff_goods')->where(array('member_id' => $sale_goods['member_id'] , 'goods_id' => $sale_goods['goods_id']))->select();
		return $goods_list;
	}
	 ...
	 * 查询二维码销售员我的商品
	 */
	public function getSalesstaffGoodsList($sale_goods){
		$goods_list = Model()->table('salesstaff_goods')->where(array('member_id' => $sale_goods['member_id']))->select();
		return $goods_list;
	}
     ...
     * @return boolean
     */
    public function editMyGoodsSoldoutPutaway($update, $condition) {
        return $this->table('salesstaff_goods')->where($condition)->update($update);
    }
     ...
     * @return boolean
     */
    public function deleteMyGoods($condition) {
        return $this->table('salesstaff_goods')->where($condition)->delete();
    }
