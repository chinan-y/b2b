...
<div id="footer" class="wrapper">
  ...
  <?php echo $output['setting_config']['gcwebshop_version'];?> <?php echo $output['setting_config']['icp_number']; ?><br />
...
function load_cart_information(){
	$.getJSON('index.php?gct=cart&gp=ajaxcart', function(result){
	    if(result){
	        var result  = result;
	       	...
	       	if(result.goods_all_num >0){
	            ...
	            for (i = 0; i < data.length; i++)
	            {
	            	...
	            	html+="<td class='name'><a href='<?php echo SiteUrl.'/';?>index.php?gct=goods&goods_id="+data[i]['goodsid']+"' title='"+data[i]['gname']+"' target='_top'>"+data[i]['gname']+"</a></td>";
...
function drop_topcart_item(store_id, spec_id){
    ...
    $.getJSON('index.php?gct=cart&gp=drop&specid=' + spec_id + '&storeid=' + store_id, function(result){
        if(result.done){
            //删除成功
...
$(function(){
	...
	$.include('index.php?gct=scan&type=updown');
	<?php }?>
});
</script>
<style type="text/css">
