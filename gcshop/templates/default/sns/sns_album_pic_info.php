...
<div class="left-content">
  ...
  <div id="pictureFolder" class="album">
    <div class="intro mb20">
      <div class="covers"><span class="thumb size60"><i></i><a href="index.php?gct=sns_album&gp=album_pic_list&id=<?php echo $output['class_info']['ac_id']?>">
      ...
      <div class="ad-nav">
        <div class="ad-thumbs">
          <ul class="ad-thumb-list">
            ...
            <li>
			<a href="<?php echo UPLOAD_SITE_URL.DS.ATTACH_MALBUM.DS.$output['master_id'].DS.$v['ap_cover'];?>" value="<?php echo $v['ap_id'] ?>">
			<span class="thumb"><i></i>
			<?php if(C(OSS_IS_STORAGE) == 0) {?>
			<img nctype="thumb" title="<?php echo $v['ap_name']?>" src="<?php echo UPLOAD_SITE_URL.DS.ATTACH_MALBUM.DS.$output['master_id'].DS.str_ireplace('.', '_240.', $v['ap_cover']);?>" />
			<?php }?>
			<?php if(C(OSS_IS_STORAGE) == 1) {?>
			<img nctype="thumb" title="<?php echo $v['ap_name']?>" src="<?php echo UPLOAD_SITE_URL.DS.ATTACH_MALBUM.DS.$output['master_id'].DS. $v['ap_cover'];?>" />
			<?php }?>
...
<script type="text/javascript">
  $(function() {
	...
	$('.ad-back').live('click',function(){
		...
		$('.ad-thumb-list').load('index.php?gct=sns_album&gp=album_pic_scroll_ajax&id=<?php echo $_GET['id'];?>&class_id=<?php echo $_GET['class_id'];?>&mid=<?php echo $_GET['mid'];?>&curpage='+curpage);
	});
...
function ajax_change_imgmessage(url){
	...
	$.getJSON("<?php echo SHOP_SITE_URL; ?>/index.php?gct=sns_album&gp=ajax_change_imgmessage&mid=<?php echo $output['master_id'];?>", {'url':url}, function(data){
