...
<div class="wrap">
  ...
  <form id="list_form" method="get" action="index.php" target="_self">
    <table class="ncm-search-table" style="height:80px;">
	  ...
	  <input type="hidden" name= "gp" value="member_my_teamorder" />
      ...
      <tr>
        ...
        <td><select name="state_type">
            ...
            <option value="40" <?php echo $_GET['state_type']=='state_success'?'selected':''; ?>>交易完成</option>
            <option value="0" <?php echo $_GET['state_type']=='state_cancel'?'selected':''; ?>>已取消</option>
          </select>
		</td>
	    <th>会员</th>
        <td class="w80"><input type="text" class="text w80" name="member_name" value="<?php echo $_GET['member_name']; ?>"></td>
  ...
  <table class="ncm-default-table">
    <thead>
      <tr class="thead">
		...
		<th class="w80">推广提成</th>
		<th class="w80">收货日期</th>
        <th class="w80">订单状态 </th>
        <th class="w60 align-center"><a class="btns" target="_blank" href="index.php?<?php echo $_SERVER['QUERY_STRING'];?>&gp=export_step1"><span><?php echo $lang['nc_export'];?>导出Excel</span></a></th>
      </tr>
    </thead>
    <tbody>
      ...
      <tr class="hover">
		...
		<?php if($order['area_member_id']>0 && $order['area_rebate']>0){?>
