...
<div class="ncm-oredr-show">
  <div class="ncm-order-info">
    ...
    <div class="ncm-order-condition">
      ...
      <ul>
        <li>1. 您尚未对该订单进行支付，请<a href="index.php?gct=show_live_groupbuy&gp=pay&order_sn=<?php echo $output['order']['order_sn']; ?>" class="ncm-btn-mini ncm-btn-orange"><i></i>支付订单</a>以确保抢购兑换码及时发放。</li>
        <li>2. 如果您不想购买此订单抢购，请选择					
		<a href="javascript:void(0)" class="ncm-btn-mini" nc_type="dialog" dialog_width="480" dialog_title="取消抢购" dialog_id="buyer_order_cancel_order" uri="index.php?gct=member_live&gp=cancel&order_id=<?php echo $output['order']['order_id'];?>"  id="order<?php echo $output['order']['order_id'];?>_action_cancel">取消抢购</a>操作。
		</li>
    ...
    <table class="ncm-default-table order">
      ...
      <tr>
        <th colspan="20"><span class="ml10" title="线下抢购订单编号">线下抢购单号：<?php echo $output['order']['order_sn'];?></span><span>下单时间：<?php echo date("Y-m-d H:i",$output['order']['add_time']);?></span><span><a href="<?php echo urlShop('show_store','index',array('store_id'=>$output['store']['store_id']));?>"  target="_blank" title="<?php echo $output['store']['store_name'];?>"><?php echo $output['store']['store_name'];?></a></span> 
          ...
          <span member_id="<?php echo $output['store']['member_id'];?>" style="display:none;">
