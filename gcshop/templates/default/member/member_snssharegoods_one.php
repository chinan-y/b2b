...
.dialog-share .share-widget .s-app .weixin{background:url("<?php echo SHOP_TEMPLATES_URL;?>/images/member/shareicon/weixin.png") no-repeat 0 0; display: block; width: 24px; height: 24px; cursor: pointer;}
...
<div class="dialog-share">
  <form method="post" action="index.php?gct=member_snsindex&gp=sharegoods" id="sharegoods_form">
    ...
    <div class="share-set">
      <div class="share-widget"><?php echo $lang['sharebind_alsoshareto'];?>
        <ul class="s-app">
		  ...
		  <li title="微信"><i class='weixin'></i></li>
          ...
          <?php }?>
          <?php }?>		  
          <div class="tip"><span class="arrow"></span>点亮图标分享商品到热门社交网络去~<a target="_blank" href="index.php?gct=member_sharemanage"><?php echo $lang['sharebind_alsosharesetting'];?></a></div>
          <?php }?>
        </ul>
      </div>
...
<div class="bottom"><a href="javascript:void(0);" nc_type="finishbtn" data-param='{"apikey":"@apikey"}' class="mini-btn"><?php echo $lang['sharebind_list_finishbind'];?></a><span><?php echo $lang['sharebind_list_unfinishedbind'];?><a target="_blank" href="<?php echo SHOP_SITE_URL;?>/api.php?gct=sharebind&type=@apikey"><?php echo $lang['sharebind_list_againbind'];?></a></span>
</div>
</div>
</textarea>
...
$(function(){
	...
	$("[nc_type='bindbtn']").bind('click',function(){
	    ...
	    if(isbind == '1'){//已经绑定
		    ...
		    window.open('api.php?gct=sharebind&type='+data_str.apikey);
		}
	});
	$("[nc_type='finishbtn']").live('click',function(){
	    ...
	    eval( "data_str = "+data_str);
		//验证是否绑定成功
		var url = 'index.php?gct=member_sharemanage&gp=checkbind';
