...
<div class="wrap">
  ...
  <form method="get" action="index.php" target="_self">
    <table class="ncm-search-table">
      <input type="hidden" name="gct" value="member_order" />
  ...
  <table class="ncm-default-table order">
    ...
    <tbody order_id="" <?php if (!empty($group_info['pay_amount']) && $p == 0) {?> class="pay" <?php }?>>
      ...
      <tr>
        <td colspan="19" class="pay-td"><span class="ml15">在线支付金额：<em>￥<?php echo ncPriceFormat($group_info['pay_amount']);?></em></span> <a class="ncm-btn ncm-btn-orange fr mr15" href="index.php?gct=buy&gp=pay&store_id=<?php echo $order_info['store_id']; ?>&pay_sn=<?php echo $order_pay_sn; ?>"><i class="icon-shield"></i>订单支付</a></td>
      </tr>
      <?php }?>
      <?php $p++;?>
      <tr>
        <th colspan="19"> <span class="ml10">
          ...
          <div class="mobile-order"><i class="icon-mobile-phone"></i>手机下单</div>
          <?php }?>
          </span>
          <!-- order_time -->
          ...
          <a href="javascript:void(0);" class="order-trash" onclick="ajax_get_confirm('您确定要删除吗?删除后该订单可以在回收站找回，或彻底删除', 'index.php?gct=member_order&gp=change_state&state_type=order_delete&order_id=<?php echo $order_info['order_id']; ?>');"><i class="icon-trash"></i>删除</a>
          <?php } ?>

          <!-- 还原订单 -->

          <?php if ($order_info['if_restore']) { ?>
          <a href="javascript:void(0);" class="order-trash" onclick="ajax_get_confirm('您确定要还原吗?', 'index.php?gct=member_order&gp=change_state&state_type=order_restore&order_id=<?php echo $order_info['order_id']; ?>');"><i class="icon-refresh"></i>还原</a>
          <?php } ?>
        </th>
      </tr>
      ...
      <tr>
        ...
        <td class="tl"><dl class="goods-name">
            <dt><a href="<?php echo $goods_info['goods_url'];?>" target="_blank"><?php echo $goods_info['goods_name']; ?></a><br /><?php echo $goods['goods_serial']; ?></dt>
        ...
        <td><?php echo $goods_info['taxes_total'];?></td>
        <td><?php echo $goods_info['goods_num']; ?></td>
        <td><!-- 退款 -->

          <?php if ($goods_info['refund'] == 1){?>
          <p><a href="index.php?gct=member_refund&gp=add_refund&order_id=<?php echo $order_info['order_id']; ?>&goods_id=<?php echo $goods_info['rec_id']; ?>">退款/退货</a></p>
          <?php }?>

          <!-- 投诉 -->

          <?php if ($order_info['if_complain']){ ?>
          <p><a href="index.php?gct=member_complain&gp=complain_new&order_id=<?php echo $order_info['order_id']; ?>&goods_id=<?php echo $goods_info['rec_id']; ?>">交易投诉</a></p>
          <?php } ?>
		  
		  <?php if($order_info['goods_count'] > 1 ){ ?><a href="javascript:void(0)" class="share-goods" nc_type="sharegoods" data-param='{"gid":"<?php echo $goods_info['goods_id'];?>"}'><i class="icon-share"></i><?php echo $lang['member_order_snsshare'];?></a><?php } ?>
		</td>
        ...
        <td class="bdl" rowspan="<?php echo $order_info['goods_count'];?>"><p><?php echo $order_info['state_desc']; ?> <?php echo $order_info['evaluation_status'] ? $lang['member_order_evaluated'] : '';?></p>
          ...
          <p><a href="index.php?gct=member_order&gp=show_order&order_id=<?php echo $order_info['order_id']; ?>" target="_blank"><?php echo $lang['member_order_view_order'];?></a></p>

          <!-- 物流跟踪 -->

          <?php if ($order_info['if_deliver'] && $order_info['refund_state'] ==0){ ?>
          <p><a href='index.php?gct=member_order&gp=search_deliver&order_id=<?php echo $order_info['order_id']; ?>&order_sn=<?php echo $order_info['order_sn']; ?>' target="_blank"><?php echo $lang['member_order_show_deliver']?></a></p>
          <?php } ?></td>
        <td class="bdl bdr" rowspan="<?php echo $order_info['goods_count'];?>"><!-- 永久删除 -->

          <!-- 锁定-->

          <?php if($order_info['lock_state'] ==1) { ?>
			<p><?php echo $lang['order_state_refund_underway'];?></p>
          <?php }?>
		  
		  <?php if($order_info['refund_state'] >0 && $order_info['refund_amount']){ ?>
          ...
          <p><a href="javascript:void(0)" class="ncm-btn ncm-btn-red" nc_type="dialog" dialog_width="480" dialog_title="<?php echo $lang['member_order_cancel_order'];?>" dialog_id="buyer_order_cancel_order" uri="index.php?gct=member_order&gp=change_state&state_type=order_cancel&order_id=<?php echo $order_info['order_id']; ?>"  id="order<?php echo $order_info['order_id']; ?>_action_cancel"><i class="icon-ban-circle"></i> <?php echo $lang['member_order_cancel_order'];?></a></p>
          <?php } ?>

          <!-- 退款取消订单 -->

          <?php if ($order_info['if_refund_cancel']){ ?>
          <p><a href="index.php?gct=member_refund&gp=add_refund_all&order_id=<?php echo $order_info['order_id']; ?>" class="ncm-btn"><i class="icon-legal"></i>订单退款</a></p>
          <?php } ?>

          <!-- 收货 -->
					<?php if($order_info['refund_state'] < 1){ ?>
          <?php if ($order_info['if_receive']) { ?>
          <p><a href="javascript:void(0)" class="ncm-btn" nc_type="dialog" dialog_id="buyer_order_confirm_order" dialog_width="480" dialog_title="<?php echo $lang['member_order_ensure_order'];?>" uri="index.php?gct=member_order&gp=change_state&state_type=order_receive&order_sn=<?php echo $order_info['order_sn']; ?>&order_id=<?php echo $order_info['order_id']; ?>" id="order<?php echo $order_info['order_id']; ?>_action_confirm"><?php echo $lang['member_order_ensure_order'];?></a></p>
          <?php } ?>

          <!-- 评价 -->

          <?php if ($order_info['if_evaluation']) { ?>
          <p><a class="ncm-btn ncm-btn-acidblue" href="index.php?gct=member_evaluate&gp=add&order_id=<?php echo $order_info['order_id']; ?>"><i class="icon-thumbs-up-alt"></i><?php echo $lang['member_order_want_evaluate'];?></a></p>
          <?php } ?>

          <!-- 已经评价 -->

          <?php if ($order_info['evaluation_state'] == 1) { echo $lang['order_state_eval'];} ?>
          <?php }else if($order_info['refund_state'] == 1){?>
          	<!--<p>退款退货中</p>-->
          
          <?php }else if($order_info['refund_state'] == 2){?>
          	<p>已取消</p>
          <?php } ?>	
          <?php if ($order_info['if_drop']) { ?>
          <p><a href="javascript:void(0);" onclick="ajax_get_confirm('您确定要永久删除吗?永久删除后您将无法再查看该订单，也无法进行投诉维权，请谨慎操作！', 'index.php?gct=member_order&gp=change_state&state_type=order_drop&order_id=<?php echo $order_info['order_id']; ?>');" class="ncm-btn ncm-btn-red mt5"><i class="icon-trash"></i>永久删除</a></p>
          <?php } ?></td>
        <!-- E 合并TD -->
        <?php } ?>
      </tr>
