...
<div class="eject_con">
  <div class="adds">
    ...
    <div id="warning"></div>
    <form method="post" action="index.php?gct=member_address&gp=delivery_add" id="address_form" target="_parent">
      ...
      <dl>
        ...
        <dd>
          <input type="text" class="text w200" maxlength="18" name="true_idnum" value="<?php echo $output['address_info']['true_idnum'];?>"/>
          <p class="hint"></p>
        </dd>
      </dl>
...
$(document).ready(function(){
    ...
    $('#address_form').on('change','select',function(){
        ...
        if (area_id != '-请选择-') {
        	$('#zt_address').load("<?php echo SHOP_SITE_URL;?>/index.php?gct=member_address&gp=delivery_list&dlyp_id=<?php echo $output['address_info']['dlyp_id'];?>&area_id="+area_id);
        }
    });

    <?php if (intval($_GET['id'])) { ?>
    $('#zt_address').load("<?php echo SHOP_SITE_URL;?>/index.php?gct=member_address&gp=delivery_list&dlyp_id=<?php echo $output['address_info']['dlyp_id'];?>&area_id=<?php echo $output['address_info']['area_id']?>");
