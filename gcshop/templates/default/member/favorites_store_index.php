    ...
    <thead>
      ...
      <tr>
        <td colspan="20"><input type="checkbox" id="all" class="checkall"/>
          ...
          <a href="javascript:void(0);" class="ncm-btn-mini" uri="index.php?gct=member_favorites&gp=delfavorites&type=store" name="fav_id" confirm="<?php echo $lang['nc_ensure_del'];?>" nc_type="batchbutton"><i class="icon-trash"></i><?php echo $lang['nc_del'];?></a></td>
      </tr>
      <?php }?>
    </thead>
    <?php if(!empty($output['favorites_list']) && is_array($output['favorites_list'])){?>
    <tbody>
      ...
      <tr class="bd-line">
        ...
        <td class="ncm-table-handle"><span><a href="javascript:void(0);" class="btn-acidblue" nc_type="sharestore" data-param='{"sid":"<?php echo $favorites['store']['store_id'];?>"}'><i class="icon-share"></i>
          ...
          </a></span> <span><a href="javascript:void(0)" class="btn-red" onclick="ajax_get_confirm('<?php echo $lang['nc_ensure_del'];?>', 'index.php?gct=member_favorites&gp=delfavorites&type=store&fav_id=<?php echo $favorites['fav_id'];?>');"><i class="icon-trash"></i>
          <p><?php echo $lang['nc_del'];?></p>
          </a></span></td>
      </tr>
      <tr id="store-goods-<?php echo $favorites['store']['store_id'];?>" class="gcshop-new-goods" style="display:none;">
    ...
    <tfoot>
      <tr>
        <th colspan="20"><input type="checkbox" id="all2" class="checkall"/>
          ...
          <a href="javascript:void(0);" class="ncm-btn-mini" uri="index.php?gct=member_favorites&gp=delfavorites&type=store" name="fav_id" confirm="<?php echo $lang['nc_ensure_del'];?>" nc_type="batchbutton"><i class="icon-trash"></i><?php echo $lang['nc_del'];?></a> </th>
      </tr>
      <tr>
