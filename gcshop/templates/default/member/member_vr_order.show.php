...
<div class="ncm-oredr-show">
  <div class="ncm-order-info">
    ...
    <div class="ncm-order-condition">
      ...
      <ul>
        <li>1. 您尚未对该订单进行支付，请<a href="index.php?gct=buy_virtual&gp=pay&order_id=<?php echo $output['order_info']['order_id']; ?>" class="ncm-btn-mini ncm-btn-orange"><i></i>支付订单</a>以确保及时获取电子兑换码。</li>
      ...
      <tbody>
        <tr>
          <th colspan="20"><span class="ml10" title="虚拟订单号">虚拟订单号：<?php echo $output['order_info']['order_sn'];?></span><span>下单时间：<?php echo date("Y-m-d H:i",$output['order_info']['add_time']);?></span><span><a href="<?php echo urlShop('show_store','index',array('store_id'=>$output['order_info']['store_id']));?>" title="<?php echo $output['order_info']['store_name'];?>"><?php echo $output['store_info']['store_name'];?></a></span>
            ...
            <!-- QQ -->
            <span member_id="<?php echo $output['store_info']['member_id'];?>" style="display:none;">
        ...
        <tr>
          ...
          <td> <?php if($output['order_info']['if_refund']){ ?>
              <a href="index.php?gct=member_vr_refund&gp=add_refund&order_id=<?php echo $output['order_info']['order_id']; ?>">退款</a>
              <?php } ?></td>
          ...
          <?php if ($output['order_info']['if_cancel']){ ?>
            ...
            <p><a class="ncm-btn ncm-btn-acidblue" href="index.php?gct=member_evaluate&gp=add_vr&order_id=<?php echo $output['order_info']['order_id']; ?>"><?php echo $lang['member_order_want_evaluate'];?></a></p>
            <?php } ?>
