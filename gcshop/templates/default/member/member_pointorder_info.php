...
<div class="ncm-oredr-show">
  <div class="ncm-order-info">
    ...
    <?php if ($output['order_info']['point_orderstate'] == $output['pointorderstate_arr']['canceled'][0]){ ?>
      ...
      <ul>
        <li>已取消了该兑换订单，<a href="index.php?gct=pointprod&gp=plist">马上去看看其他兑换礼品</a></li>
      </ul>
    <?php } else { ?>
    ...
    <table class="ncm-default-table order">
      ...
      <tbody>
        ...
        <tr>
          ...
          <p><a href="javascript:void(0)" class="ncm-btn ncm-btn-orange" onclick="ajax_get_confirm('<?php echo $lang['member_pointorder_cancel_confirmtip']; ?>','index.php?gct=member_pointorder&gp=cancel_order&order_id=<?php echo $output['order_info']['point_orderid']; ?>');"><?php echo $lang['member_pointorder_cancel_title']; ?></a></p>
          <?php } ?>
            <?php if ($output['order_info']['point_orderallowreceiving']) { ?>
          <p><a href="javascript:void(0)" class="ncm-btn" onclick="ajax_get_confirm('<?php echo $lang['member_pointorder_confirmreceivingtip']; ?>','index.php?gct=member_pointorder&gp=receiving_order&order_id=<?php echo $output['order_info']['point_orderid']; ?>');"><?php echo $lang['member_pointorder_confirmreceiving']; ?></a></p>
          <?php } ?>
          </td>
          <?php } ?>
        </tr>
...
$(function(){
    $('#show_shipping').on('hover',function(){
    	...
    	$.getJSON('index.php?gct=member_order&gp=get_express&e_code=<?php echo $output['express_info']['e_code']?>&shipping_code=<?php echo $output['order_info']['point_shippingcode']?>&t=<?php echo random(7);?>',function(data){
    		if(data){
