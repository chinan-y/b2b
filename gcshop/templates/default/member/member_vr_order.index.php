...
<div class="wrap">
  ...
  <form method="get" action="index.php" target="_self">
    <table class="ncm-search-table">
      <input type="hidden" name="gct" value="member_vr_order" />
      <tr>
        <td>&nbsp;</td>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td> <?php if($order_info['if_refund']){ ?>
          <a href="index.php?gct=member_vr_refund&gp=add_refund&order_id=<?php echo $order_info['order_id']; ?>">退款</a>
          <?php } ?></td>
        ...
        <td class="bdl"><p><?php echo $order_info['state_desc'];?></p>
          <p><a href="index.php?gct=member_vr_order&gp=show_order&order_id=<?php echo $order_info['order_id'];?>">订单详情</a></p></td>
        <td class="bdl bdr">

		  <?php if($order_info['if_pay']){ ?>
          <p><a class="ncm-btn ncm-btn-orange" href="index.php?gct=buy_virtual&gp=pay&order_id=<?php echo $order_info['order_id']; ?>"><i class="icon-shield"></i>订单支付</a></p>
          <?php } ?>

          <!--取消订单-->
          <?php if ($order_info['if_cancel']) {?>
          <p class="mt5"><a href="javascript:void(0)" class="ncm-btn ncm-btn-red" nc_type="dialog" dialog_width="480" dialog_title="取消订单" dialog_id="buyer_order_cancel_order" uri="index.php?gct=member_vr_order&gp=change_state&state_type=order_cancel&order_id=<?php echo $order_info['order_id'];?>"  id="order<?php echo $order_info['order_id'];?>_action_cancel"><i class="icon-ban-circle"></i>取消订单</a></p>
		  <?php } ?>

          <!-- 评价 -->
          <?php if ($order_info['if_evaluation']) { ?>
          <p><a class="ncm-btn ncm-btn-acidblue" href="index.php?gct=member_evaluate&gp=add_vr&order_id=<?php echo $order_info['order_id']; ?>"><i class="icon-thumbs-up-alt"></i><?php echo $lang['member_order_want_evaluate'];?></a></p>
          <?php } ?>
