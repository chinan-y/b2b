  ...
  <div class="ncs-main-container">
    ...
    <div class="content ncs-all-goods-list mb15">
      <ul>
        ...
        <li>
          <dl>
		    <?php if($output['store_info']['store_id']!=8){ ?>
            <dt><a href="<?php echo urlShop('goods', 'index',array('goods_id'=>$value['goods_id'],'ref'=>$_SESSION['member_id']));?>" class="goods-thumb" target="_blank"><img src="<?php echo thumb($value, 240);?>" alt="<?php echo $value['goods_name'];?>" /></a>
            ...
            <dd class="goods-name"><a href="<?php echo urlShop('goods', 'index', array('goods_id'=>$value['goods_id'],'ref'=>$_SESSION['member_id']));?>" title="<?php echo $value['goods_name'];?>" target="_blank"><?php echo $value['goods_name']?></a></dd>
            <?php } else {?>
			   <dt><a href="index.php?gct=goodsdetail&gp=goodsdetail&goods_id=<?php echo $value['goods_id']; ?>" class="goods-thumb" target="_blank"><img src="<?php echo thumb($value, 240);?>" alt="<?php echo $value['goods_name'];?>" /></a>
              <ul class="goods-thumb-scroll-show">
              <?php if (is_array($value['image'])) {?>
                  <?php $i=0;foreach ($value['image'] as $val) {$i++?>
                  <li<?php if($i==1) {?> class="selected"<?php }?>><a href="javascript:void(0);"><img src="<?php echo thumb($val, 60);?>"/></a></li>
            ...
            <dd class="goods-name"><a href="index.php?gct=goodsdetail&gp=goodsdetail&goods_id=<?php echo $value['goods_id']; ?>" title="<?php echo $value['goods_name'];?>" target="_blank"><?php echo $value['goods_name']?></a></dd>		
			<?php  } ?>
			<dd class="goods-info">
			<?php if($output['store_info']['store_id']!=8){ ?>
			<span class="price"><?php echo $lang['currency'];?>
              <?php echo $value['goods_promotion_price']?>
             </span>
			<?php } ?>
			<span class="goods-sold" style="display:none"><?php echo $lang['nc_sell_out'];?><strong><?php echo $value['goods_salenum'];?></strong> <?php echo $lang['nc_jian'];?></span>
			</dd>
