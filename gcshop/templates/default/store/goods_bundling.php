...
<div class="ncs-goods-info-content" nctype="gbc_content">
    ...
    <div class="F-center">
      ...
      <div class="ncs-bundling-price">
        <dl>
          ...
          <?php }?>
          <dd class="mt5"><a href="javascript:void(0);"  nctype="addblcart_submit" bl_id="<?php echo $val['id']?>" class="ncs-btn ncs-btn-green"><i class="icon-th-large"></i><?php echo $lang['bundling_buy'];?></a></dd>
        </dl>
      </div>
      <?php }?>
      <?php }?>
                ...
                <?php } else {?>
                    ...
                    $.getJSON(url, {'bl_id':bl_id}, function(data){
                    	if(data != null){
                    		if (data.state)
                            {
                                // $('#bold_num1').html(data.num);
                                // $('#bold_mly1').html(price_format(data.amount));
                                // $('.ncs-cart-popup1').fadeIn('fast');
                                // // 头部加载购物车信息
                                // load_cart_information();
								// $("#rtoolbar_cartlist").load('index.php?gct=cart&gp=ajax_load&type=html');
								window.location.href = 'index.php?gct=cart';
                            }
                            else
                            {
  ...
  <div class="ncs-combo-container" <?php if ($current) {?>style="display:none;"<?php }?>>
    <div class="default-goods">
      ...
      <dl>
        ...
        <dd class="goods-price">光彩价<?php echo $lang['nc_colon'].$lang['currency'].$output['goods_info']['goods_promotion_price'];?></dd>
      </dl>
    </div>
      ...
      <ul>
        ...
        <li <?php if ($j == 0) {?>class="combo-goods-first"<?php $j++;}?>>
          ...
          <dl>
            ...
            <dd>
              <input type="checkbox" class="checkbox" nctype="comb" checked="checked" name="<?php echo $combo['goods_id'];?>" data-param="{price:<?php echo $combo['goods_promotion_price'];?>,marketprice:<?php echo $combo['goods_marketprice'];?>}">
    ...
    <dl>
    <dt>推荐组合</dt>
    <dd>光彩总价：<?php echo $lang['currency'];?><em nctype="gbcc_p"><?php echo $output['goods_info']['goods_promotion_price'];?></em></dd>
    <dd>市场总价：<?php echo $lang['currency'];?><em nctype="gbcc_mp"><?php echo $output['goods_info']['goods_marketprice'];?></em></dd>
    <dd class="mt5"><a class="ncs-btn ncs-btn-green" nctype="addblcart_submit_comb" data-param="<?php echo $comb_ids.$output['goods_info']['goods_id'];?>" href="javascript:void(0);"><i class="icon-th-large"></i><?php echo $lang['bundling_buy'];?></a></dd>
    </dl>
  </div>
  ...
  $(function(){
	  ...
	  /*增加一个组合商品默认被全选的价格*/
      $('div[nctype="combo_list"]').find('input[type="checkbox"]').ready(function(){
          var gbcc_p = g_p;
          var gbcc_mp = mg_p;
...
function addcombcart(goods_ids)
{
    ...
    var url = 'index.php?gct=cart&gp=add_comb';
    $.getJSON(url, {'goods_ids':goods_ids}, function(data){
    	if(data != null){
    		if (data.state)
            {
                ...
                // load_cart_information();
				// $("#rtoolbar_cartlist").load('index.php?gct=cart&gp=ajax_load&type=html');
				//加入成功后直接转入购物车页面，取消弹出的加入购物车成功的对话框
				window.location.href = 'index.php?gct=cart';
            }
            else
            {
