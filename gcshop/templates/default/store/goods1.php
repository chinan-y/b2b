...
<div class="wrapper pr" id="content">
	...
	<div class="goods_top_left">goods_top_left</div>
    
	<div class="goods_top_right<?php if ($output['store_info']['is_own_shop']) echo ' ownshop'; ?>">
    ...
    <div class="goods-top_summary">
      ...
      <div class="ncs-meta">
        ...
        <?php }?>
        <!-- E 促销 -->
	<div class="goods_waring">在光彩全球购买的保税商品仅限个人使用，不得进行二次销售。</div>
      </div>
      <div class="ncs-plus">
        <!-- S 物流运费  预售商品不显示物流 -->
        ...
        <dl class="ncs-freight">
          ...
          <dd id="transport_price">
            ...
            <?php }else{?>
            <!-- 如果买家承担运费 -->
            <span id="nc_kd">运费<?php echo $lang['nc_colon'];?><em><?php echo $output['goods']['goods_freight'];?></em><?php echo $lang['goods_index_yuan'];?>起</span>
            <?php }?>
            <?php }?>
          </dd>
      ...
      <!-- S 购买按钮 -->
		...
		<?php if ($output['IsHaveBuy']=="0") {?>
         ...
         <div style="width:300px; margin:auto; text-align:center; font-size:18px; font-weight:bold">
         <h1 style=" color:red">您已参加过此商品的抢购活动！</h1>
         </div>
         <?php } ?>
          <!-- S 加入购物车弹出提示框 -->
    ...
    <div class="ncs-handle">
      ...
      <!-- S 对比 -->
      <a href="javascript:void(0);" class="compare" nc_type="compare_<?php echo $output['goods']['goods_id'];?>" data-param='{"gid":"<?php echo $output['goods']['goods_id'];?>"}'><i></i><?php echo $lang['goods_index_join_contrast'] ?></a><!-- S 举报 -->
      <?php if($output['inform_switch']) { ?>
      <a href="<?php if ($_SESSION['is_login']) {?>index.php?gct=member_inform&gp=inform_submit&goods_id=<?php echo $output['goods']['goods_id'];?><?php } else {?>javascript:login_dialog();<?php }?>" title="<?php echo $lang['goods_index_goods_inform'];?>" class="inform"><i></i><?php echo $lang['goods_index_goods_inform'];?></a>
      <?php } ?>
      <!-- End --> </div>
    <!--S 店铺信息-->
