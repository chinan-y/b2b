  ...
  <div class="ncs-cosult-list">
    <dl class="asker">
	  ...
	  <?php if($v['member_id']== '0') echo $lang['nc_guest']; else if($v['isanonymous'] == 1){?>
        ...
        <?php }else{?>
        <a href="index.php?gct=member_snshome&mid=<?php echo $v['member_id'];?>" target="_blank" data-param="{'id':<?php echo $v['member_id'];?>}" nctype="mcard"><?php echo str_cut($v['cmember_name'],8);?></a>
        <?php }?>        
...
<form method="post" id="message" action="index.php?gct=goods&gp=save_consult&id=<?php echo $_GET['id']; ?>">
  ...
  <div class="ncs-consult-form">
    <div class="asker-info">
      ...
      <label for="captcha"><strong><?php echo $lang['goods_index_checkcode'];?></strong>
        ...
        <img src="index.php?gct=seccode&gp=makecode&nchash=<?php echo getNchash();?>" name="codeimage" border="0" id="codeimage" onclick="this.src='index.php?gct=seccode&gp=makecode&nchash=<?php echo $output['nchash'];?>&t=' + Math.random()"/><span><?php echo $lang['goods_index_change_checkcode'];?></span></label>
      <?php } ?>
    </div>
...
$(function(){
	$('a[nc_type="consult_submit"]').click(function(){
	    if($("#message").valid()){
			$.getJSON('index.php?gct=goods&gp=save_consultajax',{
				'form_submit':'ok','formhash':$('input[name="formhash"]').val(),
					...
					if(data.done == 'true'){
						$("#cosulting_demo").load('index.php?gct=goods&gp=cosulting&goods_id=<?php echo $_GET['goods_id']; ?>');
					}else{
