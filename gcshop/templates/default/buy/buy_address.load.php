...
<ul>
  ...
  <li class="receive_add address_item <?php echo $k == 0 ? 'ncc-selected-item' : null; ?>">
    <input  address="<?php echo intval($val['dlyp_id']) ? '[自提服务站] ' : '';?><?php echo $val['area_info'].'&nbsp;'.$val['address']; ?>" true_name="<?php echo $val['true_name'];?>" order_name="<?php echo $val['order_name'];?>" true_idnum="<?php echo $val['true_idnum'];?>" &nbsp id="addr_<?php echo $val['address_id']; ?>" nc_type="addr" type="radio" class="radio" city_id="<?php echo $val['city_id']?>" area_id=<?php echo $val['area_id'];?> name="addr" value="<?php echo $val['address_id']; ?>" phone="<?php echo $val['mob_phone'] ? $val['mob_phone'] : $val['tel_phone'];?>" <?php echo $val['is_default'] == '1' ? 'checked' : null; ?> />
    <label for="addr_<?php echo $val['address_id']; ?>"><span class="true-name"><?php echo $val['true_name'];?></span><span class="true-idnum"><?php echo $val['true_idnum'];?></span>&nbsp;<span class="address"><?php echo intval($val['dlyp_id']) ? '[自提服务站] ' : '';?><?php echo $val['area_info']; ?>&nbsp;<?php echo $val['address']; ?></span><span class="phone"><i class="icon-mobile-phone"></i><?php echo $val['mob_phone'] ? $val['mob_phone'] : $val['tel_phone'];?></span></label>
    <a href="javascript:void(0);" onclick="delAddr(<?php echo $val['address_id']?>);" class="del">[ 删除 ]</a>
	<a href="<?php echo urlShop('member_address','address');?>" target="_blank" class="edit">[ 编辑 ]</a>
...
function delAddr(id){
    $('#addr_list').load(SITEURL+'/index.php?gct=buy&gp=load_addr&id='+id);
}
$(function(){
    function addAddr() {
        $('#add_addr_box').load(SITEURL+'/index.php?gct=buy&gp=add_addr');
    }
    ...
    $('#hide_addr_list').on('click',function(){
		is_click_judge();
		var address_id = $('input[nc_type="addr"]:checked').val();
        if ($('input[nc_type="addr"]:checked').val() == '0'){
			...
			var true_idnum = $('input[name="addr"]:checked').attr('true_idnum');
            ...
            hideAddrList(addr_id,true_name,true_idnum,address,phone);
			
			var is_default = 1;
			$.ajax({
				url:"index.php?gct=member_address&gp=default_address",
				data:{is_default:is_default,address_id:address_id},
				type:"post",
			});
