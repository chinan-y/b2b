...
$(function(){
	$('a[nctype="search_in_store"]').click(function(){
		...
		$('<input type="hidden" value="<?php echo $output['store_info']['store_id'];?>" name="store_id" /> <input type="hidden" name="gp" value="goods_all" />').appendTo("#formSearch");
		$('#formSearch').submit();
	});
	...
	$('.favorites').mouseover(function(){
		var $this = $(this);
		if(storeTrends){
			$.getJSON('index.php?gct=show_store&gp=ajax_store_trend_count&store_id=<?php echo $output['store_info']['store_id'];?>', function(data){
	...
	$('a[nctype="share_store"]').click(function(){
		...
		login_dialog();
		<?php } else {?>
		ajax_form('sharestore', '分享店铺', 'index.php?gct=member_snsindex&gp=sharestore_one&inajax=1&sid=<?php echo $output['store_info']['store_id'];?>');
		<?php }?>
	});
});
</script>
