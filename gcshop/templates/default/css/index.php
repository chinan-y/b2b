									  ...
									  <?php foreach ($output['show_goods_class'] as $key => $val) {  ?>
										  ...
										  <?php foreach ($val['class2'] as $k => $v) { ?>
											  ...
											  <?php foreach ($v['brands'] as $k3 => $v3) {
												...
												<div class="pai">
												<a href="<?php echo urlShop('brand','list',array('brand'=> $v3['brand_id'])); ?>"><img src="https://www.qqbsmall.com/data/upload/gcshop/brand/<?php echo $v3['brand_pic'];?>"/></a>
												</div>
  ...
  $('[nc_type="area_name"]').html('<?php echo $output['province_array'][intval($_GET['area_id'])]; ?>');
	...
	$('#guesslike_div').load('<?php echo urlShop('search', 'get_guesslike', array()); ?>', function(){
        $(this).show();
                    ...
                    <div id=ads>
						<a href="https://www.qqbsmall.com/gcshop/integral.html"><img class="us" src="/gcshop/templates/default/images/7.png"/></a>
...
<script type="text/javascript">
	$("#ads").hide();
		$(window).scroll(function() {
            ...
            if ($(document).height() - $(this).scrollTop() - $(this).height() < 10300) {
				$("#ads").show();
            }
            if ($(document).height() - $(this).scrollTop() - $(this).height() > 9500) {
				$("#ads").hide();
            }
