...
<div class="tabmenu">
  ...
  <a class="ncsc-btn ncsc-btn-green" href="index.php?gct=store_transport&gp=add&type=<?php echo $_GET['type'];?>"><?php echo $lang['transport_tpl_add'];?> </a> </div>
<!-----------------list begin------------------------>
  ...
  <tbody>
    ...
    <tr>
      <th colspan="20"><?php if ($_GET['type'] == "select"){?>
        <a class="ml5 ncsc-btn-mini ncsc-btn-orange" data-param="{name:'<?php echo $v['title'];?>',id:'<?php echo $v['id'];?>',price:'<?php echo intval($output['extend'][$v['id']]['price']);?>',address_name:'<?php echo $output['regions'][$v['region_id']]['address_name'];?>'}" href="javascript:void(0)"><i class="icon-truck"></i><?php echo $lang['transport_applay'];?></span></a>
        <?php }?><h3><?php echo $v['title'];?><?php if($v['type'] == 0){echo '(按重量计费)';}else{echo '(按数量计费)';}?><?php if($v['region_id']){echo '('.$output['regions'][$v['region_id']]['address_name'].')';}?></h3>
...
$(function(){	
	$('a[class="J_Delete ncsc-btn-mini"]').click(function(){
		...
		get_confirm('<?php echo $lang['transport_del_confirm'];?>','<?php echo SHOP_SITE_URL;?>/index.php?gct=store_transport&gp=delete&type=<?php echo $_GET['type'];?>&id='+id);
//		$(this).attr('href','<?php echo SHOP_SITE_URL;?>/index.php?gct=transport&gp=delete&type=<?php echo $_GET['type'];?>&id='+id);
//		return true;
	});
	...
	$('a[class="J_Modify ncsc-btn-mini"]').click(function(){
		var id = $(this).attr('data-id');
		if(typeof(id) == 'undefined') return false;
		$(this).attr('href','<?php echo SHOP_SITE_URL;?>/index.php?gct=store_transport&gp=edit&type=<?php echo $_GET['type'];?>&id='+id);
		return true;
	});
	...
	$('a[class="J_Clone ncsc-btn-mini"]').click(function(){
		var id = $(this).attr('data-id');
		if(typeof(id) == 'undefined') return false;
		$(this).attr('href','<?php echo SHOP_SITE_URL;?>/index.php?gct=store_transport&gp=clone&type=<?php echo $_GET['type'];?>&id='+id);
		return true;
	});
	$('a[class="ml5 ncsc-btn-mini ncsc-btn-orange"]').click(function(){
		...
		$("#transport_address_name", opener.document).html(data_str.address_name);
