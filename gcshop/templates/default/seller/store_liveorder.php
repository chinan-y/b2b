...
<form method="get">
  <table class="search-form">
    <input type="hidden" id='gct' name='gct' value='store_liveorder' />
    <input type="hidden" id='gp' name='gp' value='store_liveorder' />
    <tr>
      <td>&nbsp;</td>
      <td></td>
...
<table class="ncsc-table-style order">
  ...
  <tbody>
    ...
    <th colspan="20"><span class="fl ml10">线下抢单号：<span class="goods-num"><em><?php echo $val['order_sn'];?></em></span></span><span class="fl ml20">下单时间：<em class="goods-time"><?php echo date("Y-m-d H:i",$val['add_time']);?></em></span><span class="fr mr5"> <a href="index.php?gct=store_liveorder&gp=store_livedetail&order_id=<?php echo $val['order_id'];?>"  class="ncsc-btn-mini"><i class="icon-compass"></i>查看订单</a> </span></th>
  <tr>
    <td class="bdl"></td>
    ...
    <td class="bdl bdr"><p>
      ...
      <p>
        <?php if($val['state']==1){?>
        <a href="javascript:void(0)" class="ncsc-btn-mini ncsc-btn-red mt5" nc_type="dialog" dialog_width="480" dialog_title="取消订单" dialog_id="buyer_order_cancel_order" uri="index.php?gct=store_liveorder&gp=cancel&order_id=<?php echo $val['order_id'];?>"  id="order<?php echo $val['order_id'];?>_action_cancel"><i class="icon-ban-circle"></i>取消订单</a>
        <?php }?>
      </p></td>
  </tr>
  <?php }?>
