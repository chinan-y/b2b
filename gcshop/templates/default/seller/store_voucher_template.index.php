...
<?php } ?>
  ...
  <form method="get">
    <table class="search-form">
      <input type="hidden" id='gct' name='gct' value='store_voucher' />
      <input type="hidden" id='gp' name='gp' value='templatelist' />
      <tr>
        <td>&nbsp;</td>
      ...
      <tr class="bd-line">
        ...
        <td class="nscs-table-handle">
        	<?php if($val['voucher_t_state']==$output['templatestate_arr']['usable'][0] && !$val['voucher_t_giveout']) {//代金券模板有效并且没有领取时可以编辑?>
        		<span>
        		  <a class="btn-blue" href="index.php?gct=store_voucher&gp=templateedit&tid=<?php echo $val['voucher_t_id'];?>">
        		      <i class="icon-edit"></i><p>编辑</p>
        		  </a>
        	   </span>
        	<?php }else {//代金券模板失效或者有领取时可以查看?>
        		<span>
        		  <a class="btn-blue" href="index.php?gct=store_voucher&gp=templateinfo&tid=<?php echo $val['voucher_t_id'];?>"><i class="icon-th-list"></i>
        		      <p>详细</p>
        		  </a>
        	   </span>
        	<?php }?>
        	...
        	<span>
        	   <a class="btn-red" href="javascript:void(0)" onclick="ajax_get_confirm('<?php echo $lang['nc_ensure_del'];?>','index.php?gct=store_voucher&gp=templatedel&tid=<?php echo $val['voucher_t_id'];?>');">
