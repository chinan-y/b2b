...
<div class="eject_con">
  <div class="adds">
    <div id="warning"></div>
    <form method="post" action="index.php?gct=store_deliver&gp=buyer_address&order_id=<?php echo $_GET['order_id'];?>" id="address_form" target="_parent">
      ...
      <dl>
        <dt class="required">身份证号码：</dt>
        <dd>
          <input type="text" class="text" name="new_reciver_idnum" id="new_reciver_idnum" value="<?php echo $output['address_info']['reciver_idnum'];?>"/>
        </dd>
      </dl>
      <dl>
...
$(document).ready(function(){
    $('#address_form').validate({
        ...
        messages : {
			...
			new_reciver_idnum: {
                required : '<i class="icon-exclamation-sign"></i>身份证号码不能为空'
            },
            new_area: {
		...
		if ($('#address_form').valid()) {
            ...
            .done(function(data) {
                if(data == 'true') {
					...
					$('#reciver_idnum').val(reciver_idnum);
                    var content = reciver_name + '&nbsp' +reciver_idnum + '&nbsp' + reciver_mob_phone + ',' + reciver_tel_phone + '&nbsp;' + reciver_area + '&nbsp;' + reciver_street;
	...
	$('#new_reciver_idnum').val($('#reciver_idnum').val());	
});
</script>
