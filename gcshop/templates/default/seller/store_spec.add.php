...
<form id="spec_form" method="post" action="<?php echo SHOP_SITE_URL;?>/index.php?gct=store_spec&gp=save_spec">
...
$(function(){
    ...
    $('a[nctype="submit-btn"]').click(function(){
        ...
        if (_submit) {
            ajaxpost('spec_form', '<?php echo SHOP_SITE_URL;?>/index.php?gct=store_spec&gp=save_spec', '', 'onerror');
        }
    });
    ...
    $('a[nctype="spec_del"]').click(function(){
        ...
        $.getJSON('index.php?gct=store_spec&gp=ajax_delspec&id='+data_str.id, function(data){
            if(data){
                $this.parents('tr:first').remove();
            }
        });
