...
<div class="ncsc-oredr-show">
  <div class="ncsc-order-info">
    <div class="ncsc-order-details">
      ...
      <div class="content">
		<dl>
          <dt><?php echo "买家信息".$lang['nc_colon'];?></dt>
          <dd><span><?php echo $output['order_info']['buyer_name'];?></span></dd>
        </dl>
        <dl>
    ...
    <div class="ncsc-order-condition">
      ...
      <ul>
        ...
        <li>2. 可以在<a href="index.php?gct=store_evaluate&gp=list" class="ncsc-btn-mini">评价管理</a>查看详细内容。</li>
      </ul>
    </div>
    <?php } else { ?>
    <div class="ncsc-order-condition">
        ...
        <tr class="bd-line">
          ...
          <td class="tl"><dl class="goods-name">
              ...
              <dd>
                ...
                退款单号：<a target="_blank" href="index.php?gct=store_refund&gp=view&refund_id=<?php echo $output['refund_all']['refund_id'];?>"><?php echo $output['refund_all']['refund_sn'];?></a>
                <?php }else if($goods['extend_refund']['refund_type'] == 1) {?>
                退款单号：<a target="_blank" href="index.php?gct=store_refund&gp=view&refund_id=<?php echo $goods['extend_refund']['refund_id'];?>"><?php echo $goods['extend_refund']['refund_sn'];?></a></dd>
              <?php }else if($goods['extend_refund']['refund_type'] == 2) {?>
              退货单号：<a target="_blank" href="index.php?gct=store_return&gp=view&return_id=<?php echo $goods['extend_refund']['refund_id'];?>"><?php echo $goods['extend_refund']['refund_sn'];?></a>
              </dd>
              <?php } ?>
            </dl></td>
          ...
          <td class="bdl bdr" rowspan="<?php echo $output['order_info']['goods_count'];?>"><?php echo $output['order_info']['state_desc']; ?>
            ...
            <p><a href="javascript:void(0)" class="ncsc-btn" uri="index.php?gct=store_order&gp=change_state&state_type=modify_price&order_sn=<?php echo $output['order_info']['order_sn']; ?>&order_id=<?php echo $output['order_info']['order_id']; ?>" dialog_width="480" dialog_title="<?php echo $lang['store_order_modify_price'];?>" nc_type="dialog"  dialog_id="seller_order_adjust_fee" id="order<?php echo $output['order_info']['order_id']; ?>_action_adjust_fee" />修改运费</a></p>
            <?php }?>

            <!-- 取消订单 -->
            <?php if ($output['order_info']['if_cancel']) { ?>
            <p><a href="javascript:void(0)" style="color:#F30; text-decoration:underline;" nc_type="dialog" uri="index.php?gct=store_order&gp=change_state&state_type=order_cancel&order_sn=<?php echo $output['order_info']['order_sn']; ?>&order_id=<?php echo $output['order_info']['order_id']; ?>" dialog_title="<?php echo $lang['store_order_cancel_order'];?>" dialog_id="seller_order_cancel_order" dialog_width="400" id="order<?php echo $output['order_info']['order_id']; ?>_action_cancel" /><?php echo $lang['store_order_cancel_order'];?></a></p>
            <?php } ?>

            <!-- 发货 -->
            <?php if ($output['order_info']['if_send'] && $output['order_info']['refund_state'] == 0) { ?>
            <p><a class="ncsc-btn" href="index.php?gct=store_deliver&gp=send&order_id=<?php echo $output['order_info']['order_id']; ?>"/><i class="icon-truck"></i><?php echo $lang['store_order_send'];?></a></p>
            <?php } ?></td>
          <?php } ?>
          <!-- E 合并TD -->
        </tr>
...
$(function(){
    $('#show_shipping').on('hover',function(){
    	...
    	$.getJSON('index.php?gct=store_deliver&gp=get_express&e_code=<?php echo $output['order_info']['express_info']['e_code']?>&shipping_code=<?php echo $output['order_info']['shipping_code']?>&t=<?php echo random(7);?>',function(data){
    		if(data){
    			data = var_send+data;
