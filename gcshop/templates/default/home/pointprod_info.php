 ...
 function copy_url()
 {
	 ...
	 if(window.clipboardData)
	    ...
	    else if (window.netscape)
	    {
	        ...
	        if (!trans){return;}
	        // specificeer wat voor soort data we gp willen halen; text in dit geval
	        trans.addDataFlavor('text/unicode');
	        // om de data uit de transferable te halen hebben we 2 nieuwe objecten
	        // nodig om het in gp te slaan
	        var str = new Object();
	        var len = new Object();
...
function add_to_cart()
{
	...
	if(checkresult == false){
		...
		$.getJSON('<?php echo SHOP_SITE_URL; ?>/index.php?gct=pointcart&gp=add&pgid=<?php echo $output['prodinfo']['pgoods_id']; ?>&quantity='+quantity, function(result){
	        if(result.done){
	        	window.location.href = '<?php echo SHOP_SITE_URL; ?>/index.php?gct=pointcart';
	        } else {
		        if(result.url){
