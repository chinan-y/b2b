    ...
    <ul class="ncp-voucher-list">
      ...
      <li>
        <div class="ncp-voucher">
          <div class="cut"></div>
          <div class="info">
		  <?php if($v['voucher_t_store_id']==0){echo "<b>".$v['voucher_t_desc']."</b>";} else {?>
		  <a href="<?php echo urlShop('show_store', 'index', array('store_id'=>$v['voucher_t_store_id']));?>" class="store">
		  <?php echo $v['voucher_t_storename'];}?>
		  </a>
          ...
          <div class="point">
            <p class="required">需<em><?php echo $v['voucher_t_points'];?></em>积分
			</p>
            <p><em><?php echo $v['voucher_t_giveout'];?></em>人兑换</p>
          </div>
          <div class="button"><a target="_blank" href="###" nc_type="exchangebtn" data-param='{"vid":"<?php echo $v['voucher_t_id'];?>"}' class="ncp-btn ncp-btn-qqbsmall">立即兑换</a></div>
        </div>
      </li>
      <?php }?>
    </ul>
...
function searchvoucher(){
	var url = 'index.php?gct=pointvoucher&gp=index';
	var sc_id = $("#sc_id").val();
	if(sc_id){
		url += ('&sc_id='+sc_id);
	}
