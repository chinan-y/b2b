...
class sns_albumControl extends BaseSNSControl {
	...
	public function album_add_saveOp(){
		if (chksubmit()){
			...
			if($class_count >= 10){
				showDialog(Language::get('album_class_save_max_10'),'index.php?gct=sns_album','error');
			}
			$insert = array();
			...
			if($return){
				showDialog(Language::get('album_class_save_succeed'),'index.php?gct=sns_album','succ',empty($_GET['inajax'])?'':'CUR_DIALOG.close();');
			}
		}
	...
	public function album_edit_saveOp(){
		...
		if($re){
			showDialog(Language::get('album_class_edit_succeed'),'index.php?gct=sns_album','succ',empty($_GET['inajax'])?'':'CUR_DIALOG.close();');
		}else{
	...
	public function album_delOp(){
		...
		if($return){
			showDialog(Language::get('album_class_file_del_succeed'),'index.php?gct=sns_album','succ');
		}else{
			showDialog(Language::get('album_class_file_del_lose'));
		}
	}
	...
	public function change_album_coverOp(){
		...
		if(C(OSS_IS_STORAGE) == 1){
			$update['ac_cover']	= $pic_info['ap_cover'].'?x-oss-process=image/resize,m_pad,w_240';	
		}
	...
	public function album_pic_delOp(){
		...
		foreach($id as $v){
			...
			if(C(OSS_IS_STORAGE) == 1){
				...
				$objectname = DIR_UPLOAD.DS.ATTACH_MALBUM.DS.$_SESSION['member_id'].DS.$ap_info['ap_cover'];
				$result = $oss->deleteObject($bucketname,$objectname);
			}
	...
	private function profile_menu($menu_type,$menu_key=''){
		...
		switch ($menu_type) {
			case 'album':
				$menu_array	= array(
					1=>array('menu_key'=>'album','menu_name'=>$album_menuname,'menu_url'=>'index.php?gct=sns_album&mid='.$this->master_id),
				);
				break;
			case 'album_pic':
				$menu_array	= array(
					1=>array('menu_key'=>'album','menu_name'=>$album_menuname,'menu_url'=>'index.php?gct=sns_album&mid='.$this->master_id),
					2=>array('menu_key'=>'pic_list','menu_name'=>Language::get('nc_sns_path_album_pic_list'),'menu_url'=>'index.php?gct=sns_album&gp=album_pic_list&id='.intval($_GET['id']).'&mid='.$this->master_id),
				);
				break;
			case 'album_pic_info':
				$menu_array	= array(
					1=>array('menu_key'=>'album','menu_name'=>$album_menuname,'menu_url'=>'index.php?gct=sns_album&mid='.$this->master_id),
					2=>array('menu_key'=>'pic_info','menu_name'=>Language::get('nc_sns_path_album_pic_info'),'menu_url'=>'index.php?gct=sns_album&gp=album_pic_info&id='.intval($_GET['id']).'&class_id='.intval($_GET['class_id']).'$mid='.$this->master_id),
				);
				break;
		}
    ...
    public function swfuploadOp() {
		...
		if(C(OSS_IS_STORAGE) == 1){
			...
			$imgname	= 'm' . $member_id . '_' . date('YmdHis') . rand(0,9999);
			$imgfullname		= $imgname . '.' . $extend;
			$objectname 		= DIR_UPLOAD.DS.ATTACH_MALBUM.DS. $member_id . DS .$imgfullname;
			...
			list($width, $height, $type, $attr) = getimagesize(UPLOAD_SITE_URL.DS.ATTACH_MALBUM.DS.$member_id.DS.$img_path);
		}
