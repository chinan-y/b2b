...
class store_specControl extends BaseSellerControl {
    ...
    public function save_specOp(){
        ...
        if (is_array($_POST['sv']['old'])) {
            foreach ($_POST['sv']['old'] as $key=> $value) {
                ...
                $re = $model_spec->editSpecValue($update, $where);
            }
			if($re){
				// 记录操作日志
				$this->recordSellerLog('更新商品规格值，分类gc_id:'.$gc_id);
			}
        }
        ...
        if (is_array($_POST['sv']['new'])) {
            ...
            $re = $model_spec->addSpecValueALL($insert_array);
			
			if($re){
				// 记录操作日志
				$this->recordSellerLog('添加商品规格值，分类gc_id:'.$gc_id);
			}
        }
    ...
    public function ajax_delspecOp(){
        ...
        if ($rs) {
			$this->recordSellerLog('删除商品规格值，规格值id:'.$sp_value_id);
            echo 'true'; exit();
        } else {
            echo 'false'; exit();
        }
    ...
    private function _profile_menu($menu_type, $menu_key) {
        ...
        switch ($menu_type) {
            case 'spec':
                $menu_array = array(
                    array('menu_key' => 'spec', 'menu_name' => L('nc_member_path_store_spec'), 'menu_url' => 'index.php?gct=store_spec')
                );
            break;
        }
