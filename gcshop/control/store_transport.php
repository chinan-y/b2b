...
class store_transportControl extends BaseSellerControl {
	...
	public function __construct(){
		...
		Tpl::output('transport_type',
			array(
				array('id'=>0,'name'=>'按重量计费'),
				array('id'=>1,'name'=>'按数量计费'),
			)
		);
	}
	...
	public function deleteOp(){
		...
		if($model_transport->delTansport(array('id'=>$id))){
			$this->recordSellerLog('删除运费模板，模板id:'.$id);
	...
	public function saveOp(){
		...
		$trans_info['type'] 		= $_POST['transport_type'];
		$trans_info['region_id'] 		= $_POST['region_id'];
		...
		if ($result){
			if($re){
				$this->recordSellerLog('新增运费模板，模板id:'.$transport_id.'，模板名称:'.$_POST['title']);
			}else{
				$this->recordSellerLog('编辑运费模板，模板id:'.$transport_id.'，模板名称:'.$_POST['title']);
			}
			
			header('location: index.php?gct=store_transport&type='.$_POST['type']);exit;
		}else{
			showMessage(Language::get('transport_op_fail'),$_SERVER['HTTP_REFERER'],'html','error');
		}
	}
	...
	private function profile_menu($menu_type,$menu_key='') {
		...
		switch ($menu_type) {
            ...
            case 'offpay_area':
				$menu_array = array(
				1=>array('menu_key'=>'transport',	'menu_name'=>Language::get('nc_member_path_postage'),		'menu_url'=>'index.php?gct=store_transport'),
				);
                //if (checkPlatformStore()) {
                    $menu_array[] = array(
                        'menu_key' => 'offpay_area',
                        'menu_name' => '配送地区',
                        'menu_url' => 'index.php?gct=store_transport&gp=offpay_area'
                    );
                //}
                break;
		}
