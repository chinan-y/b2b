...
class show_live_groupbuyControl extends BaseHomeControl {
	...
	public function select_cityOp(){
		...
		redirect("index.php?gct=show_live_groupbuy");
	}
	...
	public function livegroupbuyorderOp(){
		if($_SESSION['is_login'] !== '1'){//检测是否登录
			showMessage('请先登录','index.php?gct=login&gp=index','','error');
		}
		...
		if(chksubmit()){
			...
			if(isset($_POST['pd_pay']) && $_POST['pd_pay']==1){
				...
				if($params['price']>$member['available_predeposit']){//使用预存款支付部分
					...
					if(!$res){
						showMessage('订单支付失败','index.php?gct=member_live','html','error');
					}
			...
			if($res){
				//转向到商城支付页面
				$pay_url = 'index.php?gct=show_live_groupbuy&gp=pay&order_sn='.$order_sn;
				redirect($pay_url);
			}else{
				showMessage('抢购失败','index.php?gct=member_live&gp=index','html','error');
			}
		}else{
			showMessage('抢购失败','index.php?gct=member_live&gp=index','html','error');
		}
	...
	public function payOp(){
        ...
        if (!preg_match('/^\d{18}$/',$order_sn)){
            showMessage('订单不存在','index.php?gct=member_live&gp=index','html','error');
        }
		...
		if(!empty($order_info['payment_code'])&&$order_info['payment_code']=='predeposit'){
			...
			if($price>0){//使用预存款支付
				...
				if($res1 && $res2 && $res3){
					...
					//转向到商城支付页面
					$pay_url = 'index.php?gct=show_live_groupbuy&gp=pay_ok&order_sn='.$order_sn;
					redirect($pay_url);
				}else{
					showMessage('抢购失败','index.php?gct=member_live&gp=index','html','error');
				}
		...
		if (empty($payment_list)) {
			showMessage('暂未找到合适的支付方式','index.php?gct=member_live','html','error');
		}
	...
	public function paymentOp(){
		...
		if(empty($order_info)){
			showMessage('该订单不存在','index.php?gct=member_live','html','error');
		}
