...
class member_snsfriendControl extends BaseMemberControl {
	 ...
	 * 感恩人列表
	 */
	public function inviterOp() {
		if(!empty($_POST['inviter_mobile']))
			{
			$invtermobile=$_POST['inviter_mobile'];
			$inviter_model = Model();
			$inviter_list = $inviter_model->table('member')->where(array('member_mobile'=>$_POST['inviter_mobile']))->find();
			$inviter_id=intval($inviter_list['member_id']);
				
				if(!empty($inviter_id))
				{
				$inviter_update=$inviter_model->table('member')->where(array('member_id'=>$_SESSION['member_id']))->update(array('inviter_id'=>$inviter_id));
				if($inviter_update){showDialog('感恩人添加成功!','index.php?gct=member_snsfriend&gp=inviter','succ');}
				}
				if(empty($inviter_id))
				{
				showDialog('很抱歉，没有查询到该手机号码!','index.php?gct=member_snsfriend&gp=inviter','error');
				}
				
			}
			$inviter_model = Model();
			//$inviter_list = $inviter_model->where(array('inviter_id'=>"{$_SESSION['member_id']}")->select();
			$inviter_list = $inviter_model->table('member')->find($_SESSION['member_id']);
			$inviterid=intval($inviter_list['inviter_id']);
			$inviter_list = $inviter_model->table('member')->find($inviterid);

			if(!empty($_POST['delrelation']))
			{	
				$del_model = Model();
				$del_relation=$del_model->table('member')->where(array('member_id'=>$_SESSION['member_id']))->update(array('inviter_id'=>0));
				if($del_relation){showDialog('感恩关系已解除!','index.php?gct=member_snsfriend&gp=inviter','succ');}
							
			}

			Tpl::output('inviter_list',$inviter_list);
			self::profile_menu('inviter');
			Tpl::showpage('member_snsfriend_inviter');
	}
	/**
	 * 用户中心右边，小导航
	 *
	...
	private function profile_menu($menu_key) {
		$menu_array = array(
			4=>array('menu_key'=>'inviter',	'menu_name'=>'感恩人',	'menu_url'=>'index.php?gct=member_snsfriend&gp=inviter'),
			1=>array('menu_key'=>'find',	'menu_name'=>Language::get('snsfriend_findmember'),	'menu_url'=>'index.php?gct=member_snsfriend&gp=find'),
			2=>array('menu_key'=>'follow',	'menu_name'=>Language::get('snsfriend_follow'),	'menu_url'=>'index.php?gct=member_snsfriend&gp=follow'),
			3=>array('menu_key'=>'fan',		'menu_name'=>Language::get('snsfriend_fans'),	'menu_url'=>'index.php?gct=member_snsfriend&gp=fan')
		);
