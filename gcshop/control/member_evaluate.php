...
class member_evaluateControl extends BaseMemberControl{
    ...
    public function addOp(){
        ...
        if (!$order_id){
            showMessage(Language::get('wrong_argument'),'index.php?gct=member_order','html','error');
        }
        ...
        if (empty($order_info) || !$order_info['evaluate_able']){
            showMessage(Language::get('member_evaluation_order_notexists'),'index.php?gct=member_order','html','error');
        }

        //查询店铺信息
        $store_info = $model_store->getStoreInfoByID($order_info['store_id']);
        if(empty($store_info)){
            showMessage(Language::get('member_evaluation_store_notexists'),'index.php?gct=member_order','html','error');
        }
        ...
        if (!$_POST){
            for ($i = 0, $j = count($order_goods); $i < $j; $i++) {
				$common_id = Model('goods')->getGoodsInfo(array('goods_id'=>$order_goods[$i]['goods_id']),'goods_commonid');
                $order_goods[$i]['goods_image_url'] = cthumb($order_goods[$i]['goods_image'], 60, $store_info['store_id'], $common_id['goods_commonid']);
            }
...
//             //添加店铺评价
            ...
            showDialog(Language::get('member_evaluation_evaluat_success'),'index.php?gct=member_order', 'succ');
        }
    }
    ...
    public function add_vrOp(){
        $order_id = intval($_GET['order_id']);
        if (!$order_id){
            showMessage(Language::get('wrong_argument'),'index.php?gct=member_vr_order','html','error');
        }
        ...
        if (!$order_info['evaluate_able']){
            showMessage(Language::get('member_evaluation_order_notexists'),'index.php?gct=member_vr_order','html','error');
        }

        //查询店铺信息
        $store_info = $model_store->getStoreInfoByID($order_info['store_id']);
        if(empty($store_info)){
            showMessage(Language::get('member_evaluation_store_notexists'),'index.php?gct=member_vr_order','html','error');
        }
...
//             }
            ...
            showDialog(Language::get('member_evaluation_evaluat_success'),'index.php?gct=member_vr_order', 'succ');
        }
    }
    ...
    public function listOp(){
		...
		foreach($goodsevallist as $k => $val){
			$common_id = Model('goods')->getGoodsInfo(array('goods_id'=>$val['geval_goodsid']), 'goods_commonid');
			$goodsevallist[$k]['goods_commonid'] = $common_id['goods_commonid'];
		}
