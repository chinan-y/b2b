	...
	public function indexOp() {
		...
		foreach ($order_list as $key => $order_info) {
		    ...
		    foreach ($order_info['extend_order_goods'] as $value) {
		        $value['image_60_url'] = cthumb($value['goods_image'], 60, $value['store_id'], $value['goods_commonid']);
		        $value['image_240_url'] = cthumb($value['goods_image'], 240, $value['store_id'], $value['goods_commonid']);
	...
	public function sendOp(){
		...
		if (chksubmit()){
			...
			if (!$result['state']) {
				...
				$this->recordSellerLog('设置发货成功，订单编号:'.$order_info['order_sn']);
			    showDialog($result['msg'],$_POST['ref_url'],'succ');
			}
		}
		foreach($order_info['extend_order_goods'] as $key =>$val){
			$commonid= Model('goods')->getGoodsIn(array('goods_id'=>$val['goods_id'], 'goods_commonid'));
...
// 		}
    ...
    public function buyer_address_saveOp() {
		...
		$data['reciver_idnum'] = $_POST['reciver_idnum'];
	...
	public function search_deliverOp(){
		...
		// if (!is_numeric($order_sn)) showMessage(Language::get('wrong_argument'),'','html','error');//订单号增加gc 不能用数字验证
		$model_order	= Model('order');
		...
		foreach($order_info['extend_order_goods'] as $k =>$val){
			$commonid = Model('goods')->getGoodsIn(array('goods_id' => $val['goods_id']), 'goods_commonid');
			$order_info['extend_order_goods'][$k]['goods_commonid'] = $commonid[0]['goods_commonid'];
		}
		Tpl::output('order_info',$order_info);
		//卖家发货信息
	...
	private function profile_menu($menu_type,$menu_key='') {
		...
		switch ($menu_type) {
			case 'deliver':
				$menu_array = array(
				array('menu_key'=>'deliverno',			'menu_name'=>Language::get('nc_member_path_deliverno'),	'menu_url'=>'index.php?gct=store_deliver&gp=index&state=deliverno'),
				array('menu_key'=>'delivering',			'menu_name'=>Language::get('nc_member_path_delivering'),	'menu_url'=>'index.php?gct=store_deliver&gp=index&state=delivering'),
				array('menu_key'=>'delivered',		'menu_name'=>Language::get('nc_member_path_delivered'),	'menu_url'=>'index.php?gct=store_deliver&gp=index&state=delivered'),
				);
				break;
			case 'search':
				$menu_array = array(
				1=>array('menu_key'=>'nodeliver',			'menu_name'=>Language::get('nc_member_path_deliverno'),	'menu_url'=>'index.php?gct=store_deliver&gp=index&state=nodeliver'),
				2=>array('menu_key'=>'delivering',			'menu_name'=>Language::get('nc_member_path_delivering'),	'menu_url'=>'index.php?gct=store_deliver&gp=index&state=delivering'),
				3=>array('menu_key'=>'delivered',		'menu_name'=>Language::get('nc_member_path_delivered'),	'menu_url'=>'index.php?gct=store_deliver&gp=index&state=delivered'),
