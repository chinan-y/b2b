...
class show_groupbuyControl extends BaseHomeControl {
    ...
    public function indexOp(){
		...
		$groupbuy = $model_groupbuy->userGroupbuyOnlineList(array(
            'recommended' => 1,
            'is_vr' => 0,
            'time_num' => $_GET['time_num'],
            'be_doing' => $_GET['be_doing']
        ), 9);
		foreach($groupbuy as $k => $val){
			$re = Model('goods')->getGoodsInfo(array('goods_id'=> $val['goods_id']),'goods_jingle, country_code');
			$groupbuy[$k]['goods_jingle'] = $re['goods_jingle'];
			$groupbuy[$k]['country_code'] = $re['country_code'];
			Tpl::output('class_name', $val['time_num']); 
		}
		
		$groupbuy_list = $model_groupbuy->userGroupbuyOnlineList(array('is_vr' => 0), 9 , 'time_num asc', '*');
		...
		foreach($groupbuy_list as $val){
			...
			$val['time_s'] = date('d日 H点', $val['start_time']);
			// $val['time_e'] = date('d天 H点', $val['end_time']);
			$all_groupbuy[$val['time_num']][] = $val;
		}
		
        Tpl::output('groupbuy', $groupbuy);
        Tpl::output('all_groupbuy', $all_groupbuy);
    ...
    private function _show_groupbuy_list($function_name) {
        ...
        $groupbuy_list = $model_xianshi_goods->getXianshiGoodsCommendList(200);
        Tpl::output('groupbuy_list', $groupbuy_list);
        Tpl::output('show_page', $model_groupbuy->showpage(5));
