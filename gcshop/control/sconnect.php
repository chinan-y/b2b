...
class sconnectControl extends BaseHomeControl{
	...
	public function registerOp(){
		...
		if($result) {
			...
			showMessage(Language::get('login_usersave_regist_fail'),SHOP_SITE_URL.'/index.php?gct=login&gp=register','html','error');
		}
	}
	/**
	 * 新浪微博账号绑定已有用户
	...
	public function bindsinaOp(){
		...
		if (is_array($member_info) && count($member_info)>0){
			...
			showMessage(Language::get('home_sconnect_binding_exist'),'index.php?gct=member_connect&gp=sinabind','html','error');
		}
		//处理sina账号信息
		require_once (BASE_PATH.DS.'api'.DS.'sina'.DS.'saetv2.ex.class.php');
		$c = new SaeTClientV2( C('sina_wb_akey'), C('sina_wb_skey') , $_SESSION['slast_key']['access_token']);
		...
		if ($edit_state){
			showMessage(Language::get('home_sconnect_binding_success'),'index.php?gct=member_connect&gp=sinabind');
		}else {
			showMessage(Language::get('home_sconnect_binding_fail'),'index.php?gct=member_connect&gp=sinabind','html','error');
		}
	}
	/**
	 * 更换绑定新浪微博账号
	...
	public function changesinaOp(){
		...
		header('Location:'.SHOP_SITE_URL.'/api.php?gct=tosina');
		exit;
	}
}
