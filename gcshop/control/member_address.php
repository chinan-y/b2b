...
class member_addressControl extends BaseMemberControl{
	...
	public function addressOp() {
		...
		if (!empty($_GET['type'])){
			...
			if (intval($_GET['id']) > 0){
				...
				if ($address_info['member_id'] != $_SESSION['member_id']){
					showMessage($lang['member_address_wrong_argument'],'index.php?gct=member_address&gp=address','html','error');
				}
				/**
				 * 输出地址信息
				 */
		...
		if ($del_id > 0){
			...
			if ($rs){
				showDialog(Language::get('member_address_del_succ'),'index.php?gct=member_address&gp=address','js');
			}else {
	...
	public function default_addressOp() {
		...
		if ($_POST['is_default']) {
			$address_class->editAddress(array('is_default'=>0),array('member_id'=>$_SESSION['member_id'],'is_default'=>1));
			$address_class->editAddress(array('is_default'=>1),array('member_id'=>$_SESSION['member_id'],'address_id'=>$_POST['address_id']));
		}
	}
	...
	public function delivery_addOp() {
	    if (chksubmit()) {
	        ...
	        $data['member_id'] = $_SESSION['member_id'];
	        $data['true_name'] = $_POST['true_name'];
			$data['true_idnum'] = $_POST['true_idnum'];
	...
	private function profile_menu($menu_type,$menu_key='') {
		...
		switch ($menu_type) {
			case 'address':
				$menu_array = array(
				1=>array('menu_key'=>'address','menu_name'=>Language::get('nc_member_path_address_list'),	'menu_url'=>'index.php?gct=member_adderss&gp=address'));
				break;
		}
