...
class store_billControl extends BaseSellerControl {
	...
	public function export_orderOp(){
		...
		if (!is_numeric($_GET['curpage'])){
    		...
    		if ($count > self::EXPORT_SIZE ){
    		    ...
    		    Tpl::output('murl','index.php?gct=store_bill&gp=show_bill&ob_no='.$_GET['ob_no']);
	...
	public function export_refund_orderOp(){
	    ...
	    if (!is_numeric($_GET['curpage'])){
	        ...
	        if ($count > self::EXPORT_SIZE ){	//显示下载链接
	            ...
	            Tpl::output('list',$array);
	            Tpl::output('murl','index.php?gct=store_bill&gp=show_bill&query_type=refund&ob_no='.$_GET['ob_no']);
	            Tpl::showpage('store_export.excel');
	            exit();
	        }else{
	            //如果数量小，直接下载
	...
	private function profile_menu($menu_type,$menu_key='') {
		...
		switch ($menu_type) {
			case 'list':
				$menu_array = array(
				1=>array('menu_key'=>'list','menu_name'=>'实物订单结算', 'menu_url'=>'index.php?gct=bill&gp=list'),
				);
				break;
			case 'show':
				$menu_array = array(
				array('menu_key'=>'order_list','menu_name'=>'订单列表', 'menu_url'=>'index.php?gct=store_bill&gp=show_bill&ob_no='.$_GET['ob_no']),
				array('menu_key'=>'refund_list','menu_name'=>'退款订单',	'menu_url'=>'index.php?gct=store_bill&gp=show_bill&type=refund&ob_no='.$_GET['ob_no']),
				array('menu_key'=>'cost_list','menu_name'=>'促销费用',	'menu_url'=>'index.php?gct=store_bill&gp=show_bill&type=cost&ob_no='.$_GET['ob_no'])
				);
				break;
		}
