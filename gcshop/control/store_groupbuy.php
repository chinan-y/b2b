...
class store_groupbuyControl extends BaseSellerControl {
    ...
    public function __construct() {
        ...
        if (intval(C('groupbuy_allow')) !== 1){
            showMessage(Language::get('groupbuy_unavailable'),'index.php?gct=seller_center','','error');
        }
    }
    /**
     * 默认显示抢购列表
    ...
    public function groupbuy_saveOp() {
        ...
        $goods_info = $model_goods->getGoodsInfoByID($goods_id, 'goods_id,goods_commonid,goods_name,goods_price,goods_marketprice,store_id,virtual_limit');
        ...
        $start= $param['start_time'] = strtotime($_POST['start_time']);
        $end= $param['end_time'] = strtotime($_POST['end_time']);
		/*if(date('H',$start) >= 9 && date('H',$end) ==13){
			$param['time_num'] = 2 ;
		}else if(date('H',$start) >= 13 && date('H',$end) ==17){
			$param['time_num'] = 3 ;
		}else if(date('H',$start) >= 17 && date('H',$end) ==21){
			$param['time_num'] = 4 ;
		}else if(date('H',$start) >= 21 && date('H',$end) ==0){
			$param['time_num'] = 5 ;
		}else if(date('H',$start) >= 0 && date('H',$end) ==9){
			$param['time_num'] = 1 ;
		}*/
		$param['time_num'] = $_POST['time_num'];
        $param['groupbuy_price'] = floatval($_POST['groupbuy_price']);
        $param['groupbuy_rebate'] = ncPriceFormat(floatval($_POST['groupbuy_price'])/floatval($goods_info['goods_marketprice'])*10);
        ...
        if($result) {
            ...
            showDialog(Language::get('groupbuy_add_success'),'index.php?gct=store_groupbuy','succ');
        }else {
            showDialog(Language::get('groupbuy_add_fail'),'index.php?gct=store_groupbuy');
        }
    }
    ...
    private function _image_upload($file) {
        ...
        if(!empty($_FILES[$file]['name'])) {
			if(C(OSS_IS_STORAGE) == 0){
				...
				$upload->set('thumb_width',	'650,650,296');
				$upload->set('thumb_height', '650,650,296');
			...
			if(C(OSS_IS_STORAGE) == 1){
					...
					$objectname = DIR_UPLOAD.DS.ATTACH_PATH.DS.'groupbuy'.DS.$_SESSION['store_id'].DS.$imgfullname;
					$pathname 	= $_FILES[$file]['tmp_name'];
					$result = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
					if($result) {
						$data['file_name'] = $imgfullname;
						$data['origin_file_name'] = $_FILES[$file]['name'];
						$data['file_url'] = gthumb($imgfullname, '210');
					} else {
						$data['result'] = false;
    ...
    private function _image_del($image_name) {
		...
		if(C(OSS_IS_STORAGE) == 1){
			$oss =Logic('oss');
			$oss ->deleteObject(IMAGE_BUCKET_NAME, BASE_UPLOAD_PATH.DS.ATTACH_GROUPBUY.DS.$store_id.DS.$image_name);
		}
    }
    ...
    private function profile_menu($menu_key='') {
        ...
        switch ($menu_key){
        case 'groupbuy_add':
            $menu_array[] = array('menu_key'=>'groupbuy_add','menu_name'=>L('nc_member_path_new_group'),'menu_url'=>'index.php?gct=store_groupbuy&gp=groupbuy_add');
            break;
        case 'groupbuy_add_vr':
            $menu_array[] = array('menu_key'=>'groupbuy_add_vr','menu_name'=>'新增虚拟抢购','menu_url'=>'index.php?gct=store_groupbuy&gp=groupbuy_add_vr');
            break;
        case 'groupbuy_quota_add':
            ...
            break;
        case 'groupbuy_edit':
            $menu_array[] = array('menu_key'=>'groupbuy_edit','menu_name'=>L('nc_member_path_edit_group'),'menu_url'=>'index.php?gct=store_groupbuy');
            break;
        case 'cancel':
