...
class store_account_groupControl extends BaseSellerControl {
    ...
    public function group_saveOp() {
        ...
        if (empty($_POST['group_id'])) {
            ...
            $this->recordSellerLog('添加账号组成功，组编号'.$result);
            ...
            $this->recordSellerLog('编辑账号组成功，组编号'.$_POST['group_id']);
    ...
    public function group_delOp() {
        ...
        if($group_id > 0) {
            ...
            if($result) {
                $this->recordSellerLog('删除账号组成功，组编号'.$group_id);
                showDialog(Language::get('nc_common_op_succ'),'reload','succ');
            } else {
                $this->recordSellerLog('删除账号组失败，组编号'.$group_id);
