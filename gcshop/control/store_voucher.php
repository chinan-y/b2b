...
class store_voucherControl extends BaseSellerControl{
	...
	public function __construct() {
		...
		if (C('voucher_allow') != 1){
			showMessage(Language::get('voucher_unavailable'),'index.php?gct=store','html','error');
		}
		//申请记录状态
	...
	public function templatelistOp(){
		...
		if(is_array($list)){
			...
			if(C(OSS_IS_STORAGE) == 1){
				foreach ($list as $key=>$val){
					if (!$val['voucher_t_customimg']){
						$list[$key]['voucher_t_customimg'] = UPLOAD_SITE_URL.DS.defaultGoodsImage(60);
					}else{
						$list[$key]['voucher_t_customimg'] = UPLOAD_SITE_URL.DS.ATTACH_VOUCHER.DS.$_SESSION['store_id'].DS.$val['voucher_t_customimg'].'?x-oss-process=image/resize,m_lift,w_60';
					}
				}
			}
	...
	public function quotaaddOp(){
		if (chksubmit()){
            ...
            if($reault){
                showDialog(Language::get('voucher_apply_buy_succ'),'index.php?gct=store_voucher&gp=quotalist','succ');
            } else {
                showDialog(Language::get('nc_common_op_fail'),'index.php?gct=store_voucher&gp=quotalist');
            }
        }else {
            //输出导航
    ...
    public function templateaddOp(){
        ...
        if ($isOwnShop = checkPlatformStore()) {
            ...
            if(empty($quotainfo)){
                showMessage(Language::get('voucher_template_quotanull'),'index.php?gct=store_voucher&gp=quotaadd','html','error');
            }
            ...
            if ($count >= C('promotion_voucher_storetimes_limit')){
                ...
                showMessage($message,'index.php?gct=store_voucher&gp=templatelist','html','error');
            }
        }
        ...
        if(empty($pricelist)){
        	showMessage(Language::get('voucher_template_pricelisterror'),'index.php?gct=store_voucher&gp=templatelist','html','error');
        }
        if(chksubmit()){
	        //验证提交的内容面额不能大于限额
	        ...
	        if ($error){
				...
				$insert_arr['voucher_t_sku'] = intval($_POST['txt_template_sku']);//增加代金券指定商品SKU
	        	//自定义图片
		        if (!empty($_FILES['customimg']['name'])){
					...
					if(C(OSS_IS_STORAGE) == 1){
						...
						$extend 	= pathinfo($_FILES['customimg']['name']); 
						$extend 	= strtolower($extend["extension"]); 
						$imgname	= $_SESSION['store_id'].'_'.date('YmdHis').rand(0,9999);
						$imgfullname= $imgname . '.' . $extend;
						$objectname = DIR_UPLOAD.DS.ATTACH_VOUCHER.DS.$_SESSION['store_id'].DS.$imgfullname;
						$pathname 	= $_FILES['customimg']['tmp_name'];
						$result = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
						if ($result){
							$insert_arr['voucher_t_customimg'] =  $imgfullname;
						}
					}
				}
	            $rs = $model->table('voucher_template')->insert($insert_arr);
	            if($rs){
	                showDialog(Language::get('nc_common_save_succ'),'index.php?gct=store_voucher&gp=templatelist','succ');
	            }else{
	                showDialog(Language::get('nc_common_save_fail'),'index.php?gct=store_voucher&gp=templatelist','error');
	            }
	        }
        }else{
            //店铺分类
    ...
    public function templateeditOp(){
    	...
    	if ($t_id <= 0){
    		showMessage(Language::get('wrong_argument'),'index.php?gct=store_voucher&gp=templatelist','html','error');
    	}
        $model = Model('voucher');
        //查询模板信息
        $param = array();
        ...
        if(chksubmit()){
	        ...
	        if ($error){
				...
				$update_arr['voucher_t_sku'] = intval($_POST['txt_template_sku']);//编辑更新商品sku
	        	//自定义图片
		        if (!empty($_FILES['customimg']['name'])){
					if(C(OSS_IS_STORAGE) == 0){
						$upload = new UploadFile();
					...
					if(C(OSS_IS_STORAGE) == 1){
						...
						if ($result){
							//删除原图
							if (!empty($t_info['voucher_t_customimg'])){//如果模板存在，则删除原模板图片
								$oss->deleteObject(IMAGE_BUCKET_NAME, DIR_UPLOAD.DS.ATTACH_VOUCHER.DS.$_SESSION['store_id'].DS.$t_info['voucher_t_customimg']);
							}
							$update_arr['voucher_t_customimg'] =  $imgfullname;
						}
					}
				}
	            $rs = $model->table('voucher_template')->where(array('voucher_t_id'=>$t_info['voucher_t_id']))->update($update_arr);
	            if($rs){
	                showDialog(Language::get('nc_common_op_succ'),'index.php?gct=store_voucher&gp=templatelist','succ');
	            }else{
	                showDialog(Language::get('nc_common_op_fail'),'index.php?gct=store_voucher&gp=templatelist','error');
	            }
	        }
        }else{
			if(C(OSS_IS_STORAGE) ==0){
			...
			if(C(OSS_IS_STORAGE) ==1){
				if (!$t_info['voucher_t_customimg']){
					...
					$t_info['voucher_t_customimg'] = UPLOAD_SITE_URL.DS.ATTACH_VOUCHER.DS.$_SESSION['store_id'].DS.$t_info['voucher_t_customimg'].'?x-oss-process=image/resize,m_lift,w_100';
				}
			}
	...
	private function profile_menu($menu_type,$menu_key='') {
		...
		switch ($menu_type) {
			case 'voucher':
				$menu_array = array(
				1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('nc_member_path_store_voucher'), 'menu_url'=>'index.php?gct=store_voucher&gp=templatelist'),
				);
				break;
			case 'quota_add':
				$menu_array = array(
				1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('nc_member_path_store_voucher'),	'menu_url'=>'index.php?gct=store_voucher&gp=templatelist'),
				4=>array('menu_key'=>'quotaadd','menu_name'=>Language::get('voucher_applyadd'),	'menu_url'=>'index.php?gct=store_voucher&gp=quotaadd')
				);
				break;
			case 'template':
				$menu_array = array(
				1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('nc_member_path_store_voucher'),	'menu_url'=>'index.php?gct=store_voucher&gp=templatelist'),
				2=>array('menu_key'=>'templateadd','menu_name'=>Language::get('voucher_templateadd'),	'menu_url'=>'index.php?gct=store_voucher&gp=templateadd'),
				);
				break;
			case 'templateedit':
				$menu_array = array(
