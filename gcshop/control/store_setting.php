...
class store_settingControl extends BaseSellerControl {
		...
		if (chksubmit()){
			...
			if (!empty($_FILES['store_banner']['name'])){
				...
				if(C(OSS_IS_STORAGE) == 1){
					...
					$extend = pathinfo($_FILES['store_banner']['name']); 
					$extend = strtolower($extend["extension"]); 
					$newname	= 'sba_' . date('YmdHis') . rand(0,99) . '.' . $extend;
					$objectname = DIR_UPLOAD.DS.ATTACH_STORE.DS.$newname;
					$pathname 	= $_FILES['store_banner']['tmp_name'];
					$picinfo 	= getimagesize($_FILES['store_banner']['tmp_name']);
					if($_FILES['store_banner']['type'] =='image/gif' || $_FILES['store_banner']['type'] =='image/jpeg' || $_FILES['store_banner']['type'] =='image/png'){
						if($picinfo[0] < 1000 && $picinfo[1] < 250){
							$result = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
							if ($result){
								$_POST['store_banner'] = $newname;
							}else {
								showDialog('上传图片错误！');
							}
						}
			...
			if (!empty($_POST['store_banner']) && !empty($store_info['store_banner'])){
				...
				if(C(OSS_IS_STORAGE) == 1){
					$oss = Logic('oss');
					$oss->deleteObject(IMAGE_BUCKET_NAME, DIR_UPLOAD.DS.ATTACH_STORE.DS.$store_info['store_banner']);
				}
			}
			...
			if (!empty($_FILES['store_label']['name'])){
				...
				if(C(OSS_IS_STORAGE) == 1){
					...
					$extend = pathinfo($_FILES['store_label']['name']); 
					$extend = strtolower($extend["extension"]); 
					$newname	= 'slg_' . date('YmdHis') . rand(0,99) . '.' . $extend;
					$objectname = DIR_UPLOAD.DS.ATTACH_STORE.DS.$newname;
					$pathname 	= $_FILES['store_label']['tmp_name'];
					$picinfo 	= getimagesize($_FILES['store_label']['tmp_name']);
					if($_FILES['store_label']['type'] =='image/gif' || $_FILES['store_label']['type'] =='image/jpeg' || $_FILES['store_label']['type'] =='image/png'){
						$result = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
						if ($result){
							$_POST['store_label'] = $newname;
						}else {
							showDialog('上传图片错误！');
						}
					}
			...
			if (!empty($_POST['store_label']) && !empty($store_info['store_label'])){
				...
				if(C(OSS_IS_STORAGE) == 1){
					$oss = Logic('oss');
					$oss->deleteObject(IMAGE_BUCKET_NAME, DIR_UPLOAD.DS.ATTACH_STORE.DS.$store_info['store_label']);
				}
			}
			...
			if (!empty($_POST['store_logo']) && !empty($store_info['store_logo'])){
				...
				if(C(OSS_IS_STORAGE) == 1){
					$oss = Logic('oss');
					$oss -> deleteObject(IMAGE_BUCKET_NAME, DIR_UPLOAD.DS.ATTACH_STORE.DS.$store_info['store_logo']);
				}
			}
			/**
			 * 更新入库
	...
	public function store_slideOp() {
		...
		if ($_POST['form_submit'] == 'ok'){
			...
			showDialog(Language::get('nc_common_save_succ'),'index.php?gct=store_setting&gp=store_slide','succ');
		}
		...
		if(is_array($upload_info) && !empty($upload_info)){
			foreach ($upload_info as $val){
				...
				if(C(OSS_IS_STORAGE) == 1){
					$oss =Logic('oss');
					$oss->deleteObject(IMAGE_BUCKET_NAME, DIR_UPLOAD.DS.ATTACH_SLIDE.DS.$val['file_name']);
				}
			}
		}
		$model_upload->delByWhere(array('upload_type'=>3,'item_id'=>$_SESSION['store_id']));
	...
	public function silde_image_uploadOp(){
		...
		if(C(OSS_IS_STORAGE) == 1){
			...
			$extend = pathinfo($_FILES[$_POST['id']]['name']); 
			$extend = strtolower($extend["extension"]); 
			$newname	= 'sli_' . date('YmdHis') . rand(0,99) . '.' . $extend;
			$objectname = DIR_UPLOAD.DS.ATTACH_SLIDE.DS.$newname;
			$pathname 	= $_FILES[$_POST['id']]['tmp_name'];
			$picinfo 	= getimagesize($_FILES[$_POST['id']]['tmp_name']);
			if($_FILES[$_POST['id']]['type'] =='image/gif' || $_FILES[$_POST['id']]['type'] =='image/jpeg' || $_FILES[$_POST['id']]['type'] =='image/png'){
				if($picinfo[0] < 800 &&  $picinfo[1] < 500){
					$result = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
					$output	= array();
					if(!$result){
						$output['error'] = 'error';
		...
		if(intval($_POST['file_id']) > 0){
			...
			if(C(OSS_IS_STORAGE) == 1){
				$oss =Logic('oss');
				$oss->deleteObject(IMAGE_BUCKET_NAME, DIR_UPLOAD.DS.ATTACH_SLIDE.DS.$file_info['file_name']);
			}
			...
			if(!$result){
				...
				if(C(OSS_IS_STORAGE) == 1){
					$oss =Logic('oss');
					$oss->deleteObject(IMAGE_BUCKET_NAME, DIR_UPLOAD.DS.ATTACH_SLIDE.DS.$img_path);
				}
	...
	private function profile_menu($menu_key='') {
        ...
        $menu_array = array(
            1=>array('menu_key'=>'store_setting','menu_name'=>Language::get('nc_member_path_store_config'),'menu_url'=>'index.php?gct=store_setting&gp=store_setting'),
            4=>array('menu_key'=>'store_slide','menu_name'=>Language::get('nc_member_path_store_slide'),'menu_url'=>'index.php?gct=store_setting&gp=store_slide'),
            5=>array('menu_key'=>'store_theme','menu_name'=>'店铺主题','menu_url'=>'index.php?gct=store_setting&gp=theme'),
        );
