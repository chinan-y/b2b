...
class member_favoritesControl extends BaseMemberControl{
	...
	public function fglistOp(){
		...
		if (!empty($favorites_list) && is_array($favorites_list)){
			...
			$goods_model = Model('goods');
			$field = 'goods.goods_id,goods.goods_commonid,goods.goods_name,goods.store_id,goods.goods_image,goods.goods_price,goods.evaluation_count,goods.goods_salenum,goods.goods_collect,store.store_name,store.member_id,store.member_name,store.store_qq,store.store_ww,store.store_domain';
		...
		if ($_GET['type'] == 'html') {
		    ...
		    Tpl::showpage('favor_mini','null_layout');
		} else {
			...
			if ($result){
				...
				if (!empty($fav_arr)){
					if ($type=='goods'){
						...
						showDialog(Language::get('favorite_del_success'),'index.php?gct=member_favorites&gp=fglist&show='.$_GET['show'],'succ');
					}else {
						$fav_str = "'".implode("','",$fav_arr)."'";
						//更新收藏数量
						$store_model = Model('store');
						$store_model->editStore(array('store_collect'=>array('exp', 'store_collect - 1')),array('store_id'=>array('in', $fav_str)));
						showDialog(Language::get('favorite_del_success'),'index.php?gct=member_favorites&gp=fslist','succ');
					}
				}
			}else {
	...
	private function profile_menu($menu_type,$menu_key='') {
		$menu_array = array(
			1=>array('menu_key'=>'fav_goods','menu_name'=>Language::get('nc_member_path_collect_list'),	'menu_url'=>'index.php?gct=member_favorites&gp=fglist'),
			2=>array('menu_key'=>'fav_store','menu_name'=>Language::get('nc_member_path_collect_store'), 'menu_url'=>'index.php?gct=member_favorites&gp=fslist')
		);
