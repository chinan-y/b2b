...
class connectControl extends BaseHomeControl{
	...
	public function registerOp(){
		...
		if (chksubmit()){
			...
			if($result) {
				...
				showMessage(Language::get('login_usersave_regist_fail'),SHOP_SITE_URL.'/index.php?gct=login&gp=register','html','error');//"会员注册失败"
			}
		}
	}
	/**
	...
	public function bindqqOp(){
		...
		if (is_array($member_info) && count($member_info)>0){
			...
			showMessage(Language::get('home_qqconnect_binding_exist'),'index.php?gct=member_connect&gp=qqbind','html','error');//'该QQ账号已经绑定其他商城账号,请使用其他QQ账号与本账号绑定'
		}
		//获取qq账号信息
		require_once (BASE_PATH.'/api/qq/user/get_user_info.php');
		$qquser_info = get_user_info($_SESSION["appid"], $_SESSION["appkey"], $_SESSION["token"], $_SESSION["secret"], $_SESSION["openid"]);
		...
		if ($edit_state){
			showMessage(Language::get('home_qqconnect_binding_success'),'index.php?gct=member_connect&gp=qqbind');
		}else {
			showMessage(Language::get('home_qqconnect_binding_fail'),'index.php?gct=member_connect&gp=qqbind','html','error');//'绑定QQ失败'
		}
	}
	/**
	 * 绑定qq后自动登录
	...
	public function changeqqOp(){
		...
		unset($_SESSION['openid']);
		@header('Location:'.SHOP_SITE_URL.'/api.php?gct=toqq');
		exit;
	}
}
