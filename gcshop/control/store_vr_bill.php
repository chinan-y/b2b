...
class store_vr_billControl extends BaseSellerControl {
	...
	public function export_orderOp(){
		...
		if (!is_numeric($_GET['curpage'])){
    		...
    		if ($count > self::EXPORT_SIZE ){
    		    ...
    		    Tpl::output('murl','index.php?gct=store_vr_bill&gp=show_bill&ob_no='.$_GET['ob_no']);
	...
	private function profile_menu($menu_type,$menu_key='') {
		...
		switch ($menu_type) {
			case 'list':
				$menu_array = array(
				1=>array('menu_key'=>'list','menu_name'=>'虚拟订单结算', 'menu_url'=>'index.php?gct=bill&gp=list'),
				);
				break;
			case 'show':
				$menu_array = array(
				array('menu_key'=>'','menu_name'=>'已使用', 'menu_url'=>'index.php?gct=store_vr_bill&gp=show_bill&ob_no='.$_GET['ob_no']),
				array('menu_key'=>'timeout','menu_name'=>'已过期',	'menu_url'=>'index.php?gct=store_vr_bill&gp=show_bill&type=timeout&ob_no='.$_GET['ob_no'])
				);
				break;
		}
