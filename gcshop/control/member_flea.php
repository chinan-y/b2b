...
class member_fleaControl extends BaseMemberControl{
    ...
    public function flea_listOp() {
		...
		Tpl::output('menu_sign_url','index.php?gct=member_flea');
	...
	public function save_goodsOp() {
		...
		if (!empty($_POST['form_submit']) && $_POST['form_submit'] == 'ok'){
			...
			if($state) {
				...
				showMessage($lang['store_goods_index_flea_add_success'],'index.php?gct=member_flea');
			} else {
				showMessage($lang['store_goods_index_flea_add_fail'],'index.php?gct=member_flea','html','error');
			}
		}
	}
	               	/**
	...
	public function drop_goodsOp() {
		...
		if($state) {
			showDialog($lang['store_goods_index_flea_del_success'],'index.php?gct=member_flea','succ');
		} else {
			showDialog($lang['store_goods_index_flea_del_fail'],'index.php?gct=member_flea','error');
		}
	}
	/**
	 * 编辑闲置物品页面
	...
	public function edit_save_goodsOp() {
		...
		if ($_POST['form_submit'] == 'ok' &&  $goods_id!= 0){
			...
			if($state) {
				...
				showMessage($lang['store_goods_index_flea_goods_edit_success'],'index.php?gct=member_flea');
			} else {
				showMessage($lang['store_goods_index_flea_goods_edit_fail'],'index.php?gct=member_flea','html','error');
			}
		}
	}
	/**
	...
	public function image_uploadOp() {
		...
		if ($_GET['upload_type'] == 'uploadedfile') {
			...
			if(C(OSS_IS_STORAGE) == 1){
				...
				$imgname	= 'fl_'.date('His').rand(0,9999);
				$imgfullname= $imgname . '.' . $extend;
				$objectname = DIR_UPLOAD.DS.ATTACH_MALBUM.DS.$_SESSION['member_id'].DS.$imgfullname;
				...
				if($_FILES['file']['type'] =='image/gif' || $_FILES['file']['type'] =='image/jpeg' || $_FILES['file']['type'] =='image/png'){
					...
					$img_path = $_POST['pic'];
					//**取得图像大小
					list($width, $height, $type, $attr) = getimagesize(UPLOAD_SITE_URL.DS.ATTACH_MALBUM.DS.$_SESSION['member_id'].DS.$img_path);
				}
			}
	...
	public function favoritesOp(){
		...
		if (!empty($_GET['drop']) && $_GET['drop'] == ok && !empty($_GET['fav_id'])){
			...
			showDialog($lang['flea_favorite_del_success'],'index.php?gct=member_flea&gp=favorites','succ');
		}
		/**
		 * 分页
		 */
		...
		Tpl::output('show_page',$page->show());
		Tpl::output('menu_sign_url','index.php?gct=member_flea&gp=favorites');
	...
	private function profile_menu($menu_type,$menu_key='',$array=array()) {
		...
		switch ($menu_type) {
			case 'goods':
				$menu_array	= array(
					1=>array('menu_key'=>'flea_list',	'menu_name'=>闲置列表,			'menu_url'=>'index.php?gct=member_flea')
				);
				break;
			case 'add_goods':
				$menu_array = array(
					1=>array('menu_key'=>'flea_list',	'menu_name'=>闲置列表,			'menu_url'=>'index.php?gct=member_flea'),
					2=>array('menu_key'=>'goods_add',	'menu_name'=>发布闲置,	'menu_url'=>'index.php?gct=member_flea&gp=add_goods')
				);
				break;
			case 'favorites':
				$menu_array = array(
					1=>array('menu_key'=>'favorites',	'menu_name'=>编辑闲置,	'menu_url'=>'index.php?gct=member_flea&gp=favorites')
				);
		}
		if(!empty($array)) {
			$menu_array[] = $array;
