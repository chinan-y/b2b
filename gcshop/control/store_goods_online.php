...
class store_goods_onlineControl extends BaseSellerControl {
    public function __construct() {
		...
		//获取原产国
		$model_messCountrys = Model('mess_country');
		$messCountrys = $model_messCountrys->getMessCountrys();
		Tpl::output('messCountrys', $messCountrys);
		
		//获取申报单位
		$model_legalUnit = Model('legal_unit');
		$legalUnit = $model_legalUnit->getLegalUnit();
		Tpl::output('legalUnit', $legalUnit);

        //获取供货商
        $model_messSupplier = Model('mess_supplier');
        $messSupplier = $model_messSupplier->getMessSupplier();
        Tpl::output('messSupplier', $messSupplier);
		
		//获取商品来源
        $model_source = Model('source');
        $goods_source = $model_source->getGoodsSource();
        Tpl::output('goods_source', $goods_source);
    }
    ...
    public function edit_goodsOp() {
        ...
        $region = Model()->table('region')->order('id asc') ->select();
        ...
        // 取得商品规格的输入值
        $goods_array = $model_goods->getGoodsList($where, 'goods_id,goods_marketprice,goods_price,goods_weight,goods_storage,goods_serial,goods_storage_alarm,goods_spec,sku_spec');
        $sp_value = array();
        if (is_array($goods_array) && !empty($goods_array)) {
            ...
            foreach ( $goods_array as $k => $v ) {
                ...
                if (!empty($a)) {
					...
					$sp_value ['i_' . $id . '|sku_spec'] = $v['sku_spec'];
    ...
    public function edit_save_goodsOp() {
        ...
        $obj_validate->validateparam = array (
			...
			array (
                "input" => $_POST["g_weight"],
                "require" => "true",
                "message" => L('store_goods_index_goods_weight_null')
            ),
            array (
                "input" => $_POST["g_price"],
                "require" => "true",
        ...
        $update_common['records_name']       = $_POST['rec_name'];
        $update_common['goods_jingle']       = $_POST['g_jingle'];
		$update_common['sku_spec']			 = $_POST['g_sku_spec'];
		$update_common['pack_units']		 = $_POST['g_pack_units'];
		$update_common['goods_fenlei']       = $_POST['g_fenlei'];
		$update_common['goods_sorts']		 = $_POST['g_sorts'];
		$update_common['store_from']		 = $_POST['g_from'];
		$update_common['mess_country_code']  = $_POST['g_mess_country_code'];
		$update_common['country_code']		 = $_POST['g_country_code'];
        $update_common['supplier_code']		 = $_POST['g_supplier_code'];
		$update_common['goods_hscode']		 = $_POST['g_hscode'];
		$update_common['goods_reduced']		 = $_POST['g_reduced'];
		if($_POST['unit2']){
			$update_common['unit2']		         = $_POST['unit2'];
		}else{
			$update_common['unit2']		         = null;
		}
		$update_common['unit1']		         = $_POST['unit1'];
		if($_POST['qty2']){
			$update_common['qty2']		         = $_POST['qty2'];
		}else{
			$update_common['qty2']		         = null;
		}
		$update_common['goods_pack']	  	 = $_POST['g_pack'];
		$update_common['goods_con_num']		 = $_POST['g_con_num'];
		...
		if (is_array ( $_POST ['spec'] )) {
            foreach ($_POST['spec'] as $value) {
				$update_common['goods_serial']       = $value['sku'];
			}
		}
        $update_common['post_tax_no']        = $_POST['g_post_tax_no'];
        $update_common['goods_weight']       = floatval($_POST['g_weight']); 
		$update_common['goods_rebate_rate']  = floatval($_POST['g_rebate_rate']);//增加商品返利率
		$update_common['title_key']       	 = $_POST['title_key'];
		$update_common['keywords_key']       = $_POST['keywords_key'];
		$update_common['desc_key']      	 = $_POST['desc_key'];
		if(!empty($_POST['g_valite_time'])){
		$update_common['goods_valite_time']  = strtotime($_POST['g_valite_time']);//增加商品有效期
		}else{$update_common['goods_valite_time']  =0;}
		if(!empty($_POST['g_alarm_time'])){
		$update_common['goods_alarm_time']  = strtotime($_POST['g_alarm_time']);//增加商品有效期预警日期
		}else{$update_common['goods_alarm_time']  =0;}
        ...
        $update_common['goods_video']        = $_POST['g_video'];
        // 序列化保存手机端商品描述数据
        if ($_POST['m_body'] != '') {
...
// header("Content-type: text/html; charset=utf-8");echo '<pre>';print_r($update_common);exit;
        // 开始事务
        ...
        if (is_array ( $_POST ['spec'] )) {
            foreach ($_POST['spec'] as $value) {
                ...
                if (!empty($goods_info)) {
                    ...
                    $update['records_name']      = $update_common['records_name'];
                    $update['goods_jingle']      = $update_common['goods_jingle'];
					$update['sku_spec']			 = $value['sku_spec'];
					$update['pack_units']        = $update_common['pack_units'];
					$update['goods_fenlei']      = $update_common['goods_fenlei'];
					$update['goods_sorts']		 = $update_common['goods_sorts'];
					$update['store_from']        = $update_common['store_from'];
					$update['mess_country_code'] = $update_common['mess_country_code'];
					$update['country_code'] 	 = $update_common['country_code'];
                    $update['supplier_code'] 	 = $update_common['supplier_code'];
					$update['goods_hscode'] 	 = $update_common['goods_hscode'];
					$update['goods_reduced'] 	 = $update_common['goods_reduced'];
					
                    $update['goods_promotion_price']=$update_common['goods_price'];
					...
					$update['post_tax_no']       = $update_common['post_tax_no'];
					$update['goods_weight']		 = $update_common['goods_weight'];
					$update['goods_rebate_rate'] = $update_common['goods_rebate_rate'];
					$update['goods_valite_time'] = $update_common['goods_valite_time'];
					$update['goods_alarm_time']  = $update_common['goods_alarm_time'];
                    ...
                    $insert['records_name']      = $update_common['records_name'];
                    $insert['goods_jingle']      = $update_common['goods_jingle'];
					$insert['sku_spec']			 = $value['sku_spec'];
					$insert['pack_units']		 = $update_common['pack_units'];
					$insert['goods_fenlei']      = $update_common['goods_fenlei'];
					$insert['goods_sorts']		 = $update_common['goods_sorts'];
					$insert['store_from']		 = $update_common['store_from'];
					$insert['mess_country_code'] = $update_common['mess_country_code'];
					$insert['country_code']		 = $update_common['country_code'];
                    $insert['supplier_code']     = $update_common['supplier_code'];
					$insert['goods_hscode']		 = $update_common['goods_hscode'];
					$insert['goods_reduced']		 = $update_common['goods_reduced'];
					...
					$insert['post_tax_no']       = $update_common['post_tax_no'];
					$insert['goods_weight']      = $update_common['goods_weight'];
					$insert['goods_rebate_rate']  = $update_common['goods_rebate_rate'];
					$insert['goods_valite_time']  = $update_common['goods_valite_time'];
					$insert['goods_alarm_time']  = $update_common['goods_alarm_time'];
            ...
            if (!empty($goods_info)) {
                ...
                $update['records_name']      = $update_common['records_name'];
                $update['goods_jingle']      = $update_common['goods_jingle'];
				$update['sku_spec']			 = $update_common['sku_spec'];
				$update['pack_units']        = $update_common['pack_units'];
				$update['goods_fenlei']      = $update_common['goods_fenlei'];
				$update['goods_sorts']		 = $update_common['goods_sorts'];
				$update['store_from']        = $update_common['store_from'];
                ...
                $insert['records_name']      = $update_common['records_name'];
                $insert['goods_jingle']      = $update_common['goods_jingle'];
				$insert['sku_spec']			 = $update_common['sku_spec'];
				$insert['pack_units']        = $update_common['pack_units'];
				$insert['goods_fenlei']      = $update_common['goods_fenlei'];
				$insert['goods_sorts']		 = $update_common['goods_sorts'];
				$insert['store_from']        = $update_common['store_from'];
                ...
                $insert['goods_storage']     = intval($update_common['g_storage']);
                $insert['goods_image']       = $update_common['goods_image'];
                $insert['goods_state']       = $update_common['goods_state'];
                $insert['goods_verify']      = $update_common['goods_verify'];
                $insert['goods_addtime']     = TIMESTAMP;
    ...
    public function goods_unshowOp() {
        ...
        if ($return) {
            ...
            showDialog(L('store_goods_index_goods_unshow_success'), getReferer() ? getReferer() : 'index.php?gct=store_goods_online&gp=goods_list', 'succ', '', 2);
        } else {
    ...
    public function get_goods_list_ajaxOp() {
        ...
        foreach ($goods_list as $key => $val) {
			...
			$commonid = $model_goods->getGoodsIn(array('goods_id' => $val['goods_id']), 'goods_commonid');
			$val['goods_commonid'] =  $commonid[0]['goods_commonid'];
