...
class member_messageControl extends BaseMemberControl {
	...
	public function systemmsgOp(){
		...
		$page->setEachNum(10);
		$page->setStyle('admin');
		$member = Model('member')->getMemberInfo(array('member_id'=>$_SESSION['member_id']),'member_time'); 
		$message_array	= $model_message->listMessage(array('member_time'=>$member['member_time'],'from_member_id'=>'0','message_type'=>'1','to_member_id'=>$_SESSION['member_id'],'no_del_member_id'=>$_SESSION['member_id']),$page);
	...
	public function savemsgOp() {
		...
		if (chksubmit()) {
			...
			if($_GET['type'] == 'sns_board'){
				...
				showDialog(Language::get('home_message_send_success'),'index.php?gct=member_message&gp=privatemsg','succ');
			}
		}
	}
	/**
	 ...
	 * @return
	 */
	public function showmsgcommon1Op() {
		$model_message	= Model('message');
		...
		$drop_state	= $model_message->updateMultitermMessage($param,$_SESSION['member_id']);
	 ...
	 * @return
	 */
	public function showmsgbatchallOp() {
		$model_message	= Model('message');
		$message_id = trim($_GET['message_id']);
		$drop_type = trim($_GET['drop_type']);
		$referer_url = getReferer();
	...
	public function savereplyOp() {
		...
		if (!$isallowsend){
			if($_GET['inajax'] == 1){
				...
				showMessage(Language::get('home_message_noallowsend'),'index.php?gct=member_message&gp=message','html','error');
			}
		}
		if ($_POST['form_submit'] == 'ok') {
			$message_id = intval($_POST["message_id"]);
			if ($message_id <=0){
				showMessage(Language::get('wrong_argument'),'index.php?gct=member_message&gp=message','html','error');
			}
			$obj_validate = new Validate();
			$obj_validate->validateparam = array(
			...
			if($_GET['inajax'] == 1){
				...
				showMessage(Language::get('home_message_send_success'),'index.php?gct=member_message&gp=privatemsg');
			}
		}else {
			if($_GET['inajax'] == 1){
	...
	private function receivedSystemNewNum(){
	    ...
	    $condition_arr['member_time'] = $member['member_time'];
	...
	private function profile_menu($menu_key='') {
		$menu_array	= array(
    		1=>array('menu_key'=>'message',	'menu_name'=>Language::get('home_message_received_message'),'menu_url'=>'index.php?gct=member_message&gp=message'),
    		2=>array('menu_key'=>'private',	'menu_name'=>Language::get('home_message_private_message'),	'menu_url'=>'index.php?gct=member_message&gp=privatemsg'),
    		3=>array('menu_key'=>'system',	'menu_name'=>Language::get('home_message_system_message'),	'menu_url'=>'index.php?gct=member_message&gp=systemmsg'),
    		4=>array('menu_key'=>'close',	'menu_name'=>Language::get('home_message_close'),	'menu_url'=>'index.php?gct=member_message&gp=personalmsg'),
		    5=>array('menu_key'=>'setting', 'menu_name'=>'接收设置', 'menu_url'=>urlShop('member_message', 'setting'))
		);
		if($menu_key == 'sendmsg') {
			$menu_array[] = array('menu_key'=>'sendmsg','menu_name'=>Language::get('home_message_send_message'),'menu_url'=>'index.php?gct=member_message&gp=sendmsg');
