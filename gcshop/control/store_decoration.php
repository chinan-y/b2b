...
class store_decorationControl extends BaseSellerControl {
    ...
    public function decoration_album_uploadOp() {
		...
		if(C(OSS_IS_STORAGE) == 1){
			...
			$newname	= $store_id . '_' . date('YmdHis') . rand(0,99) . '.' . $extend;
			$objectname = DIR_UPLOAD.DS.ATTACH_STORE_DECORATION.DS . $store_id . DS.$newname;
			$pathname 	= $_FILES['file']['tmp_name'];
			$picinfo 	= getimagesize($_FILES['file']['tmp_name']);
			if($_FILES['file']['type'] =='image/gif' || $_FILES['file']['type'] =='image/jpeg' || $_FILES['file']['type'] =='image/png'){
				$result_file = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
				if($result_file){
					$image = $newname;
				}else{
					$error = 'upload error!';
			...
			//图片尺寸
			list($width, $height) = getimagesize(UPLOAD_SITE_URL . DS . ATTACH_STORE_DECORATION . DS . $store_id . DS . $image);	
			}
		}
