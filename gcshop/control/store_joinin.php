...
class store_joininControl extends BaseHomeControl {
	...
	public function __construct() {
        ...
        if($_GET['gp'] != 'check_seller_name_exist' && $_GET['gp'] != 'checkname') {
            ...
            switch (intval($joinin_detail['joinin_state'])) {
                ...
                case STORE_JOIN_STATE_VERIFY_FAIL:
                    if(!in_array($_GET['gp'], array('step1', 'step2', 'step3', 'step4'))) {
                        $this->show_join_message('审核失败:'.$joinin_detail['joinin_message'], SHOP_SITE_URL.DS.'index.php?gct=store_joinin&gp=step1');
                    }
                    break;
                case STORE_JOIN_STATE_PAY_FAIL:
                    if(!in_array($_GET['gp'], array('pay', 'pay_save'))) {
                        $this->show_join_message('付款审核失败:'.$joinin_detail['joinin_message'], SHOP_SITE_URL.DS.'index.php?gct=store_joinin&gp=pay');
                    }
                    break;
    ...
    public function step4Op() {
        ...
        @header('location: index.php?gct=store_joinin');
