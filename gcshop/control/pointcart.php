...
class pointcartControl extends BasePointShopControl {
	public function __construct() {
		...
		if ($_SESSION['is_login'] != '1'){
			showDialog(L('pointcart_unlogin_error'),'index.php?gct=login','error');
		}
	}
	/**
	 * 积分礼品购物车首页
	...
	public function addOp() {
		...
		if (!$data['state']){
		    switch ($data['error']){
		        case 'ParameterError':
		            echo json_encode(array('done'=>false,'msg'=>$data['msg'],'url'=>'index.php?gct=pointprod&gp=plist')); die;
		            break;
		        default:
	...
	public function step1Op(){
		...
		if (!$data['state']){
		    showDialog($data['msg'],'index.php?gct=pointprod','error');
		}
	...
	public function step2Op() {
		...
		if (!$data['state']){
		    showDialog($data['msg'],'index.php?gct=pointcart','error');
		}
		...
		//创建兑换订单
		$data = Model('pointorder')->createOrder($_POST, $pointprod_arr, array('member_id'=>$_SESSION['member_id'],'member_name'=>$_SESSION['member_name'],'member_email'=>$_SESSION['member_email'],'member_mobile'=>$_SESSION['member_mobile']));
		if (!$data['state']){
		    showDialog($data['msg'],'index.php?gct=pointcart&gp=step1','error');
		}
		$order_id = $data['data']['order_id'];
		@header("Location:index.php?gct=pointcart&gp=step3&order_id=".$order_id);
	}
	/**
	 * 流程第三步
	 */
