...
class buy_virtualControl extends BaseBuyControl {
    ...
    public function buy_step3Op() {
        ...
        //转向到商城支付页面
        redirect('index.php?gct=buy_virtual&gp=pay&order_id='.$result['data']['order_id']);
    }
    ...
    public function payOp() {
        ...
        if ($order_id <= 0){
            showMessage('该订单不存在','index.php?gct=member_vr_order','html','error');
        }
        ...
        if ($diff_pay_amount == 0) {
            redirect('index.php?gct=buy_virtual&gp=pay_ok&order_sn='.$order_info['order_sn'].'&order_id='.$order_info['order_id'].'&order_amount='.ncPriceFormat($order_info['order_amount']));
        }
        ...
        if (empty($payment_list)) {
            showMessage('暂未找到合适的支付方式','index.php?gct=member_vr_order','html','error');
        }
