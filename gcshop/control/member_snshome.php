...
class member_snshomeControl extends BaseSNSControl {
	...
	public function indexOp(){
		...
		if (!empty($storelist)){
			...
			if (!empty($goodslist)){
				foreach ($goodslist as $k=>$v){
					$v['goodsurl'] = urlShop('goods', 'index', array('goods_id'=>$v['goods_id'],'ref'=>$_SESSION['member_id']));
	...
	public function goodsinfoOp(){
		...
		if ($share_id <= 0){
			showDialog(Language::get('wrong_argument'),"index.php?gct=member_snshome&mid={$this->master_id}",'error');
		}
		//查询分享和喜欢商品信息
		$sharegoods_model = Model('sns_sharegoods');
		$condition = array();
		...
		$sharegoods_info['snsgoods_goodsurl'] = urlShop('goods', 'index', array('goods_id'=>$sharegoods_info['snsgoods_goodsid'],'ref'=>$_SESSION['member_id']));
