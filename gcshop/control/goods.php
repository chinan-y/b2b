...
class goodsControl extends BaseGoodsControl {
	...
	public function indexOp() {
		...
		//检测是否有推荐组合商品
		$gpgoods_info = $model_goods -> getGoodsOnlineInfoByID($goods_id);
		if (!empty($gpgoods_info) && $model_goods -> checkIsGeneral($gpgoods_info)) {
			$array = Model('goods_combo') -> getGoodsComboCacheByGoodsId($goods_id);
			Tpl::output('gpgoods_info', $gpgoods_info);
		...
		//用户浏览商品增加积分，每天有积分增加上限
		$member = Model('member')->getMemberInfo(array('member_id'=>$_SESSION['member_id']),'browse_goods_up');
		if($member['browse_goods_up'] >= intval(C('browse_goods_up'))){
			$upper = 0;
		}else{
			$upper = 1 ;
			$re = Model('points')->savePointsLog('browse',array('pl_memberid'=>$_SESSION['member_id'],'pl_membername'=>$_SESSION['member_name'],'pl_desc'=>$goods_info['goods_name']));
		}
		Tpl::output('upper', $upper);
		
		//商品SEO优化信息
		$seo_param = array();
		$seo_param['name'] 			= $goods_info['goods_name'];
		if($goods_info['title_key']){
			$seo_param['title_key'] 	= $goods_info['title_key'];
		}else{
			$seo_param['title_key'] 	= $goods_info['goods_name'].'【价格_图片_评价-光彩全球网】';
		}
		$seo_param['keywords_key'] 	= $goods_info['keywords_key'];
		if($goods_info['desc_key']){
			$seo_param['desc_key'] 		= $goods_info['desc_key'];
		}else{
			$seo_param['desc_key'] 		= $goods_info['goods_jingle'];
		}
	...
	public function comments_listOp() {
		...
		// 当前位置导航
		$nav_link_list = Model('goods_class') -> getGoodsClassNav($goods_info['gc_id'], 0);
		$nav_link_list[] = array('title' => $goods_info['goods_name'], 'link' => urlShop('goods', 'index', array('goods_id' => $goods_id, 'ref' => $_SESSION['member_id'])));
	...
	private function _get_comments($goods_id, $type, $page) {
		...
		foreach($goodsevallist as $k=>$val){
			if(preg_match("/^gc1[34578]\d{9}$/", $val['geval_frommembername'])){
				$goodsevallist[$k]['geval_frommembername'] = substr($val['geval_frommembername'],0,5).'****'.substr($val['geval_frommembername'],-4);
			}
			if(preg_match("/^1[34578]\d{9}$/", $val['geval_frommembername'])){
				$goodsevallist[$k]['geval_frommembername'] = substr($val['geval_frommembername'],0,3).'****'.substr($val['geval_frommembername'],-4);
			}
		}
