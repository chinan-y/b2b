...
//		Tpl::output('show_page',$page->show());
	...
	public function album_add_saveOp(){
		if (chksubmit()){
			...
			if($class_count['count'] >= 20){
				showDialog(Language::get('album_class_save_max_20'),'index.php?gct=store_album','error',empty($_GET['inajax'])?'':'CUR_DIALOG.close();');
			}
			/**
			 * 实例化相册模型
			 */
			...
			if($return){
				$this->recordSellerLog('创建相册，相册名称:'.$param['aclass_name']);
				showDialog(Language::get('album_class_save_succeed'),'index.php?gct=store_album','succ',empty($_GET['inajax'])?'':'CUR_DIALOG.close();');
			}
		}
	...
	public function album_edit_saveOp(){
		...
		if($return){
			...
			if($re){
				$this->recordSellerLog('编辑相册，相册名称:'.$param['aclass_name']);
				showDialog(Language::get('album_class_edit_succeed'),'index.php?gct=store_album','succ',empty($_GET['inajax'])?'':'CUR_DIALOG.close();');
			}
		}else{
	...
	public function album_delOp(){
		...
		if(!$return){
			...
			$this->recordSellerLog('删除相册，相册id:'.$_GET['id']);
		}
		/**
		 * 更新图片分类
		 */
		...
		if($return){
			showDialog(Language::get('album_class_file_del_succeed'),'index.php?gct=store_album','succ');
		}else{
			showDialog(Language::get('album_class_file_del_lose'));
		}
	}
	...
	public function album_pic_delOp(){
		...
		if(!empty($_POST['goods_commonid']) && is_array($_POST['goods_commonid'])){
			$goods_commonid = implode("','", $_POST['goods_commonid']);
		}else{
		...
		//删除图片
		$return = $model_album->delPic($id, $_SESSION['store_id'], $goods_commonid);
		if($return){
    ...
    public function replace_image_uploadOp() {
        $file = $_GET['id'];
        $goods_commonid = intval($_GET['goods_commonid'])?intval($_GET['goods_commonid']):0;
		...
		if(C(OSS_IS_STORAGE) == 0){
			...
			$upload->set('default_dir', ATTACH_GOODS . DS . $_SESSION['store_id'] . DS . $goods_commonid. DS . $pic_cover);
			...
			/*** 取得图像大小*/
			list($width, $height, $type, $attr) = getimagesize(BASE_UPLOAD_PATH . DS . ATTACH_GOODS . DS . $_SESSION['store_id'] . DS . $goods_commonid. DS . $apic_info['apic_cover']);
		}
		...
		if(C(OSS_IS_STORAGE) == 1){
			...
			$imgfullname= $apic_info['apic_cover'];
			$objectname = DIR_UPLOAD.DS.ATTACH_GOODS.DS. $_SESSION['store_id'] . DS . $goods_commonid. DS .$imgfullname;
			$pathname 	= $_FILES[$file]['tmp_name'];
			if($_FILES[$file]['type'] =='image/gif' || $_FILES[$file]['type'] =='image/jpeg' || $_FILES[$file]['type'] =='image/png'){
				$result 	= $oss->uploadFile($bucketname, $objectname, $pathname, $options);
			}

			list($width, $height, $type, $attr) = getimagesize(UPLOAD_SITE_URL . DS . ATTACH_GOODS . DS . $_SESSION['store_id'] . DS . $goods_commonid. DS .$apic_info['apic_cover']);
		}
	...
	public function album_pic_watermarkOp(){
		...
		if ($store_wm_info['wm_image_name'] == '' && $store_wm_info['wm_text'] == ''){
			showDialog(Language::get('album_class_setting_wm'),"index.php?gct=store_album&gp=store_watermark",'error','CUR_DIALOG.close();');//"请先设置水印"
		}
		...
		if (chksubmit()){
			...
			if (!empty($_FILES['image']['name'])){
				...
				if(C(OSS_IS_STORAGE) == 1){
					...
					$extend 	= pathinfo($_FILES['image']['name']); 
					$extend 	= strtolower($extend["extension"]); 
					$imgname	= 'w_' . date('YmdHis') . rand(0,99);
					$imgfullname= $imgname . '.' . $extend;
					$objectname = DIR_UPLOAD.DS.ATTACH_WATERMARK.DS .$imgfullname;

					$pathname 	= $_FILES['image']['tmp_name'];
					if($_FILES['image']['type'] =='image/gif' || $_FILES['image']['type'] =='image/jpeg' || $_FILES['image']['type'] =='image/png'){
						$result = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
						if ($result){
							$param['wm_image_name'] = $imgfullname;
							/** * 删除旧水印*/
							if (!empty($store_wm_info['wm_image_name'])){
								$delobjectname = DIR_UPLOAD.DS.ATTACH_WATERMARK.DS.$store_wm_info['wm_image_name'];
								$delresult = $oss->deleteObject($bucketname, $delobjectname); 
							}
						}else {
							showDialog('Error!');
						}
				...
				if (!empty($store_wm_info['wm_image_name'])){
					...
					if(C(OSS_IS_STORAGE) == 1){
						...
						$delobjectname = DIR_UPLOAD.DS.ATTACH_WATERMARK.DS.$store_wm_info['wm_image_name'];
						$param['wm_image_name'] = '';
						$delresult = $oss->deleteObject($bucketname, $delobjectname);//删除不掉OSS对象，待解决
					}
    ...
    public function image_uploadOp() {
		...
		if(C(OSS_IS_STORAGE) == 1){
			...
			$extend 	= strtolower($extend["extension"]); 
			$imgname	= $store_id. '_' . date('YmdH') . rand(0,999) . 'c' . $commonid;//遵循上传图片的命名规则
			$imgfullname= $imgname . '.' . $extend;
			$objectname = DIR_UPLOAD . DS . ATTACH_GOODS . DS . $store_id .DS . $commonid . DS . $imgfullname;
			...
			list($width, $height, $type, $attr) = getimagesize(UPLOAD_SITE_URL.DS.ATTACH_GOODS.DS.$store_id.DS.$commonid . DS . $pic);
		}
	...
	private function profile_menu($menu_type,$menu_key=''){
		...
		switch ($menu_type) {
			case 'album':
				$menu_array	= array(
				1=>array('menu_key'=>'album','menu_name'=>Language::get('nc_member_path_my_album'),'menu_url'=>'index.php?gct=store_album'),
				2=>array('menu_key'=>'watermark','menu_name'=>Language::get('nc_member_path_watermark'),'menu_url'=>'index.php?gct=store_album&gp=store_watermark')
				);
				break;
			case 'album_pic':
				$menu_array	= array(
				1=>array('menu_key'=>'album','menu_name'=>Language::get('nc_member_path_my_album'),'menu_url'=>'index.php?gct=store_album'),
				2=>array('menu_key'=>'pic_list','menu_name'=>Language::get('nc_member_path_album_pic_list'),'menu_url'=>'index.php?gct=store_album&gp=album_pic_list&id='.intval($_GET['id'])),
				3=>array('menu_key'=>'watermark','menu_name'=>Language::get('nc_member_path_watermark'),'menu_url'=>'index.php?gct=store_album&gp=store_watermark')
				);
				break;
			case 'album_pic_info':
				$menu_array	= array(
				1=>array('menu_key'=>'album','menu_name'=>Language::get('nc_member_path_my_album'),'menu_url'=>'index.php?gct=store_album'),
				2=>array('menu_key'=>'pic_info','menu_name'=>Language::get('nc_member_path_album_pic_info'),'menu_url'=>'index.php?gct=store_album&gp=album_pic_info&id='.intval($_GET['id']).'&class_id='.intval($_GET['class_id'])),
				3=>array('menu_key'=>'watermark','menu_name'=>Language::get('nc_member_path_watermark'),'menu_url'=>'index.php?gct=store_album&gp=store_watermark')
				);
				break;
		}
