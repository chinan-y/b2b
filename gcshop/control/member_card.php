...
class member_cardControl extends BaseHomeControl{
	...
	public function indexOp(){
		...
		if(preg_match("/^gc1[34578]\d{9}$/", $member_info['member_name'])){
			$data['name'] = substr($member_info['member_name'],0,5).'****'.substr($member_info['member_name'],-4);
		}
		if(preg_match("/^1[34578]\d{9}$/", $member_info['member_name'])){
			$data['name'] = substr($member_info['member_name'],0,3).'****'.substr($member_info['member_name'],-4);
		}
