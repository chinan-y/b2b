...
class predepositControl extends BaseMemberControl {
	...
	public function recharge_addOp(){
        ...
        if ($insert) {
            //转向到商城支付页面
            redirect('index.php?gct=buy&gp=pd_pay&pay_sn='.$pay_sn);
        }
	}
	...
	public function pd_cash_addOp(){
		if (chksubmit()){
			...
			$validate_arr[] = array("input"=>$_POST["alipay_no"], "require"=>"true","message"=>Language::get('predeposit_cash_add_shoukuanaccountnull_error'));
			$validate_arr[] = array("input"=>$_POST["alipay_user"], "require"=>"true","message"=>Language::get('predeposit_cash_add_shoukuannamenull_error'));
			...
			if (floatval($member_info['available_predeposit']) < $pdc_amount){
				showDialog(Language::get('predeposit_cash_shortprice_error'),'index.php?gct=predeposit&gp=pd_cash_list','error');
			}
			try {
    			...
    			$data['pdc_bank_name'] = '支付宝转账';
    			$data['pdc_bank_no'] = $_POST['alipay_no'];
    			$data['pdc_bank_user'] = $_POST['alipay_user'];
    			...
    			$model_pd->commit();
    			showDialog(Language::get('predeposit_cash_add_success'),'index.php?gct=predeposit&gp=pd_cash_list','succ','CUR_DIALOG.close()');
			} catch (Exception $e) {
			    $model_pd->rollback();
			    showDialog($e->getMessage(),'index.php?gct=predeposit&gp=pd_cash_list','error');
			}
		}
	}
	...
	public function pd_cash_infoOp(){
		...
		if ($pdc_id <= 0){
			showMessage(Language::get('predeposit_parameter_error'),'index.php?gct=predeposit&gp=pd_cash_list','html','error');
		}
		$model_pd = Model('predeposit');
		$condition = array();
		$condition['pdc_member_id'] = $_SESSION['member_id'];
		...
		if (empty($info)){
			showMessage(Language::get('predeposit_record_error'),'index.php?gct=predeposit&gp=pd_cash_list','html','error');
		}
	...
	private function profile_menu($menu_type,$menu_key=''){
		$menu_array	= array(
			array('menu_key'=>'loglist','menu_name'=>Language::get('nc_member_path_predeposit_loglist'),	'menu_url'=>'index.php?gct=predeposit&gp=pd_log_list'),
			array('menu_key'=>'recharge_list','menu_name'=>Language::get('nc_member_path_predeposit_rechargelist'),	'menu_url'=>'index.php?gct=predeposit&gp=index'),
			array('menu_key'=>'cashlist','menu_name'=>Language::get('nc_member_path_predeposit_cashlist'),	'menu_url'=>'index.php?gct=predeposit&gp=pd_cash_list'),
            array(
                'menu_key' => 'rcb_log_list',
                'menu_name' => '充值卡余额',
                'menu_url' => 'index.php?gct=predeposit&gp=rcb_log_list',
            ),
		);
		switch ($menu_type) {
			case 'rechargeinfo':
			...
			case 'log':
				break;
			case 'cashadd':
			    $menu_array[] = array('menu_key'=>'cashadd','menu_name'=>Language::get('nc_member_path_predeposit_cashadd'),	'menu_url'=>'index.php?gct=predeposit&gp=pd_cash_add');
				break;
			case 'cashinfo':
