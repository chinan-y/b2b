...
class member_orderControl extends BaseMemberControl {
    ...
    public function search_deliverOp(){
		...
		foreach($order_info['extend_order_goods'] as $k =>$val){
			$goods_commonid = Model('goods')->getGoodsList(array('goods_id' => $val['goods_id']),'goods_commonid');
			$order_info['extend_order_goods'][$k]['goods_commonid'] = $goods_commonid[0]['goods_commonid']; 
		}
    ...
    public function show_orderOp() {
        ...
        foreach ($order_info['extend_order_goods'] as $k =>$value) {
			$commonid = Model('goods')->getGoodsIn(array('goods_id' => $value['goods_id']), 'goods_commonid, store_from');
	...
	private function profile_menu($menu_key='') {
	    ...
	    $menu_array = array(
	        array('menu_key'=>'member_order','menu_name'=>Language::get('nc_member_path_order_list'), 'menu_url'=>'index.php?gct=member_order'),
	        array('menu_key'=>'member_order_recycle','menu_name'=>'回收站', 'menu_url'=>'index.php?gct=member_order&recycle=1'),
	    );
