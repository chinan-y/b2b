    ...
    public function indexOp() {
        ...
        if($gid_arr){
            ...
            for ($j=0; $j<self::MAXNUM; $j++){
                $compare_list[$i][] = $compare_listtmp[$j]?array('goods_id'=>$compare_listtmp[$j]['goods_id'],'goods_name'=>$compare_listtmp[$j]['goods_name'],'goods_image'=>$compare_listtmp[$j]['goods_image'],'store_id'=>$compare_listtmp[$j]['store_id'], 'goods_commonid'=>$compare_listtmp[$j]['goods_commonid']):'';
            }
            $i++;
            //商品价格
                ...
                foreach ($compare_listtmp[0]['spec_name_arr'] as $k=>$v){
                    ...
                    for ($j=0;$j<self::MAXNUM;$j++){
                        ...
                        if ($compare_listtmp[$j]){
                            //处理规格值
                            if ($compare_listtmp[$j]['spec_value_arr'][$k] && $compare_listtmp[$j]['goods_spec_arr']){
                                $tmp = array_values(array_intersect_assoc($compare_listtmp[$j]['spec_value_arr'][$k],$compare_listtmp[$j]['goods_spec_arr']));
                                if ($tmp){
