...
class store_printsetupControl extends BaseSellerControl {
	...
	public function indexOp(){
		...
		if(chksubmit()){
			...
			if ($rs){
				showDialog(Language::get('nc_common_save_succ'),'index.php?gct=store_printsetup','succ');
			}else {
				showDialog(Language::get('nc_common_save_fail'),'index.php?gct=store_printsetup','error');
			}
		}else{
	...
	private function profile_menu($menu_key='') {
        ...
        $menu_array = array(
            1=>array('menu_key'=>'store_printsetup','menu_name'=>'打印设置','menu_url'=>'index.php?gct=store_printsetup&gp=index'),
        );
