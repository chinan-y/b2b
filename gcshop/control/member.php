...
class memberControl extends BaseMemberControl{
    ...
    public function ajax_load_order_infoOp() {
		...
		foreach($cart_list as $k=>$val){
			...
			$cart_list[$k]['goods_commonid'] =  $commonid[0]['goods_commonid'];
		}
     ...
     * 会员身份信息
     */
    public function add_identityOp() {
		...
		if (chksubmit()){
			...
			$result = $member->editMember(array('member_id'=>$member_id),$data);
