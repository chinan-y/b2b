...
class store_goods_addControl extends BaseSellerControl {
    ...
    private function checkStore(){
        ...
        if ($goodsLimit > 0) {
            ...
            if ($goods_num >= $goodsLimit) {
                showMessage(L('store_goods_index_goods_limit') . $goodsLimit . L('store_goods_index_goods_limit1'), 'index.php?gct=store_goods_online&gp=goods_list', 'html', 'error');
            }
        }
    }
    ...
    public function save_goodsOp() {
        if (chksubmit()) {
            ...
            $common_array['records_name']       = $_POST['rec_name'];
            $common_array['goods_jingle']       = $_POST['g_jingle'];
			$common_array['sku_spec']			= $_POST['g_sku_spec'];
			$common_array['pack_units']			= $_POST['g_pack_units'];
			$common_array['goods_fenlei']       = $_POST['g_fenlei'];
			$common_array['goods_sorts']		= $_POST['g_sorts'];
			$common_array['store_from']			= $_POST['g_from'];
			$common_array['mess_country_code']  = $_POST['g_mess_country_code'];
			$common_array['country_code']		= $_POST['g_country_code'];
            $common_array['supplier_code']		= $_POST['g_supplier_code'];
			$common_array['goods_hscode']		= $_POST['g_hscode'];
			$common_array['goods_reduced']		= $_POST['g_reduced'];
			if(!empty($_POST['unit2'])){
				$common_array['unit2']		        = $_POST['unit2'];
			}else{
				$common_array['unit2']		        = null;
			}
			$common_array['unit1']		        = $_POST['unit1'];
			if(!empty($_POST['qty2'])){
				$common_array['qty2']		        = $_POST['qty2'];
			}else{
				$common_array['qty2']		        = null;
			}
            ...
            $common_array['goods_serial']       = $_POST['g_serial'];
			 ...
			 if (is_array ( $_POST ['spec'] )) {
				foreach ($_POST['spec'] as $value) {
					$common_array['goods_serial']       = $value['sku'];
				}
			 }
            $common_array['post_tax_no']        = $_POST['g_post_tax_no'];
            $common_array['goods_weight']       = floatval($_POST['g_weight']);
			$common_array['goods_rebate_rate']  = floatval($_POST['g_rebate_rate']);
			$common_array['title_key']       	= $_POST['title_key'];
			$common_array['keywords_key']       = $_POST['keywords_key'];
			$common_array['desc_key']      	 	= $_POST['desc_key'];
			if(!empty($_POST['g_valite_time'])){
			$common_array['goods_valite_time']  = strtotime($_POST['g_valite_time']);//增加商品有效期
			}else{$common_array['goods_valite_time']  = 0;}
			if(!empty($_POST['g_alarm_time'])){
			$common_array['goods_alarm_time']  = strtotime($_POST['g_alarm_time']);//增加商品有效期预警日期
			}else{$common_array['goods_alarm_time']  =0;}
			...
			$common_array['goods_video']        = $_POST['g_video'];
            ...
            // 保存数据
			// header("Content-type: text/html; charset=utf-8");echo '<pre>';print_r($common_array);exit;
            ...
            if ($common_id) {
                ...
                if (is_array($_POST['spec'])) {
                    foreach ($_POST['spec'] as $value) {
                        ...
                        $goods['records_name']      = $common_array['records_name'];
                        $goods['goods_jingle']      = $common_array['goods_jingle'];
						$goods['sku_spec']			= $value['sku_spec'];
						$goods['pack_units']		= $common_array['pack_units'];
						$goods['goods_fenlei']      = $common_array['goods_fenlei'];
						$goods['goods_sorts']		= $common_array['goods_sorts'];
						$goods['store_from']		= $common_array['store_from'];
						$goods['mess_country_code'] = $common_array['mess_country_code'];
						$goods['country_code'] 	 	= $common_array['country_code'];
                        $goods['supplier_code'] 	= $common_array['supplier_code'];
						$goods['goods_hscode'] 	 	= $common_array['goods_hscode'];
						$goods['goods_reduced'] 	= $common_array['goods_reduced'];
						...
						$goods['post_tax_no']       = $common_array['post_tax_no'];
						$goods['goods_weight']		= $common_array['goods_weight'];
						$goods['goods_rebate_rate']	= $common_array['goods_rebate_rate'];
						$goods['goods_valite_time']	= $common_array['goods_valite_time'];
						$goods['goods_alarm_time']	= $common_array['goods_alarm_time'];
			 ...
			 // 生成商品二维码
                    ...
                    $goods['records_name']      = $common_array['records_name'];
                    $goods['goods_jingle']      = $common_array['goods_jingle'];
					$goods['sku_spec']			= $common_array['sku_spec'];
					$goods['pack_units']		= $common_array['pack_units'];
					$goods['goods_fenlei']      = $common_array['goods_fenlei'];
					$goods['goods_sorts']		= $common_array['goods_sorts'];
					$goods['store_from']		= $common_array['store_from'];
		    ...
		    $PhpQRCode->set('date',WAP_SITE_URL . '/tmpl/product_detail.html?goods_id='.$goods_id);
                ...
                // 记录日志
                $this->recordSellerLog('发布新商品，平台货号:'.$common_id);
    ...
    public function image_uploadOp() {
		...
		if(C(OSS_IS_STORAGE) == 0){
			...
			$upload->set('default_dir', ATTACH_GOODS . DS . $_SESSION ['store_id'] . DS . $upload->getSysSetPath(). DS .$commonid);
			...
			// 取得图像大小
			list($width, $height, $type, $attr) = getimagesize(BASE_UPLOAD_PATH . DS . ATTACH_GOODS . DS . $_SESSION['store_id'] . DS . $upload->getSysSetPath(). DS .$commonid. DS . $img_path);
			
			
			$data = array ();
			$data ['thumb_name'] = cthumb($upload->getSysSetPath() . $upload->thumb_image, 240, $_SESSION['store_id'], $commonid);
			$data ['name']      = $upload->getSysSetPath().$img_path;
		}
		...
		if(C(OSS_IS_STORAGE) == 1){
			...
			$extend 	= pathinfo($_FILES[$_POST['name']]['name']); 
			$extend 	= strtolower($extend["extension"]); 
			$imgname	= $_SESSION ['store_id'] . '_' . date('YmdH') . rand(0,999). 'c' . $commonid;//遵循上传图片的命名规则
			$imgfullname= $imgname . '.' . $extend;
			$objectname = DIR_UPLOAD.DS.ATTACH_GOODS.DS . $_SESSION ['store_id'] . DS .$commonid. DS .$imgfullname;
			
			$pathname 	= $_FILES[$_POST['name']]['tmp_name'];
			if($_FILES[$_POST['name']]['type'] =='image/gif' || $_FILES[$_POST['name']]['type'] =='image/jpeg' || $_FILES[$_POST['name']]['type'] =='image/png'){
				$result = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
			}
			
			$img_path = $imgfullname;
			list($width, $height, $type, $attr) = getimagesize(UPLOAD_SITE_URL.DS.ATTACH_GOODS.DS . $_SESSION ['store_id'] . DS .$commonid. DS .$img_path);
			
			$data = array ();
			$data ['thumb_name'] = cthumb($imgfullname, 240, $_SESSION['store_id'], $commonid);  //function->goods.php->function cthumb
			$data ['name']      = $img_path;
		}
        ...
        $insert_array['goods_commonid'] = $commonid;
