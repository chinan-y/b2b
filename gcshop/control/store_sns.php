...
class store_snsControl extends BaseSellerControl{
    ...
    public function image_uploadOp() {
		...
		if(C(OSS_IS_STORAGE) == 1){
			...
			$imgname	= 'sac_' . date('YmdHis') . rand(0,9999);
			$imgfullname= $imgname . '.' . $extend;
			$objectname = DIR_UPLOAD.DS.ATTACH_GOODS.DS.$_SESSION['store_id'].DS.'0'.DS.$imgfullname;
			...
			if($_FILES[$_POST['id']]['type'] =='image/gif' || $_FILES[$_POST['id']]['type'] =='image/jpeg' || $_FILES[$_POST['id']]['type'] =='image/png'){
				...
				$thumb_page = $imgfullname;
				// 取得图像大小
				list($width, $height, $type, $attr) = getimagesize(UPLOAD_SITE_URL.DS.ATTACH_GOODS.DS.$_SESSION['store_id'].DS.'0'.DS.$img_path);
			}
		}
	...
	public function store_sns_saveOp(){
		...
		if($rs){
			showDialog(Language::get('nc_common_op_succ'), 'index.php?gct=store_sns', 'succ');
		}else{
	...
	private function profile_menu($menu_key) {
		$menu_array	= array(
				1=>array('menu_key'=>'store_sns_add', 'menu_name'=>Language::get('store_sns_add'), 'menu_url'=>'index.php?gct=store_sns&gp=add'),
				2=>array('menu_key'=>'store_sns_setting', 'menu_name'=>Language::get('store_sns_setting'), 'menu_url'=>'index.php?gct=store_sns&gp=setting'),
