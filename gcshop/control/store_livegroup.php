...
class store_livegroupControl extends BaseSellerControl {
	...
	public function groupbuy_addOp(){
		if(chksubmit()){
			...
			if($res){
				showMessage('添加成功','index.php?gct=store_livegroup&gp=store_livegroup','','succ');
			}else{
	...
	public function edit_livegroupbuyOp(){
		if(chksubmit()){//数据验证
			...
			if($res){
				showMessage('编辑成功','index.php?gct=store_livegroup&gp=store_livegroup','','succ');
			}else{
				showMessage('编辑失败','index.php?gct=store_livegroup&gp=store_livegroup','','error');
			}
		}
	...
	private function profile_menu($menu_key='') {
		$menu_array	= array(
			array('menu_key'=>'store_livegroupbuy','menu_name'=>'线下抢列表','menu_url'=>'index.php?gct=store_livegroup&gp=groupbuy'),
		);
		switch($menu_key){
			case 'store_livegroupbuyadd':
				$menu_array[]=array('menu_key'=>'store_livegroupbuyadd','menu_name'=>'新增线下抢','menu_url'=>'index.php?gct=store_livegroup&gp=groupbuy_add');
				break;
