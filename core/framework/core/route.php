...
class Route {
    ...
    private function _parseRuleRewrite() {
        ...
        if (!empty($path_info) && $path_info != 'index.php' && strpos($path_info, $this->_rewrite_extname)){
            ...
            //gct,gp强制赋值
            $_GET['gct'] = isset($split_array[0]) ? $split_array[0] : 'index';
            $_GET['gp'] = isset($split_array[1]) ? $split_array[1] : 'index';
            unset($split_array[0]);
            unset($split_array[1]);
     ...
     * @return mixed
     */
    private function _gcshopPathInfo($path_info) {
        $reg_match_from = array(
            '/^category$/',
            '/^goodsid-(\d+)$/',
            '/^gcwebshop-(\d+)$/',
            '/^gcwebshop_view-(\d+)-(\d+)-([0-5])-([0-2])-(\d+)$/',
            '/^articlenum-(\d+)$/',
            '/^article_cate-(\d+)$/',
            '/^document-([a-z_]+)$/',
            '/^cateclass-(\d+)-([0-9_]+)-([0-9_]+)-([0-3])-([0-2])-([0-1])-([0-1])-(\d+)-(\d+)$/',
            '/^brandname-(\d+)-([0-3])-([0-2])-([0-1])-([0-1])-(\d+)-(\d+)$/',
            '/^brandname$/',
