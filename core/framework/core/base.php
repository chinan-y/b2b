...
final class Base{
	...
	private static function control(){
		//二级域名
		if ($GLOBALS['setting_config']['enabled_subdomain'] == '1' && $_GET['gct'] == 'index' && $_GET['gp'] == 'index'){
			$store_id = subdomain();
			if ($store_id > 0) $_GET['gct'] = 'show_store';
		}
		$act_file = realpath(BASE_PATH.'/control/'.$_GET['gct'].'.php');
		$class_name = $_GET['gct'].'Control';
		if (!@include($act_file)){
		    if (C('debug')) {
	...
	private static function cp(){
		...
		if (strpos(self::CPURL,'||') !== false){
			...
			return;
			//header('location: https://www.qqbsmall.com');exit();
		}else{
