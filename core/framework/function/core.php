 ...
 * @return string
 */
function makeAuthSeccode($nchash){
    $seccode = random(6, 1);
    $seccodeunits = '';
    ...
    $secauthcode = serialize(array(
        'seccode' => strtoupper($seccode),
        'checktime' => TIMESTAMP,
    ));
    setNcCookie('secauthcode'.$nchash, encrypt(strtoupper($secauthcode)."\t".(time())."\t".$nchash,MD5_KEY),3600);
    return $seccode;
}
 ...
 * @return boolean
 */
function checkSeccodeOld($nchash,$value){
...
function checkSeccode($nchash,$value){
	if(!empty($_SESSION['verify_code_captcha']) && $_SESSION['verify_code_captcha'] == strtoupper($value)){
		$_SESSION['verify_code_captcha_times'] = empty($_SESSION['verify_code_captcha_times']) ? 1 : $_SESSION['verify_code_captcha_times']+1;
		if($_SESSION['verify_code_captcha_times'] >= 2){
			unset($_SESSION['verify_code_captcha_times']);
			unset($_SESSION['verify_code_captcha']);
		}
		return true;
	}else{
		return false;
 ...
 * @return boolean
 */
function checkAuthSeccode($nchash,$value){
    list($checkvalue, $checktime, $checkidhash) = unserialize(decrypt(cookie('seccode'.$nchash),MD5_KEY));
    $return = $checkvalue == strtoupper($value) && $checkidhash == $nchash;
    if (!$return) setNcCookie('secauthcode'.$nchash,'',-3600);
    return $return;
}
...
/**
 * 当访问的gct或gp不存在时调用此函数并退出脚本
 *
 * @param string $gct
 * @param string $gp
 * @return void
 */
function requestNotFound($gct = null, $gp = null) {
 ...
 * @return string 字符串类型的返回结果
 */
function getNchash($gct = '', $gp = ''){
    $gct = $gct ? $gct : $_GET['gct'];
    $gp = $gp ? $gp : $_GET['gp'];
    if (C('captcha_status_login')){
        return substr(md5(SHOP_SITE_URL.$gct.$gp),0,8);
    } else {
        return '';
    }
}
...
function json_return($state,$msg,$data=array()){
	if(!$state || !$msg){
		$arr = array('state'=>501,'msg'=>'缺少必要的参数');
	}
	if($data){
		$arr = array('state'=>$state,'msg'=>$msg,'data'=>$data);
	}else{
 ...
 * @return obj 对象形式的返回结果
 */
function Server($model = null, $base_path = null){
    static $_cache = array();
    $cache_key = $model.'.'.$base_path;
    if (!is_null($model) && isset($_cache[$cache_key])) return $_cache[$cache_key];
    $base_path = $base_path == null ? BASE_DATA_PATH : $base_path;
    $file_name = $base_path.'/server/'.$model.'.server.php';
    $class_name = $model.'Server';
    if (!file_exists($file_name)){
        return $_cache[$cache_key] =  new Model($model);
    }else{
        require_once($file_name);
        if (!class_exists($class_name)){
            $error = 'Server Error:  Class '.$class_name.' is not exists!';
            throw_exception($error);
        }else{
            return $_cache[$cache_key] = new $class_name();
        }
...
function defaultGoodsImage($key){
	...
	if(C(OSS_IS_STORAGE) == 1){
		$file = C('default_goods_image').'?x-oss-process=image/resize,m_pad,w_'.$key;
		return ATTACH_COMMON.DS.$file;
	}
 ...
 * @return string
 */
function getMemberAvatarForID($id,$isanonymous=array()){
	if($isanonymous==1){
 ...
 * @return string
 */
function getMemberQcodeForID($id){
	
	return 'http://qr.liantu.com/api.php?w=300&text='.WAP_SITE_URL.'/index.html?ref='.$id;
}
 ...
 * @return string
 */
function getMemberWXQcodeForID($id){
	
	return 'http://qr.liantu.com/api.php?w=300&text='.WXSHOP_SITE_URL.'/index.html?ref='.$id;
}
...
function getCMSArticleUrl($article_id) {
    if(URL_MODEL) {
        ...
        return CMS_SITE_URL.DS.'index.php?gct=article&gp=article_detail&article_id='.$article_id;
    }
}
...
function getCMSPictureUrl($picture_id) {
    if(URL_MODEL) {
        ...
        return CMS_SITE_URL.DS.'index.php?gct=picture&gp=picture_detail&picture_id='.$picture_id;
    }
}
...
function getCMSArticleImageUrl($image_path, $image_name, $type='list') {
    if(empty($image_name)) {
		...
		if(C(OSS_IS_STORAGE) == 1){
			$file_path = ATTACH_CMS.DS.'article'.DS.$image_path.DS.$image_name;
			if(1) {
				$image_name = UPLOAD_SITE_URL.DS.$file_path;
			} else {
				$image_name = UPLOAD_SITE_URL.DS.ATTACH_CMS.DS.'no_cover.png';
...
function getCMSSpecialUrl($special_id) {
    return CMS_SITE_URL.DS.'index.php?gct=special&gp=special_detail&special_id='.$special_id;
}
...
function getShopSpecialUrl($special_id) {
    return SHOP_SITE_URL.DS.'index.php?gct=special&gp=special_detail&special_id='.$special_id;
}
    ...
    if(!empty($personal_image_list)){
        foreach ($personal_image_list as $value) {
            if(!empty($type) && in_array($type,$ext_array)) {
				if(C(OSS_IS_STORAGE) == 0){$file_name = str_replace('.', '_'.$type.'.', $value);}
				if(C(OSS_IS_STORAGE) == 1){$file_name = $value.'?x-oss-process=image/resize,m_pad,w_150';}
            } else {
                $file_name = $value;
            }
...
function getWaybillImageUrl($image_name = '') {
	...
	if(C(OSS_IS_STORAGE) == 1){
		if(1){//if(1)判断OSS-object是否存在，ing……
			return UPLOAD_SITE_URL . $image_path;
		} else {
			return UPLOAD_SITE_URL.'/'.defaultGoodsImage('240');
		}
 ...
 * @return boolean
 */
function chksubmitOld($check_token = false, $check_captcha = false, $return_type = 'alert'){
	...
	if ($check_captcha){
		if (!checkSeccodeOld($_POST['nchash'],$_POST['captcha'])){
...
function write_file($filepath, $data, $mode = null)
{
    ...
    $data = "<?php defined('GcWebShop') or exit('Access Invalid!'); return ".$data.";";
...
function themeImagePath($param){
	...
	if(C(OSS_IS_STORAGE) == 1){
		return DIR_UPLOAD.'/'.ATTACH_CIRCLE.'/theme/'.$param;
	}
}
/**
 * 话题图片url
...
function circleLogo($id){
	...
	if(C(OSS_IS_STORAGE) == 1){
		if(1){
			return UPLOAD_SITE_URL.'/'.ATTACH_CIRCLE.'/group/'.$id.'.jpg?x-oss-process=image/resize,m_pad,w_80';
		}else{
			return UPLOAD_SITE_URL.'/'.ATTACH_CIRCLE.'/default_group_logo.gif';
		}
	}
 ...
 *
 * 若指向网站首页，可以传空:
 * url() => 表示gct和gp均为index，返回当前站点网址
 *
 * url('search,'index','array('cate_id'=>2)); 实际指向 index.php?gct=search&gp=index&cate_id=2
 * 传递数组参数时，若gct（或gp）值为index,则可以省略
 * 上面示例等同于
 * url('search','',array('gct'=>'search','cate_id'=>2));
 *
 * @param string $gct control文件名
 * @param string $gp gp方法名
 * @param array $args URL其它参数
 * @param boolean $model 默认取当前系统配置
 * @param string $site_url 生成链接的网址，默认取当前网址
 * @return string
 */
function url($gct = '', $gp = '', $args = array(), $model = false, $site_url = ''){
    //伪静态文件扩展名
    $ext = '.html';
    //入口文件名
    $file = 'index.php';
    ...
    //定义变量存放返回url
    $url_string = '';
    if (empty($gct) && empty($gp) && empty($args)) {
        return $site_url;
    }
    $gct = !empty($gct) ? $gct : 'index';
    $gp = !empty($gp) ? $gp : 'index';
    ...
    $reg_match_to = array(
        ...
        'goodsid-\\1.html',
        'gcwebshop-\\1.html',
        'gcwebshop_view-\\1-\\2-\\3-\\4-\\5.html',
        'articlenum-\\1.html',
        'article_cate-\\1.html',
        'document-\\1.html',
        'cateclass-\\1-\\2-\\3-\\4-\\5-\\6-\\7-\\8-\\9.html',
        'brandname-\\1-\\2-\\3-\\4-\\5-\\6-\\7.html',
        'brandname.html',
 ...
 * @return string
 */
function urlShop($gct = '', $gp = '', $args = array(), $store_domain = ''){

    // 如何使自营店则返回javascript:;

//    if ($gct == 'show_store' && $gp != 'goods_all') {
...
//    }
    ...
    if(URL_MODEL && in_array($gct.':'.$gp, $rewrite_item)) {
        $rewrite_flag = true;
    ...
    return url($gct, $gp, $args, $rewrite_flag, SHOP_SITE_URL);
}
 ...
 *
 * @param string $gct control文件名
 * @param string $gp op方法名
 * @param array $args URL其它参数
 * @return string
 */
function urlAdmin($gct = '', $gp = '', $args = array()){
    return url($gct, $gp, $args, false, ADMIN_SITE_URL);
}
/**
 ...
 * @return string
 */
function urlCMS($gct = '', $gp = '', $args = array()){
    return url($gct, $gp, $args, false, CMS_SITE_URL);
}
/**
 ...
 * @return string
 */
function urlCircle($gct = '', $gp = '', $args = array()){
    return url($gct, $gp, $args, false, CIRCLE_SITE_URL);
}
/**
 ...
 * @return string
 */
function urlMicroshop($gct = '', $gp = '', $args = array()){
    return url($gct, $gp, $args, false, MICROSHOP_SITE_URL);
}
/**
 * 验证是否为平台店铺
 *
