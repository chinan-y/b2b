	...
	public function linkOp(){
		...
		if (is_array($link_list)){
			foreach ($link_list as $k => $v){
				if (!empty($v['link_pic'])){
					$link_list[$k]['link_pic'] = UPLOAD_SITE_URL.DS.ATTACH_PATH.DS.'common'.DS.$v['link_pic'];
				}
			}
		}
		...
		if (intval($_GET['link_id']) > 0){
			...
			if (!empty($tmp['link_pic'])){
				...
				if(C(OSS_IS_STORAGE) == 1){
					...
					$objectname = DIR_UPLOAD.DS.ATTACH_COMMON.DS.$tmp['link_pic'];
					$result = $oss->deleteObject($bucketname,$objectname);
				}
			}
			$model_link->del($tmp['link_id']);
			//H('link',null);;
			showMessage($lang['link_index_del_succ'],'index.php?gct=link&gp=link');
		}else {
			showMessage($lang['link_index_choose_del'],'index.php?gct=link&gp=link');
		}
	}
			...
			if ($error != ''){
				...
				if ($_FILES['link_pic']['name'] != ''){
					...
					if(C(OSS_IS_STORAGE) == 1){
						...
						$extend = pathinfo($_FILES['link_pic']['name']); 
						$extend = strtolower($extend["extension"]); 
						$newname	= 'lk_' . date('YmdHis') . rand(0,9999) . '.' . $extend;
						$objectname = DIR_UPLOAD.DS.ATTACH_COMMON.DS.$newname;
						$pathname 	= $_FILES['link_pic']['tmp_name'];
						if($_FILES['link_pic']['type'] =='image/gif' || $_FILES['link_pic']['type'] =='image/jpeg' || $_FILES['link_pic']['type'] =='image/png'){
							$result = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
							if ($result){
								$_POST['link_pic'] = $newname;
							}else {
								showMessage('上传错误');
							}
						}
				...
				if ($result){
					...
					$url = array(
						array(
							'url'=>'index.php?gct=link&gp=link_add',
							'msg'=>$lang['link_add_again'],
						),
						array(
							'url'=>'index.php?gct=link&gp=link',
		...
		if ($_POST['form_submit'] == 'ok'){
			...
			if ($error != ''){
				...
				if ($_FILES['link_pic']['name'] != ''){
					if(C(OSS_IS_STORAGE) == 0){
						$upload = new UploadFile();
						$upload->set('default_dir',ATTACH_PATH.DS.'common');
					...
					if(C(OSS_IS_STORAGE) == 1){
						...
						$extend = strtolower($extend["extension"]); 
						$newname	= 'lk_' . date('YmdHis') . rand(0,9999) . '.' . $extend;
						$objectname = DIR_UPLOAD.DS.ATTACH_PATH.DS.'common'.DS.$newname;
						$pathname 	= $_FILES['link_pic']['tmp_name'];
						if($_FILES['link_pic']['type'] =='image/gif' || $_FILES['link_pic']['type'] =='image/jpeg' || $_FILES['link_pic']['type'] =='image/png'){
							$result = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
							if ($result){
				...
				if ($result){
					...
					$url = array(
						array(
							'url'=>'index.php?gct=link&gp=link_edit&link_id='.intval($_POST['link_id']),
