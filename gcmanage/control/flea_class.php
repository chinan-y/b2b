...
class flea_classControl extends SystemControl{
	...
	public function brach_edit_saveOp(){
		$lang	= Language::getLangContent();
		if ($_POST['gc_show'] == '-1'){
			showMessage($lang['goods_class_batch_edit_succ'],'index.php?gct=flea_class&gp=flea_class');
		}
		if ($_POST['form_submit'] == 'ok'){
			$model_class = Model('flea_class');
		...
		if ($_POST['form_submit'] == 'ok'){
			...
			if ($error != ''){
				...
				if ($result){
					$url = array(
						array(
							'url'=>'index.php?gct=flea_class&gp=goods_class_add&gc_parent_id='.$_POST['gc_parent_id'],
							'msg'=>$lang['goods_class_add_again'],
						),
						array(
							'url'=>'index.php?gct=flea_class&gp=flea_class',
		...
		if ($_POST['form_submit'] == 'ok'){
			...
			if ($error != ''){
				...
				if ($result){
					$url = array(
						array(
							'url'=>'index.php?gct=flea_class&gp=goods_class_edit&gc_id='.$_POST['gc_id'],
	...
	public function goods_class_importOp(){
		...
		if ($_POST['form_submit'] == 'ok'){
			...
			if (!empty($_FILES['csv']) && !empty($_FILES['csv']['name']) && $file_type == 'csv'){
				 ...
				 * 重新生成缓存
				 */
				showMessage($lang['goods_class_import_succ'],'index.php?gct=flea_class&gp=flea_class');
			}else {
	...
	public function goods_class_delOp(){
		...
		if (intval($_GET['gc_id']) > 0){
			...
			showMessage($lang['goods_class_index_del_succ'],'index.php?gct=flea_class&gp=flea_class');
		}else {
			showMessage($lang['goods_class_index_choose_del'],'index.php?gct=flea_class&gp=flea_class');
		}
	}
	/**
	 * ajax操作
