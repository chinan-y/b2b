...
class articleControl extends SystemControl{
					...
					if (is_array($upload_list)){
						foreach ($upload_list as $k_upload => $v_upload){
							...
							if(C(OSS_IS_STORAGE) ==1){
								$oss =Logic('oss');
								$oss ->deleteObject(IMAGE_BUCKET_NAME, DIR_UPLOAD.DS.ATTACH_ARTICLE.DS.$v_upload['file_name']);				
							}
						}
					}
					$model_article->del($v);
		...
		if (chksubmit()){
			...
			if ($error != ''){
				...
				if ($result){
					...
					$url = array(
						array(
							'url'=>'index.php?gct=article&gp=article',
							'msg'=>"{$lang['article_add_tolist']}",
						),
						array(
							'url'=>'index.php?gct=article&gp=article_add&ac_id='.intval($_POST['ac_id']),
			...
			if ($error != ''){
				...
				if ($result){
					...
					$url = array(
						...
						array(
							'url'=>'index.php?gct=article&gp=article_edit&article_id='.intval($_POST['article_id']),
	...
	public function article_pic_uploadOp(){
		...
		if(C(OSS_IS_STORAGE) == 1){
			...
			$imgname	= 'a_'.date('YmdHis').rand(0,99999);
		...
		switch ($_GET['branch']){
			...
			case 'del_file_upload':
				if (intval($_GET['file_id']) > 0){
					...
					if(C(OSS_IS_STORAGE) == 1){
						$oss =Logic('oss');
						$oss ->deleteObject(IMAGE_BUCKET_NAME, DIR_UPLOAD.DS.ATTACH_ARTICLE.DS.$file_array['file_name']);
					}
					/**
					 * 删除信息
					 */
