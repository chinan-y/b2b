...
class cms_indexControl extends SystemControl{
    ...
    public function save_page_moduleOp() {
        ...
        if($result) {
            ...
            showMessage(Language::get('nc_common_save_succ'), 'index.php?gct=cms_index&gp=cms_index');
        } else {
    ...
    public function image_uploadOp() {
        ...
        if(!empty($_FILES['image_upload']['name'])) {
			...
			if(C(OSS_IS_STORAGE) == 1){
				...
				$extend 	= pathinfo($_FILES['image_upload']['name']); 
				$extend 	= strtolower($extend["extension"]); 
				$imgname	= 'gs_'.date('His').rand(0,99);
				$imgfullname= $imgname . '.' . $extend;
				$objectname = DIR_UPLOAD.DS.ATTACH_CMS.DS.'index'.DS.$imgfullname;
				$pathname 	= $_FILES['image_upload']['tmp_name'];
				$result = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
				if(!$result) {
					$data['status'] = 'fail';
					$data['error'] = 'OSS CMS ERROR!';
				}
				$data['file_name'] = $imgfullname;
				$data['file_url'] = getCMSIndexImageUrl($imgfullname);
			}
    ...
    private function show_menu($menu_key) {
        $menu_array = array(
            'cms_index'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_manage'),'menu_url'=>'index.php?gct=cms_index&gp=cms_index'),
        );
