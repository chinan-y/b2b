...
class voucherControl extends SystemControl{
	...
	private $uservoucher_arr;

	public function __construct(){
		parent::__construct();
		Language::read('voucher');
		if (C('voucher_allow') != 1 || C('points_isuse')!=1){
			showMessage(Language::get('admin_voucher_unavailable'),'index.php?gct=operation&gp=point','html','succ',1,4000);
		}
		...
		//用户代金券状态
		$this->uservoucher_arr = array('unuse'=>array(1,'未用'),'used'=>array(2,'已用'),'guoqi'=>array(3,'过期'),'shouhui'=>array(4,'收回'));
		...
		Tpl::output('uservoucher_arr',$this->uservoucher_arr);
	}
		...
		if (chksubmit()){
	        ...
	        if ($error != ''){
	            ...
	            if ($rs){
	            	...
	            	showMessage(Language::get('nc_common_save_succ'),'index.php?gct=voucher&gp=pricelist');
	            }else {
	            	showMessage(Language::get('nc_common_save_fail'),'index.php?gct=voucher&gp=priceadd');
	            }
	        }
		}else {
	 ...
	 * 添加代金券指定派发页面
	 */
	public function assignedOp(){

		//获得代金券金额列表
		$voucher_t_state = 1;
		$model1 = Model('voucher_template');
		$voucher_template = $model1->order('voucher_t_id asc')->where(array('voucher_t_state' => $voucher_t_state))->limit(100, 1000)->select();

		$model2 = Model('member');
		//$member = $model2->order('member_id asc')->limit(100000,100000)->select();


		if (chksubmit()) {
			if(empty($_POST['voucher_t_id']) || empty($_POST['member_id'])){
				showDialog('会员ID和模版ID不能为空');exit;
			}
			//查询会员信息
			//$member_info = Model('member')->getMemberInfoByID($_POST['member_id']);
			//$member_name = $member_info['member_name'];
			//查询代金券模板信息
			$voucher_t_id = $_POST['voucher_t_id'];
			$voucherinfo = Model('voucher_template')->where(array('voucher_t_id' => $voucher_t_id))->find();

			$i = 0;
			$_POST['member_id'] = str_replace('，', ',', $_POST['member_id']);
			foreach (explode(',', trim($_POST['member_id'])) as $v) {
				$v = intval(trim($v));
				if (empty($v)) {
					continue;
				}
				$member_info = Model('member')->getMemberInfoByID($v);
				...
				$insert_arr['voucher_title'] = $voucherinfo['voucher_t_title'];
				$insert_arr['voucher_desc'] = $voucherinfo['voucher_t_desc'];
				$insert_arr['voucher_start_date'] = time();
				$insert_arr['voucher_end_date'] = $voucherinfo['voucher_t_end_date'];
				$insert_arr['voucher_price'] = $voucherinfo['voucher_t_price'];
				$insert_arr['voucher_limit'] = $voucherinfo['voucher_t_limit'];
				$insert_arr['voucher_store_id'] = $voucherinfo['voucher_t_store_id'];
				$insert_arr['voucher_design_sku'] = $voucherinfo['voucher_t_sku'];//增加代金券券指定商品sku
				$insert_arr['voucher_state'] = $voucherinfo['voucher_t_state'];
				...
				if($result && $member_info['member_mobile'] && ($_POST['check_msg']=='checked') && $_POST['message']) {
					$sms = new Sms();
					$sms->send($member_info['member_mobile'],$_POST['message']);
				}				
			}

			if ($i >= 1) {
				$updategiveout = $model->table('voucher_template')->where(array('voucher_t_id' => $voucher_t_id))->update(array('voucher_t_giveout' => array('exp', 'voucher_t_giveout+'.$i)));
				$this->log('派发代金券[ '.$voucherinfo['voucher_t_title'].' ]给'.$member_info['member_name']);
				showDialog('派发代金券成功');exit;
			} else {
				showDialog('派发代金券失败');exit;
			}
		...
		$this->show_menu('voucher','assigned');
		Tpl::showpage('voucher.assigned');
     ...
     * 平台通用代金券设置
     */
    public function platvouchersetOp(){
		//添加平台通用代金券信息
		if($_POST['voucher_t_title'] && $_POST['voucher_t_price'] && $_POST['voucher_t_eachlimit']){
			$insert = array();
			$insert['voucher_t_title'] = $_POST['voucher_t_title'];//代金券模板标题
			$insert['voucher_t_desc'] = $_POST['voucher_t_desc'];//代金券模板描述
			$insert['voucher_t_start_date'] =time();
			$insert['voucher_t_end_date'] =$_POST['voucher_t_end_date']*3600*24+time();//有效截止日期
			$insert['voucher_t_price'] = $_POST['voucher_t_price'];//模板面额
			$insert['voucher_t_total'] = $_POST['voucher_t_total'];//可发放的代金券总数
			$insert['voucher_t_limit'] = $_POST['voucher_t_limit'];//代金券使用时的订单限额
			$insert['voucher_t_store_id'] = 0;
			$insert['voucher_t_storename'] = '光彩全球网通用';
			$insert['voucher_t_sc_id'] = 0;//所属店铺分类ID
			//$insert['voucher_t_creator_id']=$_SESSION['admin_id'];//取得当前管理员或者创建者ID？？
			$insert['voucher_t_state'] = 1;//默认为1,代金卷模版有效,2为失效
			$insert['voucher_t_add_date']=TIMESTAMP;
			$insert['voucher_t_points'] = $_POST['voucher_t_points'];//兑换积分voucher_t_eachlimit
			$insert['voucher_t_eachlimit'] = $_POST['voucher_t_eachlimit'];//每人限领张数
			$insert['voucher_t_recommend'] = 1;//0不推荐，1推荐
			$insert['voucher_t_sku'] = $_POST['voucher_t_sku'];//代金券指定使用的商品
			//var_dump($insert);

			$model = Model();
				
			$result = $model->table('voucher_template')->insert($insert);
			if($result){showMessage('添加平台通用代金券成功！','index.php?gct=voucher&gp=platvoucherset');}
			else{showMessage('添加平台通用代金券失败！','index.php?gct=voucher&gp=platvoucherset');}
			}



    	$this->show_menu('voucher','platvoucherset');
	    Tpl::output('setting',$setting);
	    Tpl::showpage('voucher.platvoucherset');
	 ...
	 * 推送至用户的代金券列表
	 */
	public function assignedtemplatelistOp(){
		//获得代金券金额列表
		$model 	= Model();
		$param = array();
        if(trim($_GET['store_name'])){
        	$param['store.store_name'] = array('like',"%{$_GET['store_name']}%");
        }
		if(trim($_GET['voucher_number'])){
        	$param['voucher_code'] = array('like',"%{$_GET['voucher_number']}%");
        }
		if(trim($_GET['member_name'])){
        	$param['voucher_owner_name'] = array('like',"%{$_GET['member_name']}%");
        }
    	if(trim($_GET['sdate']) && trim($_GET['edate'])){
    		$sdate = strtotime($_GET['sdate']);
    		$edate = strtotime($_GET['edate']);
        	$param['voucher_active_date'] = array('between',"$sdate,$edate");
        }elseif (trim($_GET['sdate'])){
        	$sdate = strtotime($_GET['sdate']);
        	$param['voucher_active_date'] = array('egt',$sdate);
        }elseif (trim($_GET['edate'])){
        	$edate = strtotime($_GET['edate']);
        	$param['voucher_active_date'] = array('elt',$edate);
        }
        $state = intval($_GET['state']);
    	if($state){
        	$param['voucher_state'] = $state;
        }

		$field	='voucher.*,store.store_id,store.store_name,goods.goods_id,goods.goods_serial,order.order_id,order.order_sn';
		$on		='voucher.voucher_store_id=store.store_id,voucher.voucher_design_sku=goods.goods_id,voucher.voucher_order_id=order.order_id';
		$order	='voucher.voucher_id desc';
		$voucher_list 	= $model->table('voucher,store,goods,order')->where($param)-> field($field)->join('left,left,left')->on($on)->order($order)->page(10)->select();
		//var_dump($voucher_list);
        if (empty($voucher_list)) {
           showMessage('没有符合条件的代金券！','index.php?gct=voucher&gp=assignedtemplatelist');
        }
		...
		*/
		...
		if($_GET['tid']){
			$update_arr = array();
	    	$update_arr['voucher_state'] = 4;
			$where_arr	=array();
			$where_arr['voucher_id']	=$_GET['tid'];
			$where_arr['voucher_state']	=1;	
			$tbackvou	=  $model->table('voucher')->where($where_arr)->update($update_arr);
			if($tbackvou){
				showMessage('代金券收回成功！','index.php?gct=voucher&gp=assignedtemplatelist');
			}
		}
		...
		Tpl::output('show_page',$model->showpage(2));
		$this->show_menu('voucher','assignedtemplatelist');
		Tpl::showpage('voucher.assignedtemplatelist');
	}
    ...
    public function priceeditOp(){
    	...
    	if ($id <= 0){
    		showMessage(Language::get('param_error'),'index.php?gct=voucher&gp=pricelist');
    	}
    	$model = Model();
    	if (chksubmit()){
    		$obj_validate = new Validate();
			...
			if ($error != ''){
	    		...
	    		if ($rs){
	    			...
	    			showMessage(Language::get('nc_common_save_fail'),'index.php?gct=voucher&gp=pricelist');
	    		}
			}
    	}else {
    ...
    public function pricedropOp(){
        ...
        if ($rs){
        	...
        	showMessage(Language::get('nc_common_del_succ'),'index.php?gct=voucher&gp=pricelist');
        }else{
        	showMessage(Language::get('nc_common_del_fail'),'index.php?gct=voucher&gp=pricelist');
        }
    }
    ...
    public function templateeditOp(){
    	...
    	if ($t_id <= 0){
    		showMessage(Language::get('param_error'),'index.php?gct=voucher&gp=templatelist','','error');
    	}
        $model = Model('voucher');
        //查询模板信息
        $param = array();
        ...
        if (empty($t_info)){
        	showMessage(Language::get('param_error'),'index.php?gct=voucher&gp=templatelist','html','error');
        }
        if(chksubmit()){
        	...
        	if ($points < 0){
	       	...
	       	$update_arr['voucher_t_title'] = $_POST['vtitle'];
	       	$update_arr['voucher_t_end_date'] = strtotime($_POST['end_date']);
	       	$update_arr['voucher_t_price'] = $_POST['vprice'];
	       	$update_arr['voucher_t_total'] = $_POST['vtotal'];
	       	$update_arr['voucher_t_limit'] = $_POST['vlimit'];
	       	$update_arr['voucher_t_eachlimit'] = $_POST['eachlimit'];
	       	$update_arr['voucher_t_desc'] = $_POST['vdesc'];
	       	$update_arr['voucher_t_sku'] = $_POST['vsku'];
	       	...
	       	if($rs){
	       		...
	       		showMessage(Language::get('nc_common_save_succ'),'index.php?gct=voucher&gp=templatelist','succ');
	       	}else{
	       		showMessage(Language::get('nc_common_save_fail'),'index.php?gct=voucher&gp=templatelist','error');
	       	}
        }else{
            //查询店铺分类
    ...
    private function show_menu($menu_type,$menu_key='') {
		...
		switch ($menu_type) {
			case 'voucher':
				$menu_array = array(
				1=>array('menu_key'=>'platvoucherset','menu_name'=>'平台代金券设置','menu_url'=>'index.php?gct=voucher&gp=platvoucherset'),
				2=>array('menu_key'=>'quotalist','menu_name'=>Language::get('admin_voucher_quota_manage'), 'menu_url'=>'index.php?gct=voucher&gp=quotalist'),
				3=>array('menu_key'=>'pricelist','menu_name'=>Language::get('admin_voucher_pricemanage'), 'menu_url'=>'index.php?gct=voucher&gp=pricelist'),
				4=>array('menu_key'=>'priceadd','menu_name'=>Language::get('admin_voucher_priceadd'), 'menu_url'=>'index.php?gct=voucher&gp=priceadd'),
				5=>array('menu_key'=>'setting','menu_name'=>Language::get('admin_voucher_setting'),	'menu_url'=>'index.php?gct=voucher&gp=setting'),
				7=>array('menu_key'=>'assigned','menu_name'=>'派发代金券','menu_url'=>'index.php?gct=voucher&gp=assigned'),
				6=>array('menu_key'=>'templatelist','menu_name'=>Language::get('admin_voucher_template_manage'), 'menu_url'=>'index.php?gct=voucher&gp=templatelist'),
				8=>array('menu_key'=>'assignedtemplatelist','menu_name'=>'用户代金券','menu_url'=>'index.php?gct=voucher&gp=assignedtemplatelist')
				);
				break;
			case 'priceedit':
				$menu_array = array(
				1=>array('menu_key'=>'setting','menu_name'=>Language::get('admin_voucher_setting'),	'menu_url'=>'index.php?gct=voucher&gp=setting'),
				2=>array('menu_key'=>'pricelist','menu_name'=>Language::get('admin_voucher_pricemanage'), 'menu_url'=>'index.php?gct=voucher&gp=pricelist'),
				3=>array('menu_key'=>'priceedit','menu_name'=>Language::get('admin_voucher_priceedit'), 'menu_url'=>'')
				);
				break;
			case 'templateedit':
				$menu_array = array(
				1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('admin_voucher_template_manage'), 'menu_url'=>'index.php?gct=voucher&gp=templatelist'),
