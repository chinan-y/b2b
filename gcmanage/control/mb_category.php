...
class mb_categoryControl extends SystemControl{
	...
	public function mb_category_delOp(){
		...
		if (intval($_GET['gc_id']) > 0){
			...
			showMessage($lang['link_index_del_succ'],'index.php?gct=mb_category&gp=mb_category_list');
		}else {
			showMessage($lang['link_index_choose_del'],'index.php?gct=mb_category&gp=mb_category_list');
		}
	}
		...
		if ($_POST['form_submit'] == 'ok'){
			...
			if ($_FILES['link_pic']['name'] != ''){
				...
				if(C(OSS_IS_STORAGE) == 1){
					...
					$imgname	= 'mc_'.date('His').rand(0,99);
					$imgfullname= $imgname . '.' . $extend;
					$objectname = DIR_UPLOAD.DS.ATTACH_MOBILE.DS.'category'.DS.$imgfullname;
					$pathname 	= $_FILES['link_pic']['tmp_name'];
					$result = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
					if ($result){
						$_POST['link_pic'] = $imgfullname;
					}else{
						showMessage('Category upload error!', '', '', 'error');
					}
				}
			}
			...
			if ($result){
				$url = array(
					array(
						'url'=>'index.php?gct=mb_category&gp=mb_category_add',
						'msg'=>$lang['link_add_again'],
					),
					array(
						'url'=>'index.php?gct=mb_category&gp=mb_category_list',
	...
	public function mb_category_editOp(){
		...
		if ($_POST['form_submit'] == 'ok'){
			...
			if ($result){
				...
				$url = array(
					array(
						'url'=>'index.php?gct=mb_category&gp=mb_category_edit&gc_id='.intval($_POST['gc_id']),
