...
WHEN 'wxpay' THEN '微信支付'
WHEN 'wxapppay' THEN '微信APP支付'
WHEN 'wxapp_ys' THEN '原生APP微信支付'
WHEN 'wxminipay' THEN '微信小程序支付'
WHEN 'ccbpay' THEN '建行支付'
WHEN 'tonglian' THEN '通联支付'
WHEN 'czypay' THEN '彩之云支付'
WHEN 'gzbank' THEN '贵州银行支付'
ELSE '其他支付方式'
END 支付方式,
...
from 33hao_order o LEFT JOIN 33hao_order_common oc ON oc.order_id=o.order_id
...
LEFT JOIN 33hao_partner pa ON o.partner_id=pa.partner_id 
...
LEFT JOIN 33hao_area a3 ON a3.area_id = reciver_area_id
				...
				$statlist['headertitle'] = array('订单时间','店铺名称','订单编号','外部订单号','订单返利方式','订单是否已返利','买家','消费奖励','U1用户','U1奖励','U2奖励','U3奖励','平台合作方','平台提成比例','平台合作方推广提成','区域合作方','授权区域','区域提成比例','区域合作方推广提成','订单总金额（已减代金券）','订单税金','充值卡支付金额','预付款支付金额','运费','订单状态','是否退款','订单收货时间','Pay-支付单号','Tra-交易号','支付方式','付款时间','买家支付金额','DJQ-代金券编码','代金券面额','物流公司及单号','发货时间','收货省','收货市','收货区域','收货人');

				foreach((array)$order_list as $k => $val){
				...
				foreach ((array)$statlist['data'] as $k=>$v){
					...
					$excel_data[$k+1][] = array('data'=>$v['U1用户']);
					$excel_data[$k+1][] = array('data'=>$v['U1奖励']);
					$excel_data[$k+1][] = array('data'=>$v['U2奖励']);
					$excel_data[$k+1][] = array('data'=>$v['U3奖励']);
			...
			}else if ($_POST['action']=='order_list_goods') {
				$start_time = $_POST['start_time'];
				$end_time = $_POST['end_time'];
				if(!$start_time || !$end_time){
					exit("请务必输入起止时间");
...
og.taxes_total 商品税金,og.goods_pay_price 商品成交金额,g.goods_storage 商品最新库存,su.supplier_name 供货商,
...
LEFT JOIN 33hao_region re ON re.id=tr.region_id
				...
				$excel_data = $excel_obj->charset($excel_data,CHARSET);
				$excel_obj->addArray($excel_data);
				$excel_obj->addWorksheet($excel_obj->charset('订单商品流水表',CHARSET));
				$excel_obj->generateXML($excel_obj->charset('订单商品流水表',CHARSET).$start_time.'至'.$end_time);
				exit();
...
m.member_id 会员ID,
m.member_name 会员名,
m.member_truename 姓名,
m.member_mobile 手机号码,
FROM_UNIXTIME(m.member_time,'%Y-%m-%d %H:%i:%S') 注册时间, 
m1.member_id U1会员ID,
m1.member_name U1会员名,
m2.member_id U2会员ID,
m2.member_name U2会员名,
m3.member_id U3会员ID,
m3.member_name U3会员名
FROM 33hao_member m 
left join 33hao_member m1 on m.refer_id= m1.member_id
left JOIN 33hao_member m2 on m1.refer_id = m2.member_id
left JOIN 33hao_member m3 on m2.refer_id = m3.member_id
					WHERE m.member_time > UNIX_TIMESTAMP('".$start_time." 00:00:00')
					AND m.member_time < UNIX_TIMESTAMP('".$end_time." 23:59:59')");
					
				$statlist['headertitle'] = array('会员ID','会员名','姓名','手机号码','注册时间','U1会员ID','U1会员名','U2会员ID','U2会员名','U3会员ID','U3会员名');
				...
				//头部列标题
				foreach ((array)$statlist['headertitle'] as $v){
					$excel_data[0][] = array('styleid'=>'s_title','data'=>$v);
				}
				$num = 0;	
				foreach ($statlist['data'] as $k=>$v){
					$excel_data[$k+1][] = array('data'=>$v['会员ID']);
					$excel_data[$k+1][] = array('data'=>$v['会员名']);
					$excel_data[$k+1][] = array('data'=>$v['姓名']);
					$excel_data[$k+1][] = array('data'=>$v['手机号码']);
					$excel_data[$k+1][] = array('data'=>$v['注册时间']);
					$excel_data[$k+1][] = array('data'=>$v['U1会员ID']);
					$excel_data[$k+1][] = array('data'=>$v['U1会员名']);
					$excel_data[$k+1][] = array('data'=>$v['U2会员ID']);
					$excel_data[$k+1][] = array('data'=>$v['U2会员名']);
					$excel_data[$k+1][] = array('data'=>$v['U3会员ID']);
					$excel_data[$k+1][] = array('data'=>$v['U4会员名']);
					$num++ ;
				}
				...
				$goods_pricing = DB::getAll("SELECT 
					...
					case a.store_id 
						...
						WHEN '18' THEN '光彩全球旅拍'
						WHEN '19' THEN '光彩新鲜馆'
						WHEN '20' THEN '重庆西永2号仓'
						WHEN '21' THEN '光彩全球B2BWZ综合仓'
						WHEN '22' THEN '光彩全球综合仓'
					ELSE '重庆保税仓'
					END 发货仓,
				...
				$statlist['headertitle'] = array('商品货号','商品名称','商品价格','商品库存','库存预警值','仓库名称','商品分类','有效天数','上架时间','商品来源','包装单位','原产国','商品链接');
				...
				foreach ($statlist['data'] as $k=>$v){
					...
					$excel_data[$k+1][] = array('data'=>'https://www.qqbsmall.com/gcshop/index.php?gct=goods&gp=index&goods_id='.$v['商品链接']);
				}
				$excel_data = $excel_obj->charset($excel_data,CHARSET);
				$excel_obj->addArray($excel_data);
				...
				if(!$reciver_name){
					...
					WHERE b.reciver_info LIKE '%".$reciver_address."%'");
				}
				if(!$reciver_address){
					$specified = DB::getAll("SELECT 
					FROM_UNIXTIME(a.add_time,'%Y-%m-%d %H:%i:%S') 订单时间,
					...
					LEFT JOIN 33hao_order_goods c ON a.order_id = c.order_id 
					LEFT JOIN 33hao_goods d ON d.goods_id = c.goods_id 
					WHERE b.reciver_name LIKE '%".$reciver_name."%'");
				}
				if($reciver_address && $reciver_name){
				$specified = DB::getAll("SELECT 
					FROM_UNIXTIME(a.add_time,'%Y-%m-%d %H:%i:%S') 订单时间,
					a.order_sn 订单编号,
					b.reciver_name 收货人,
					...
					LEFT JOIN 33hao_order_goods c ON a.order_id = c.order_id 
					LEFT JOIN 33hao_goods d ON d.goods_id = c.goods_id 
					WHERE b.reciver_name LIKE '%".$reciver_name."%' and b.reciver_info LIKE '%".$reciver_address."%'");
				}

				
				$statlist['headertitle'] = array('订单时间','订单编号','收货人','收货地址','联系电话','商品名称','商品货号','商品数量','商品价格');
				
				foreach((array)$specified as $k =>$val){
