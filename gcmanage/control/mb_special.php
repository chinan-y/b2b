...
class mb_specialControl extends SystemControl{
    ...
    public function special_item_addOp() {
        ...
        //广告只能添加一个
        if($param['item_type'] == 'adv_list' || $param['item_type'] == 'adv_list1') {
    ...
    public function special_image_uploadOp() {
        ...
        if(!empty($_FILES['special_image']['name'])) {
			...
			if(C('OSS_IS_STORAGE') == 1){
				...
				$extend = pathinfo($_FILES['special_image']['name']); 
				$extend = strtolower($extend["extension"]); 
				$newname	= 's' . $_POST['special_id'] . '_' . date('YmdHis') . rand(0,9999) . '.' . $extend;
				$objectname = DIR_UPLOAD.DS.ATTACH_MOBILE.DS. 'special' . DS . 's' . $_POST['special_id'] . DS .$newname;
				$pathname 	= $_FILES['special_image']['tmp_name'];
				if($_FILES['special_image']['type'] =='image/gif' || $_FILES['special_image']['type'] =='image/jpeg' || $_FILES['special_image']['type'] =='image/png'){
    ...
    public function goods_listOp() {
        if(isset($_REQUEST['keyword'])){
            ...
            $_SERVER['REQUEST_URI'] .= '&keyword=' . $keyword;
        }
        ...
        if($_REQUEST['item_id']){
            ...
            $_SERVER['REQUEST_URI'] .= '&item_id=' . $item_id;
            $model_mb_special = Model('mb_special');
            $item_info = $model_mb_special->getMbSpecialItemInfoByID($item_id);
            $goods_ids = array();
            if(isset($item_info['item_data']['item'])){
                foreach($item_info['item_data']['item'] as $k=>$v){
        ...
        $goods_list = $model_goods->getGoodsOnlineList($condition, 'goods_id,goods_commonid,goods_name,goods_price,goods_rebate_rate,goods_promotion_price,goods_promotion_type,goods_marketprice,goods_image', 10);
    ...
    public function groupbuy_listOp() {
        ...
        $goods_list = $model_groupbuy->getGroupbuyList($condition);
        foreach ($goods_list as $k=>$v) {
            $goods_list[$k]['format_time'] = '2h';
            $goods_list[$k]['groupbuy_image'] = gthumb($goods_list[$k]['groupbuy_image']);
            $goods_list[$k]['groupbuy_image1'] = gthumb($goods_list[$k]['groupbuy_image1']);
        }

        Tpl::output('goods_list', $goods_list);
        Tpl::output('show_page', $model_groupbuy->showpage());
        Tpl::showpage('mb_special_widget.groupbuy', 'null_layout');
    }

    /**
     * 限时折扣列表
     */
    public function promotion_xianshi_listOp() {
        if(isset($_REQUEST['keyword'])){
            $keyword = trim($_REQUEST['keyword']);
            $_SERVER['REQUEST_URI'] .= '&keyword=' . $keyword;
        }
        $model_p_xianshi_goods = Model('p_xianshi_goods');
        $condition = array();
        if($keyword){
            $condition['goods_name'] = array('like', '%' . $keyword . '%');
        }
        ...
        $goods_list = $model_p_xianshi_goods->getXianshiGoodsList($condition, 'goods_id,goods_name,goods_price,goods_rebate_rate,goods_promotion_price,goods_promotion_type,goods_marketprice,goods_image', 10);

        Tpl::output('goods_list', $goods_list);
        Tpl::output('show_page', $model_p_xianshi_goods->showpage());
        Tpl::showpage('mb_special_widget.promotion_xianshi', 'null_layout');
    }
