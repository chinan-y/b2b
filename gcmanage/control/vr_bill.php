...
class vr_billControl extends SystemControl{
    ...
    private $links = array(
    	array('url'=>'gct=vr_bill&gp=index','lang'=>'nc_manage'),
    );
	...
	public function bill_payOp(){
		...
		if (chksubmit()){
			...
			if ($update){
				...
				showMessage('保存成功','index.php?gct=vr_bill&gp=show_statis&os_month='.$bill_info['os_month']);
			}else{
			    $this->log('账单付款,账单号：'.$_GET['ob_no'],1);
	...
	public function export_billOp(){
		...
		if (!is_numeric($_GET['curpage'])){
    		...
    		if ($count > self::EXPORT_SIZE ){
    		    ...
    		    Tpl::output('murl','index.php?gct=vr_bill&gp=index');
	...
	public function export_orderOp(){
		...
		if (!is_numeric($_GET['curpage'])){
    		...
    		if ($count > self::EXPORT_SIZE ){
    		    ...
    		    Tpl::output('list',$array);
    		    Tpl::output('murl','index.php?gct=vr_bill&gp=show_bill&ob_no='.$_GET['ob_no']);
    		    Tpl::showpage('export.excel');
    		    exit();
    		}else{
    		    //如果数量小，直接下载
