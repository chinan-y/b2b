...
class storeControl extends SystemControl{
	...
	public function store_editOp(){
		...
		if (chksubmit()){
			...
			//20150705 BY MING
				...
				$url = array(
				array(
				'url'=>'index.php?gct=store&gp=store',
				'msg'=>$lang['back_store_list'],
				),
				array(
				'url'=>'index.php?gct=store&gp=store_edit&store_id='.intval($_POST['store_id']),
    ...
    public function edit_save_joininOp() {
        if (chksubmit()) {
            ...
            if ($result) {
                ...
                showMessage(L('nc_common_op_succ'), 'index.php?gct=store&gp=store');
            } else {
    ...
    private function store_joinin_verify_pass($joinin_detail) {
        ...
        if ($param['paying_amount'] > 0) {
            showMessage('店铺入驻申请审核完成','index.php?gct=store&gp=store_joinin');
        } else {
    ...
    private function store_joinin_verify_open($joinin_detail) {
        ...
        if($_POST['verify_type'] === 'pass') {
			...
			if($state) {
                ...
                showMessage('店铺开店成功','index.php?gct=store&gp=store_joinin');
            } else {
                showMessage('店铺开店失败','index.php?gct=store&gp=store_joinin');
            }
        } else {
            showMessage('店铺开店拒绝','index.php?gct=store&gp=store_joinin');
        }
    }
