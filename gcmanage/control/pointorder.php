...
class pointorderControl extends SystemControl{
	...
	public function order_dropOp(){
	    ...
	    if ($data['state']){
	        showDialog(L('admin_pointorder_del_success'),'index.php?gct=pointorder&gp=pointorder_list','succ');
	    } else {
	        showDialog($data['msg'],'index.php?gct=pointorder&gp=pointorder_list','error');
	    }
	}
	...
	public function order_cancelOp(){
	    ...
	    if ($data['state']){
	        showDialog(L('admin_pointorder_cancel_success'),'index.php?gct=pointorder&gp=pointorder_list','succ');
	    }else {
	        showDialog($data['msg'],'index.php?gct=pointorder&gp=pointorder_list','error');
	    }
	}
	...
	public function order_shipOp(){
		...
		if ($order_id <= 0){
			showDialog(L('admin_pointorder_parameter_error'),'index.php?gct=pointorder&gp=pointorder_list','error');
		}
		$model_pointorder = Model('pointorder');
		//获取订单状态
		$pointorderstate_arr = $model_pointorder->getPointOrderStateBySign();
		...
		if (!$order_info){
		    showDialog(L('admin_pointorderd_record_error'),'index.php?gct=pointorder&gp=pointorder_list','error');
		}
		if (chksubmit()){
			...
			if ($error != ''){
				showDialog(L('error').$error,'index.php?gct=pointorder&gp=pointorder_list','error');
			}
			//发货
			$data = $model_pointorder->shippingPointOrder($order_id, $_POST, $order_info);
			if ($data['state']){
				showDialog('发货修改成功','index.php?gct=pointorder&gp=pointorder_list','succ');
			}else {
				showDialog($data['msg'],'index.php?gct=pointorder&gp=pointorder_list','error');
			}
		} else {
