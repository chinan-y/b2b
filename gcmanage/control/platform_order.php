...
class platform_orderControl extends SystemControl{
    /**
     * 每次导出订单数量
     * @var int
     */
	...
	public function indexOp(){
        ...
        if($_GET['platform_name']) {
			$condition_plat = array();
			$condition_plat['partner_name'] = array('like', '%' . $_GET['platform_name'] . '%');
			$model_partnerlist = Model('partner') -> getPartnerList($condition_plat,'*',0,'');
			foreach($model_partnerlist as $k => $v){
				$partner_member .= ','.$v['member_id'];
			}
			$condition['platform_member_id'] = array('in',$partner_member);
        }else{
			$condition['platform_member_id'] = array('gt',0);//平台合作方platform_member_id均有大于0的值
		}
        if($_GET['store_name']) {
        ...
        foreach ($order_list as $order_id => $order_info) {
			...
			$order_list[$order_id]['platform_name'] = $platname['partner_name'];
			$order_list[$order_id]['salesarea_name'] = $areaname['sa_name'];
			$order_list[$order_id]['agentarea_name'] = $areaname['sa_areaname'];
        }
        //显示支付接口列表(搜索)
        ...
        Tpl::showpage('platform_order.index');
	}
...
//		}
	...
	private function createExcel($data = array()){
		...
		$excel_data[0][] = array('styleid'=>'s_title','data'=>'团队绑定会员ID');
		...
		foreach ((array)$data as $k=>$v){
			...
			$tmp[] = array('format'=>'Number','data'=>ncPriceFormat($v['three_rebate']));
			...
			$tmp[] = array('data'=>$v['platform_member_id']);
