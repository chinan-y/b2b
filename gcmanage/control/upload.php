...
class uploadControl extends SystemControl{
	private $links = array(
		array('url'=>'gct=upload&gp=param','lang'=>'upload_param'),
		array('url'=>'gct=upload&gp=default_thumb','lang'=>'default_thumb'),
		array('url'=>'gct=upload&gp=login','lang'=>'loginSettings'),
		array('url'=>'gct=upload&gp=tool','lang'=>'image_thumb_tool'),
		array('url'=>'gct=upload&gp=font','lang'=>'font_set')
	);
	...
	public function default_thumbOp(){
		...
		if (chksubmit()){
			...
			if(C(OSS_IS_STORAGE) == 1){
				...
				//默认商品图片
				if (!empty($_FILES['default_goods_image']['tmp_name'])){
					$extend 	= pathinfo($_FILES['default_goods_image']['name']); 
					$extend 	= strtolower($extend["extension"]); 
					$imgname	= 'default_goods_image' . date('Y');
					$imgfullname= $imgname . '.' . $extend;
					$objectname = DIR_UPLOAD.DS.ATTACH_COMMON.DS.$imgfullname;
					
					$pathname 	= $_FILES['default_goods_image']['tmp_name'];
					if($_FILES['default_goods_image']['type'] =='image/gif' || $_FILES['default_goods_image']['type'] =='image/jpeg' || $_FILES['default_goods_image']['type'] =='image/png'){
						$result = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
						if ($result){
							$_POST['default_goods_image'] = $imgfullname;
						}else {
							showMessage('Upload Error!','','','error');
						}
					}
				...
				//默认店铺标志
				if (!empty($_FILES['default_store_logo']['tmp_name'])){
					$extend 	= pathinfo($_FILES['default_store_logo']['name']); 
					$extend 	= strtolower($extend["extension"]); 
					$imgname	= 'default_store_logo' . date('Y');
					$imgfullname= $imgname . '.' . $extend;
					$objectname = DIR_UPLOAD.DS.ATTACH_COMMON.DS.$imgfullname;
					
					$pathname 	= $_FILES['default_store_logo']['tmp_name'];
					if($_FILES['default_store_logo']['type'] =='image/gif' || $_FILES['default_store_logo']['type'] =='image/jpeg' || $_FILES['default_store_logo']['type'] =='image/png'){
						$result = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
						if ($result){
							$_POST['default_store_logo'] = $imgfullname;
						}else {
							showMessage('Upload Error!','','','error');
						}
					}
				...
				//默认店铺头像
				if (!empty($_FILES['default_store_avatar']['tmp_name'])){
					$extend 	= pathinfo($_FILES['default_store_avatar']['name']); 
					$extend 	= strtolower($extend["extension"]); 
					$imgname	= 'default_store_avatar' . date('Y');
					$imgfullname= $imgname . '.' . $extend;
					$objectname = DIR_UPLOAD.DS.ATTACH_COMMON.DS.$imgfullname;
					
					$pathname 	= $_FILES['default_store_avatar']['tmp_name'];
					if($_FILES['default_store_avatar']['type'] =='image/gif' || $_FILES['default_store_avatar']['type'] =='image/jpeg' || $_FILES['default_store_avatar']['type'] =='image/png'){
						$result = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
						if ($result){
							$_POST['default_store_avatar'] = $imgfullname;
						}else {
							showMessage('Upload Error!','','','error');
						}
					}
				...
				//默认会员头像
				if (!empty($_FILES['default_user_portrait']['tmp_name'])){
					$extend 	= pathinfo($_FILES['default_user_portrait']['name']); 
					$extend 	= strtolower($extend["extension"]); 
					$imgname	= 'default_user_portrait' . date('Y');
					$imgfullname= $imgname . '.' . $extend;
					$objectname = DIR_UPLOAD.DS.ATTACH_COMMON.DS.$imgfullname;
					
					$pathname 	= $_FILES['default_user_portrait']['tmp_name'];
					if($_FILES['default_user_portrait']['type'] =='image/gif' || $_FILES['default_user_portrait']['type'] =='image/jpeg' || $_FILES['default_user_portrait']['type'] =='image/png'){
						$result = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
						if ($result){
							$_POST['default_user_portrait'] = $imgfullname;
						}else {
							showMessage('Upload Error!','','','error');
						}
					}
	...
	public function loginOp(){
		...
		if (chksubmit()){
			...
			if(C(OSS_IS_STORAGE) == 1){
				...
				$bucketname = IMAGE_BUCKET_NAME;
				if (!empty($_FILES['login_pic1']['name'])){
					$extend 	= pathinfo($_FILES['login_pic1']['name']); 
					$extend 	= strtolower($extend["extension"]); 
					$imgname	= '1';
					$imgfullname= $imgname . '.' . $extend;
					$objectname = DIR_UPLOAD.DS.ATTACH_PATH.DS.'login'.DS.$imgfullname;
					
					$pathname 	= $_FILES['login_pic1']['tmp_name'];
					if($_FILES['login_pic1']['type'] =='image/gif' || $_FILES['login_pic1']['type'] =='image/jpeg' || $_FILES['login_pic1']['type'] =='image/png'){
						$result = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
						if (!$result){
							showMessage('Upload error!','','','error');
						}else{
				...
				if (!empty($_FILES['login_pic2']['name'])){
					$extend 	= pathinfo($_FILES['login_pic2']['name']); 
					$extend 	= strtolower($extend["extension"]); 
					$imgname	= '2';
					$imgfullname= $imgname . '.' . $extend;
					$objectname = DIR_UPLOAD.DS.ATTACH_PATH.DS.'login'.DS.$imgfullname;
					
					$pathname 	= $_FILES['login_pic2']['tmp_name'];
					if($_FILES['login_pic2']['type'] =='image/gif' || $_FILES['login_pic2']['type'] =='image/jpeg' || $_FILES['login_pic2']['type'] =='image/png'){
						$result = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
						if (!$result){
							showMessage('Upload error!','','','error');
						}else{
				...
				if (!empty($_FILES['login_pic3']['name'])){
					$extend 	= pathinfo($_FILES['login_pic3']['name']); 
					$extend 	= strtolower($extend["extension"]); 
					$imgname	= '3';
					$imgfullname= $imgname . '.' . $extend;
					$objectname = DIR_UPLOAD.DS.ATTACH_PATH.DS.'login'.DS.$imgfullname;
					
					$pathname 	= $_FILES['login_pic3']['tmp_name'];
					if($_FILES['login_pic3']['type'] =='image/gif' || $_FILES['login_pic3']['type'] =='image/jpeg' || $_FILES['login_pic3']['type'] =='image/png'){
						$result = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
						if (!$result){
							showMessage('Upload error!','','','error');
						}else{
				...
				if (!empty($_FILES['login_pic4']['name'])){
					$extend 	= pathinfo($_FILES['login_pic4']['name']); 
					$extend 	= strtolower($extend["extension"]); 
					$imgname	= '4';
					$imgfullname= $imgname . '.' . $extend;
					$objectname = DIR_UPLOAD.DS.ATTACH_PATH.DS.'login'.DS.$imgfullname;
					
					$pathname 	= $_FILES['login_pic4']['tmp_name'];
					if($_FILES['login_pic4']['type'] =='image/gif' || $_FILES['login_pic4']['type'] =='image/jpeg' || $_FILES['login_pic4']['type'] =='image/png'){
						$result = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
						if (!$result){
							showMessage('Upload error!','','','error');
						}else{
