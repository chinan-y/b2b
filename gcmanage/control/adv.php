			...
			if ($error != ''){
				...
				if($_POST['aptype_hidden'] == '0'){
					...
					if(C(OSS_IS_STORAGE) == 1){
						...
						$extend = pathinfo($_FILES['adv_pic']['name']); 
						$extend = strtolower($extend["extension"]); 
						$newname	= 'ap_' . date('YmdHis') . rand(0,9999) . '.' . $extend;
						$objectname = DIR_UPLOAD.DS.ATTACH_ADV.DS.$newname;
						$pathname 	= $_FILES['adv_pic']['tmp_name'];
						if($_FILES['adv_pic']['type'] =='image/gif' || $_FILES['adv_pic']['type'] =='image/jpeg' || $_FILES['adv_pic']['type'] =='image/png'){
				...
				if($_POST['aptype_hidden'] == '3'){
					...
					if(C(OSS_IS_STORAGE) == 1){
						...
						$extend = pathinfo($_FILES['flash_swf']['name']); 
						$extend = strtolower($extend["extension"]); 
						$newname	= 'af_' . date('YmdHis') . rand(0,9999) . '.' . $extend;
						$objectname = DIR_UPLOAD.DS.ATTACH_ADV.DS.$newname;
						$pathname 	= $_FILES['flash_swf']['tmp_name'];
						$uploadfile = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
						$ac = array(
						'flash_swf'  =>$newname,
						'flash_url'  =>trim($_POST['flash_url'])
			    ...
			    if ($result&&$result2){
					...
					showMessage($lang['adv_add_succ'],'index.php?gct=adv&gp=adv&ap_id='.$_POST['ap_id']);
				}else {
			...
			if ($error != ''){
			    ...
			    if($_FILES['default_pic']['name'] != ''){
					...
					if(C(OSS_IS_STORAGE) == 1){
						...
						$extend = pathinfo($_FILES['default_pic']['name']); 
						$extend = strtolower($extend["extension"]); 
						$newname	= 'ap_' . date('YmdHis') . rand(0,9999) . '.' . $extend;
						$objectname = DIR_UPLOAD.DS.ATTACH_ADV.DS.$newname;
						$pathname 	= $_FILES['default_pic']['tmp_name'];
						if($_FILES['default_pic']['type'] =='image/gif' || $_FILES['default_pic']['type'] =='image/jpeg' || $_FILES['default_pic']['type'] =='image/png'){
							$result = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
						}
						$param['default_content'] = $newname;
					}
			...
			if ($error != ''){
				...
				if($_FILES['default_pic']['name'] != ''){
					...
					if(C(OSS_IS_STORAGE) == 1){
						...
						$insert_array['default_content'] = $newname;
					}
				}
				...
				if ($result){
					...
					showMessage($lang['ap_add_succ'],'index.php?gct=adv&gp=ap_manage','html','succ',1,4000);
				}else {
       ...
       /**
		 ...
		 */
		    ...
		    if (!$result){
					$url = array(
						array(
							'url'=>'index.php?gct=adv&gp=adv',
							'msg'=>$lang['goback_adv_manage'],
						),
						array(
							'url'=>'index.php?gct=adv&gp=ap_manage',
							'msg'=>$lang['goback_ap_manage'],
						),
					);
