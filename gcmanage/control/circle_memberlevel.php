...
class circle_memberlevelControl extends SystemControl{
	...
	public function ref_addOp(){
		if(chksubmit()){
			...
			if($rs){
				$url = array(
						array(
								'url'=>'index.php?gct=circle_memberlevel&gp=ref_add',
								'msg'=>L('circle_continue_add'),
						),
						array(
								'url'=>'index.php?gct=circle_memberlevel&gp=ref',
								'msg'=>L('nc_backlist'),
						)
				);
	...
	public function ref_editOp(){
		...
		if(chksubmit()){
			...
			if($rs){
				showMessage(L('nc_common_op_succ'), 'index.php?gct=circle_memberlevel&gp=ref');
			}else{
				showMessage(L('nc_common_op_fail'));
			}
		}
	...
	public function update_cacheOp() {
		...
		showMessage(L('nc_common_op_succ'), 'index.php?gct=circle_memberlevel');
