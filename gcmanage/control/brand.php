...
class brandControl extends SystemControl{
		...
		if (chksubmit()){
			...
			if ($error != ''){
				...
				if ($result){
					$url = array(
						array(
							'url'=>'index.php?gct=brand&gp=brand_add',
							'msg'=>$lang['brand_add_again'],
						),
						array(
							'url'=>'index.php?gct=brand&gp=brand',
				...
				if ($result){
					if (!empty($_POST['brand_pic']) && !empty($brand_info['brand_pic'])){
						...
						if(C(OSS_IS_STORAGE) == 1){
							$oss = Logic('oss');
							$oss->deleteObject(IMAGE_BUCKET_NAME, DIR_UPLOAD.DS.ATTACH_BRAND.DS.$brand_info['brand_pic']);
						}

					}
					$url = array(
						array(
							'url'=>'index.php?gct=brand&gp=brand_edit&brand_id='.intval($_POST['brand_id']),
	...
	public function brand_delOp(){
		if (intval($_GET['del_brand_id']) > 0){
			...
			showMessage(L('nc_common_del_succ'),'index.php?gct=brand&gp=brand');
		}else {
			$this->log(L('nc_delete,brand_index_brand').'[ID:'.intval($_GET['del_brand_id']).']',0);
			showMessage(L('nc_common_del_fail'),'index.php?gct=brand&gp=brand');
		}
	}
	...
	public function export_step1Op(){
		...
		if (!is_numeric($_GET['curpage'])){
			...
			if ($count > self::EXPORT_SIZE ){	//显示下载链接
				...
				Tpl::output('murl','index.php?gct=brand&gp=brand');
