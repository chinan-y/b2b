...
class pointprodControl extends SystemControl{
		...
		if (chksubmit()){
			...
			if (!empty($_FILES['goods_image']['name'])){
				...
				if(C(OSS_IS_STORAGE) == 1){
					...
					$extend 	= pathinfo($_FILES['goods_image']['name']); 
					$extend 	= strtolower($extend["extension"]); 
					$imgname	= 'pp_'.date('His').rand(0,999);
					$imgfullname= $imgname . '.' . $extend;
					$objectname = DIR_UPLOAD.DS.ATTACH_POINTPROD.DS.$imgfullname;
					$pathname 	= $_FILES['goods_image']['tmp_name'];
					$result = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
					if ($result){
						$indeximg_succ = true;
						$goods_array['pgoods_image'] = $imgfullname;
					}else {
						showDialog('Upload Error!','','error');
					}
				}
			...
			if($state){
				...
				showDialog(L('admin_pointprod_add_success'),'index.php?gct=pointprod&gp=pointprod','succ');
			}
		}
		//模型实例化
		$where = array();
	...
	public function prod_editOp(){
		...
		if (!$pg_id){
			showDialog(L('admin_pointprod_parameter_error'),'index.php?gct=pointprod&gp=pointprod','error');
		}
		$model_pointprod = Model('pointprod');
		//查询礼品记录是否存在
		$prod_info = $model_pointprod->getPointProdInfo(array('pgoods_id'=>$pg_id));
		if (!$prod_info){
			showDialog(L('admin_pointprod_record_error'),'index.php?gct=pointprod&gp=pointprod','error');
		}
		if (chksubmit()){
			//验证表单
			$obj_validate = new Validate();
			...
			if($state){
				...
				if ($indeximg_succ){
					...
					if (is_array($upload_list) && count($upload_list)>0){
						...
						foreach ($upload_list as $v){
							...
							if(C(OSS_IS_STORAGE) == 1){
								$oss = Logic('oss');
								$oss ->deleteObject(IMAGE_BUCKET_NAME, DIR_UPLOAD.DS.ATTACH_POINTPROD.DS.$v['file_name']);
								$oss ->deleteObject(IMAGE_BUCKET_NAME, DIR_UPLOAD.DS.ATTACH_POINTPROD.DS.$v['file_thumb']);
							}
					...
					if(C(OSS_IS_STORAGE) == 1){
						$insert_array['file_name'] = $imgfullname;
						$insert_array['file_thumb'] = $imgfullname;
						$imageinfo = $oss->getObjectMeta(IMAGE_BUCKET_NAME, DIR_UPLOAD.DS.ATTACH_POINTPROD.DS.$imgfullname);
						$insert_array['file_size'] = $imageinfo['info']['download_content_length'];
					}
					$insert_array['upload_type'] = 5;
				...
				showDialog(L('admin_pointprod_edit_success'),'index.php?gct=pointprod&gp=pointprod','succ');
			}
		}else {
		    $where = array();
			$where['upload_type'] = '6';
	...
	public function prod_dropOp(){
		...
		if($result) {
			...
			showDialog(L('admin_pointprod_del_success'),'index.php?gct=pointprod&gp=pointprod','succ');
		} else {
			showDialog(L('admin_pointprod_del_fail'),'index.php?gct=pointprod&gp=pointprod','error');
		}
	}
	...
	public function pointprod_pic_uploadOp(){
		...
		if(C(OSS_IS_STORAGE) == 1){
			...
			$oss = Logic('oss');
			$bucketname = IMAGE_BUCKET_NAME;
			$extend 	= pathinfo($_FILES['fileupload']['name']); 
			$extend 	= strtolower($extend["extension"]); 
			$imgname	= 'pf_'.date('His').rand(0,999);
			$imgfullname= $imgname . '.' . $extend;
			$objectname = DIR_UPLOAD.DS.ATTACH_POINTPROD.DS.$imgfullname;
			$pathname 	= $_FILES['fileupload']['tmp_name'];
			$result = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
			if ($result){
				$_POST['pic'] = $imgfullname;
			}else {
				echo 'error';exit;
			}
		}
	...
	public function ajaxdeluploadOp(){
		...
		if (intval($_GET['file_id']) > 0){
			...
			if(C(OSS_IS_STORAGE) == 1){
				$oss =Logic('oss');
				$oss ->deleteObject(IMAGE_BUCKET_NAME, DIR_UPLOAD.DS.ATTACH_POINTPROD.DS.$file_array['file_name']);
			}
