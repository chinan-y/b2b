...
class stat_memberControl extends SystemControl{
    private $links = array(
        array('url'=>'gct=stat_member&gp=newmember','lang'=>'stat_newmember'),
        array('url'=>'gct=stat_member&gp=analyze','lang'=>'stat_memberanalyze'),
        array('url'=>'gct=stat_member&gp=scale','lang'=>'stat_scaleanalyze'),
        array('url'=>'gct=stat_member&gp=area','lang'=>'stat_areaanalyze'),
        array('url'=>'gct=stat_member&gp=buying','lang'=>'stat_buying'),
    );
    ...
    public function __construct(){
		...
		//处理搜索时间
		if (in_array($_REQUEST['gp'],array('newmember','analyze','scale','area','buying'))){
    ...
    public function newmemberOp(){
		...
		if($this->search_arr['search_type'] == 'day'){
			...
			Tpl::output('actionurl','index.php?gct=stat_member&gp=newmember&search_type=day&search_time='.date('Y-m-d',$this->search_arr['day']['search_time']));
		}
		...
		if($this->search_arr['search_type'] == 'week'){
			...
			Tpl::output('actionurl','index.php?gct=stat_member&gp=newmember&search_type=week&searchweek_year='.$this->search_arr['week']['current_year'].'&searchweek_month='.$this->search_arr['week']['current_month'].'&searchweek_week='.$this->search_arr['week']['current_week']);
		}
		...
		if($this->search_arr['search_type'] == 'month'){
			...
			Tpl::output('actionurl','index.php?gct=stat_member&gp=newmember&search_type=month&searchmonth_year='.$this->search_arr['month']['current_year'].'&searchmonth_month='.$this->search_arr['month']['current_month']);
		}
	...
	public function showmemberOp(){
		...
		if (in_array($_GET['type'],array('newbyday','newbyweek','newbymonth'))){
		    $actionurl = 'index.php?gct=stat_member&gp=showmember&type=newbyday&t='.$_GET['t'];
		...
		Tpl::output('show_page',$model->showpage(2));
		$this->links[] = array('url'=>'gct=stat_member&gp=showmember','lang'=>'stat_memberlist');
	...
	public function area_listOp(){
		...
		Tpl::output('orderby',$this->search_arr['orderby']);
		Tpl::output('actionurl',"index.php?gct={$this->search_arr['gct']}&gp={$this->search_arr['gp']}&t={$this->search_arr['t']}");
		Tpl::output('show_page',$model->showpage(2));
		Tpl::output('top_link',$this->sublink($this->links, 'area'));
