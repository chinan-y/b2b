...
class rec_positionControl extends SystemControl{
			...
			if ($info['pic_type'] == 1 && is_array($info['content']['body'])){
				foreach ($info['content']['body'] as $v){
					...
					if(C(OSS_IS_STORAGE) == 1){
						$oss =Logic('oss');
						$oss -> deleteObject(IMAGE_BUCKET_NAME, DIR_UPLOAD.DS.$v['title']);
					}
				}
			}
			dkcache("rec_position/{$info['rec_id']}");
	...
	public function rec_saveOp(){
					...
					if (in_array($ext,array('jpg','jpeg','gif','png'))){
						...
						if ($_FILES['pic']['size'][$k]<1024*1024){
							...
							if(C(OSS_IS_STORAGE) == 1){
								...
								$objectname = DIR_UPLOAD.DS.ATTACH_REC_POSITION.DS.$imgfullname;
								$pathname 	= $_FILES['pic']['tmp_name'][$k];
		...
		showMessage(Language::get('nc_common_save_succ'),'index.php?gct=rec_position&gp=rec_list');
	}
	...
	public function rec_edit_saveOp(){
			...
			if (is_array($_FILES['pic']['tmp_name'])){
				foreach($_FILES['pic']['tmp_name'] as $k=>$v){
					...
					if (in_array($ext,array('jpg','jpeg','gif','png','bmp'))){
						...
						if(C(OSS_IS_STORAGE) == 1){
							//删除老图
							$old_file = str_ireplace(array(UPLOAD_SITE_URL,'..'),array(DIR_UPLOAD,''),$_POST['opic'][$k]);
							$oss ->deleteObject(IMAGE_BUCKET_NAME, $old_file);
						}
				...
				foreach ($oinfo['body'] as $k=>$v) {
					if (!in_array(UPLOAD_SITE_URL.'/'.$v['title'],(array)$_POST['opic'])){
						...
						if(C(OSS_IS_STORAGE) == 1){
							$oss ->deleteObject(IMAGE_BUCKET_NAME, DIR_UPLOAD.'/'.$v['title']);
						}
		...
		if ($result){
			...
			showMessage(Language::get('nc_common_save_fail'),'index.php?gct=rec_position&gp=rec_list');
		}
	}
