...
class admin_logControl extends SystemControl{
	...
	public function listOp(){
		...
		if (!empty($_GET['admin_content'])){
			$condition = "content like '%".$_GET['admin_content']."%'";
		}
		if ($time1 && $time2){
...
//		$admin = Model()->table('admin,gadmin')->field('admin_id,admin_name,gid,gname')->join('left')->on('admin.admin_gid=gadmin.gid')->select();
	...
	public function list_delOp(){
		...
		if($admin['sp'] != 1){
			showMessage(L('nc_common_powerless'),'','html','error');
		}else{
	...
	public function export_step1Op(){
		...
		if (!is_numeric($_GET['curpage'])){
			...
			if ($count > self::EXPORT_SIZE ){	//显示下载链接
				...
				Tpl::output('murl','index.php?gct=admin_log&gp=list');
