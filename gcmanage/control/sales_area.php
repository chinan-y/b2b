...
class sales_areaControl extends SystemControl{
	const EXPORT_SIZE = 500000;
	...
	public function sales_areaOp(){
		...
		if (chksubmit()){
			if (!empty($_POST['check_sa_id']) && is_array($_POST['check_sa_id']) ){
			    //$result = $model_class->delSalesArea(array('sa_id'=>array('in',$_POST['check_sa_id'])));//取消删除区域合作方
				if ($result) {
		...
		$condition = array('sa_distinguish' => 1);
		$sales_area_list = $model_class->getSalesAreaList($condition,10);
		foreach($sales_area_list as $k =>$val){
			$rate = Model('member')->getMemberInfoByID($val['sa_manager_id'], 'member_rebate_rate');
			$sales_area_list[$k]['member_rebate_rate'] = $rate['member_rebate_rate']*100 ;
		}
		Tpl::output('sales_area_list',$sales_area_list);
		Tpl::output('page',$model_class->showpage());

		Tpl::showpage('sales_area.index');
	}
	...
	public function sales_area_samlistOp(){
		...
		$order = 'is_manager desc, member_id asc';
		$samlist = $model_samlist->getMemberList($where, '', 10,$order);
		if (is_array($samlist)){
		...
		Tpl::showpage('sales_area.samlist');		
	 ...
	 * 区域合作方添加
	 */
	public function sales_area_addOp(){
		$lang	= Language::getLangContent();
		$model_class = Model('sales_area');
		if (chksubmit()){
			//验证
			...
			if ($error != ''){
				...
				$insert_array = array();
				$insert_array['sa_name'] = $_POST['sa_name'];
				$insert_array['sa_manager'] = $_POST['sa_manager'];
				$insert_array['sa_manager_id'] = intval($_POST['sa_manager_id']);
				$insert_array['sa_contact'] = $_POST['sa_contact'];
				$insert_array['sa_desc'] = $_POST['sa_desc'];
				$insert_array['sa_cash'] = intval($_POST['sa_cash']);
				$insert_array['sa_rate'] = intval($_POST['sa_rate']);
				$insert_array['sa_sort'] = intval($_POST['sa_sort']);
				$insert_array['sa_distinguish'] = intval($_POST['sa_er']);
				$insert_array['add_time'] = TIMESTAMP;
				
				//区域绑定的区域经理ID是否存在
				$isexit_manager = $model_class->where(array('sa_manager_id'=>$insert_array['sa_manager_id']))->find();
				if($isexit_manager){
					showMessage('区域经理已经存在，不能重复！');
					}
				else{
					$result = $model_class->addSalesArea($insert_array);
					}
					
				$condition_managerid = array();
				$condition_managerid['sa_manager_id'] = intval($_POST['sa_manager_id']);
				$update_said = $model_class->getSalesAreaIn($condition_managerid,'*');
				
				//取sa_managerparter_id值更新member表的sa_id和is_manager
					$update_meminfo = array();
					$update_meminfo['is_seller'] = 1;
					$update_meminfo['is_rebate'] = 0;
					$update_meminfo['is_manager'] = 1;
					$update_meminfo['is_member_rebate'] = 1;
					$update_meminfo['saleplat_id'] = 0;
					$update_meminfo['member_rebate_rate'] = $update_said[0]['sa_rate']/100;
					$update_meminfo['sa_id'] = $update_said[0]['sa_id'];
				
					$model_member = Model('member');
					$member_salerea = $model_member->editMember(array('member_id'=>intval($_POST['sa_manager_id'])),$update_meminfo);
				
				if ($result && $member_salerea){
					$url = array(
					array(
					'url'=>'index.php?gct=sales_area&gp=sales_area_add',
					'msg'=>$lang['continue_add_sales_area'],
					),
					array(
					'url'=>'index.php?gct=sales_area&gp=sales_area',
					'msg'=>$lang['back_sales_area_list'],
					)
					);

					$this->log(L('nc_add,sales_area').'['.$_POST['sa_name'].']',1);
	...
	public function sales_area_editOp(){
		...
		if (chksubmit()){
			...
			if ($error != ''){
				...
				$update_array['agent_start_time'] = TIMESTAMP;
				
				//区域绑定的区域经理ID是否存在
				$isexit_manager = $model_class->where(array('sa_manager_id'=>$update_array['sa_manager_id']))->find();
				$isexit_areaid = $model_class->where(array('sa_areaid'=>$update_array['sa_areaid']))->find();
				if($isexit_manager){
						showMessage('区域管理者已绑定过！不能重复！');
					}
				elseif($isexit_areaid){
						showMessage('区域已授权过！不能重复！');
					}
				else{
	 ...
	 * 取消授权区域或取消区域管理者
	 */
	public function sales_area_cancelOp(){
		$lang	= Language::getLangContent();
		...
		if($_GET['sa_id']){
		$update_array = array();
		$update_array['sa_areaid'] = 0;
		$update_array['sa_areaname'] = null;
		$update_array['agent_end_time'] = TIMESTAMP;
		$result = $model_class->editSalesArea($update_array,array('sa_id'=>intval($_GET['sa_id'])));
			if ($result){
				$this->log(L('nc_edit,sales_area').'['.$_GET['sa_id'].']',1);
				showMessage($lang['nc_common_save_succ'],'index.php?gct=sales_area&gp=sales_area');
			}else {
				showMessage($lang['nc_common_save_fail']);
			}
		...
		if($_GET['sa_manager_id']){
		$update_array = array();
		$update_array['sa_manager_id'] = 0;
		$update_array['sa_manager'] = null;
		$result = $model_class->editSalesArea($update_array,array('sa_manager_id'=>intval($_GET['sa_manager_id'])));
		
		$update_mem = array();
		$update_mem['sa_id'] = 0;
		$update_mem['is_manager'] = 0;
		$update_mem['is_member_rebate'] = 0;
		$update_mem['is_rebate'] = 1;
		
		$result = $model_member->editMember(array('member_id'=>intval($_GET['sa_manager_id'])),$update_mem);
			if ($result){
				$this->log(L('nc_edit,sales_area').'['.$_GET['sa_manager_id'].']',1);
				showMessage($lang['nc_common_save_succ'],'index.php?gct=sales_area&gp=sales_area');
			}else {
				showMessage($lang['nc_common_save_fail']);
			}
	...
	public function sales_area_delOp(){
		...
		if (intval($_GET['sa_id']) > 0){
			...
			//$result = $model_class->delSalesArea(array('sa_id'=>intval($_GET['sa_id'])));//取消删除区域合作方
			if ($result) {
			     $this->log(L('nc_del,sales_area').'[ID:'.$_GET['sa_id'].']',1);
			     showMessage($lang['nc_common_del_succ'],getReferer());
			}
		}
		showMessage($lang['nc_common_del_fail'],'index.php?gct=sales_area&gp=sales_area');
	}
	...
	public function orderOp(){
		...
		if($_GET['area_id'] && $_GET['area_id']>0 ) {
...
//		var_dump($order_list);die;
	...
	public function achievementOp(){
		...
		Tpl::showpage('sales_achievement');
	}
...
//		var_dump($order_list);die;
		$list_log = $_SESSION['order_or'];
//		$list_log = $order_list;
//		var_dump($list_log);die;
	...
	private function createExcel($data = array()){
		...
		$excel_data[0][] = array('styleid'=>'s_title','data'=>'订单价格');
		$excel_data[0][] = array('styleid'=>'s_title','data'=>'支付方式');
		$excel_data[0][] = array('styleid'=>'s_title','data'=>'订单状态');
		$excel_data[0][] = array('styleid'=>'s_title','data'=>'订单返利');
		$excel_data[0][] = array('styleid'=>'s_title','data'=>'上级二维码用户ID');
		...
		$excel_data[0][] = array('styleid'=>'s_title','data'=>'商品促销');
...
//			var_dump($v);die;
			...
			$tmp[] = array('data'=>$v['goods_amount']);
			$tmp[] = array('data'=>$v['payment_name']);
			$tmp[] = array('data'=>$v['state_desc']);
			$tmp[] = array('data'=>$v['goods_rebate_amount']);
			$tmp[] = array('data'=>$v['extend_member']['refer_id']);
			foreach($v['extend_order_goods'] as $i => $u){
				$tmp[] = array('data'=>$u['goods_name']);
				$tmp[] = array('data'=>$u['goods_price']);
				$tmp[] = array('data'=>$u['goods_num']);
				$tmp[] = array('data'=>$u['goods_serial']);
				$tmp[] = array('data'=>$u['goods_type']);
			}
