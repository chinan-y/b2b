...
class activityControl extends SystemControl{
	...
	public function newOp(){
		...
		if(C(OSS_IS_STORAGE) == 1){
			...
			$extend 	= pathinfo($_FILES['activity_banner']['name']); 
			$extend 	= strtolower($extend["extension"]); 
			$imgname	= 'ac_'.date('YmdHis').rand(0,99);
			$imgfullname= $imgname . '.' . $extend;
			$objectname = DIR_UPLOAD.DS.ATTACH_ACTIVITY.DS.$imgfullname;
			$pathname 	= $_FILES['activity_banner']['tmp_name'];
			if($_FILES['activity_banner']['type'] =='image/gif' || $_FILES['activity_banner']['type'] =='image/jpeg' || $_FILES['activity_banner']['type'] =='image/png'){
				$result = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
				if ($result){
					$input['activity_banner']	= $imgfullname;
				}else{
					showMessage('ActivityPic upload error!','','','error');
				}
			}
		}
		...
		if($result){
			...
			showMessage(Language::get('nc_common_op_succ'),'index.php?gct=activity&gp=activity');
		}else{
			if(C(OSS_IS_STORAGE) == 0){
			...
			//删除在Aliyun-OSS刚上传的图片
			$oss = Logic('oss');
			$oss -> deleteObject(IMAGE_BUCKET_NAME, DIR_UPLOAD.DS.ATTACH_ACTIVITY.DS.$imgfullname);
			showMessage(Language::get('nc_common_op_fail'));	
			}
	...
	public function editOp(){
		...
		if(C(OSS_IS_STORAGE) == 0){
			if($result){
				...
				showMessage(Language::get('nc_common_save_succ'),'index.php?gct=activity&gp=activity');
			}else{
				if($_FILES['activity_banner']['name']!=''){
					@unlink(BASE_UPLOAD_PATH.DS.ATTACH_ACTIVITY.DS.$upload->file_name);
				}
		...
		if(C(OSS_IS_STORAGE) == 1){
			if($result){
				//删除在Aliyun-OSS上传的图片
				$oss =Logic('oss');
				$oss ->deleteObject(IMAGE_BUCKET_NAME, DIR_UPLOAD.DS.ATTACH_ACTIVITY.DS.$row['activity_banner']);
				$this->log(L('nc_edit,activity_index').'[ID:'.$_POST['activity_id'].']',null);
				showMessage(Language::get('nc_common_save_succ'),'index.php?gct=activity&gp=activity');
			}else{
				//删除在Aliyun-OSS刚上传的图片
