...
class settingControl extends SystemControl{
	private $links = array(
		array('url'=>'gct=setting&gp=base','lang'=>'web_set'),
		array('url'=>'gct=setting&gp=dump','lang'=>'dis_dump'),
		array('url'=>'gct=setting&gp=buy','lang'=>'buy_cart'),
	);
	...
	public function baseOp(){
		...
		if (chksubmit()){
			...
			if (!empty($_FILES['site_logo']['name'])){
				...
				if(C('OSS_IS_STORAGE') == 1){
					$_POST['site_logo'] = $_FILES['site_logo']['name'];
					
					$oss = Logic('oss');
					$bucketname = IMAGE_BUCKET_NAME;
					$objectname = DIR_UPLOAD.DS.ATTACH_COMMON.DS.$_FILES['site_logo']['name'];
					$pathname 	= $_FILES['site_logo']['tmp_name'];
					$uploadfile = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
				}
			}
			if (!empty($_FILES['site_mobile_logo']['name'])){
				...
				if(C('OSS_IS_STORAGE') == 1){
					$_POST['site_mobile_logo'] = $_FILES['site_mobile_logo']['name'];
					
					$oss = Logic('oss');
					$bucketname = IMAGE_BUCKET_NAME;
					$objectname = DIR_UPLOAD.DS.ATTACH_COMMON.DS.$_FILES['site_mobile_logo']['name'];
					$pathname 	= $_FILES['site_mobile_logo']['tmp_name'];
					$uploadfile = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
				}
			}
			//二维码微信图片
			if (!empty($_FILES['site_logowx']['name'])){
				...
				if(C('OSS_IS_STORAGE') == 1){
					$_POST['site_logowx'] = $_FILES['site_logowx']['name'];
					
					$oss = Logic('oss');
					$bucketname = IMAGE_BUCKET_NAME;
					$objectname = DIR_UPLOAD.DS.ATTACH_COMMON.DS.$_FILES['site_logowx']['name'];
					$pathname 	= $_FILES['site_logowx']['tmp_name'];
					$uploadfile = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
				}
			...
			if (!empty($_FILES['member_logo']['name'])){
				...
				if(C('OSS_IS_STORAGE') == 1){
					$_POST['member_logo'] = $_FILES['member_logo']['name'];
					
					$oss = Logic('oss');
					$bucketname = IMAGE_BUCKET_NAME;
					$objectname = DIR_UPLOAD.DS.ATTACH_COMMON.DS.$_FILES['member_logo']['name'];
					$pathname 	= $_FILES['member_logo']['tmp_name'];
					$uploadfile = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
				}
			}
			if (!empty($_FILES['seller_center_logo']['name'])){
				...
				if(C('OSS_IS_STORAGE') == 1){
					$_POST['seller_center_logo'] = $_FILES['seller_center_logo']['name'];
					
					$oss = Logic('oss');
					$bucketname = IMAGE_BUCKET_NAME;
					$objectname = DIR_UPLOAD.DS.ATTACH_COMMON.DS.$_FILES['seller_center_logo']['name'];
					$pathname 	= $_FILES['seller_center_logo']['tmp_name'];
					$uploadfile = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
				}
			}
			...
			$update_array['statistics_code_baidu'] = $_POST['statistics_code_baidu'];
			...
			$update_array['gcweb_copyright'] = $_POST['gcweb_copyright'];
			...
			$update_array['site_buy'] = $_POST['site_buy'];
			$update_array['closed_reason'] = $_POST['closed_reason'];
			$update_array['site_notice'] = serialize($_POST['site_notice']);
			...
			if ($result === true){
				...
				if (!empty($list_setting['site_mobile_logo']) && !empty($_POST['site_mobile_logo'])){
					@unlink(BASE_UPLOAD_PATH.DS.ATTACH_COMMON.DS.$list_setting['site_mobile_logo']);
				}
		...
		$list_setting = $model_setting->getListSetting();
		$list_setting['site_notice'] = unserialize($list_setting['site_notice']);
	...
	public function buyOp(){
		...
		if (chksubmit()){
			...
			$update_array['is_storage']   = $_POST['is_storage'];
			$update_array['seller_goods'] = $_POST['seller_goods'];
			$result = $model_setting->updateSetting($update_array);
			if ($result === true){
	...
	{
		/*
		http://你的程序地址/crontab/index.php?gct=minutes
		http://你的程序地址/crontab/index.php?gct=hour
		http://你的程序地址/crontab/index.php?gct=date
		http://你的程序地址/crontab/index.php?gct=month
		*/
		header("content-type:text/html; charset=utf-8"); 
		$page=BASE_SITE_URL.'/crontab/index.php?gct=minutes';
        $html = file_get_contents($page,'r');
		echo '分钟任务执行成功'.$html; 
		echo "<br/>";
		$page=BASE_SITE_URL.'/crontab/index.php?gct=hour';
        $html = file_get_contents($page,'r');
		echo '小时任务执行成功'.$html; 
		echo "<br/>";
		$page=BASE_SITE_URL.'/crontab/index.php?gct=date';
        $html = file_get_contents($page,'r');
		echo '每天任务执行成功'.$html; 
		echo "<br/>";
		$page=BASE_SITE_URL.'/crontab/index.php?gct=month';
        $html = file_get_contents($page,'r');
		echo '每月任务执行成功'.$html; 
		echo "<br/>";
		echo "<br/>全部计划任务执行成功";
