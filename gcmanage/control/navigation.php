...
class navigationControl extends SystemControl{
			...
			foreach ($navigation_list as $k => $v){
				switch ($v['nav_location']){
					...
					case '2':
						...
						break;
					case '3':
						$navigation_list[$k]['nav_location'] = $lang['navigation_index_mobile'];
						break;
					/**
					 * 增加微信端
					 */
					case '4':
						$navigation_list[$k]['nav_location'] = $lang['navigation_index_wechat'];
					break;
					//增加APP端
					case '5':
						$navigation_list[$k]['nav_location'] = $lang['navigation_index_app'];
					break;					
		...
		if (chksubmit()){
			...
			if ($error != ''){
				...
				if ($result){
					...
					$url = array(
						array(
							'url'=>'index.php?gct=navigation&gp=navigation_add',
							'msg'=>$lang['navigation_add_again'],
						),
						array(
							'url'=>'index.php?gct=navigation&gp=navigation',
		...
		if (chksubmit()){
			...
			if ($error != ''){
				...
				if ($result){
					...
					$url = array(
						array(
							'url'=>'index.php?gct=navigation&gp=navigation_edit&nav_id='.intval($_POST['nav_id']),
	...
	public function navigation_delOp(){
		...
		if (intval($_GET['nav_id']) > 0){
			...
			showMessage($lang['navigation_index_del_succ'],'index.php?gct=navigation&gp=navigation');
		}else {
			showMessage($lang['navigation_index_choose_del'],'index.php?gct=navigation&gp=navigation');
		}
	}
