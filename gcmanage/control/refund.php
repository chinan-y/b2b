...
class refundControl extends SystemControl{
	...
	public function refund_allOp() {
		...
		if (trim($_GET['finish_time_from']) != '' || trim($_GET['finish_time_to']) != '') {
			$finish_time_from = strtotime(trim($_GET['finish_time_from']));
			$finish_time_to = strtotime(trim($_GET['finish_time_to']));
			if ($finish_time_from !== false || $finish_time_to !== false) {
				$condition['admin_time'] = array('time',array($finish_time_from,$finish_time_to));
			}
		}
		$refund_list = $model_refund->getRefundList($condition,10);
		Tpl::output('refund_list',$refund_list);
	...
	public function editOp() {
		...
		if (chksubmit()) {
			...
			if ($state) {
				...
				showMessage(Language::get('nc_common_save_succ'),'index.php?gct=refund&gp=refund_manage');
			} else {
				showMessage(Language::get('nc_common_save_fail'));
			}
		}
	...
	public function add_reasonOp() {
		...
		if (chksubmit()) {
			...
			if ($state) {
				...
				showMessage(Language::get('nc_common_save_succ'),'index.php?gct=refund&gp=reason');
			} else {
				showMessage(Language::get('nc_common_save_fail'));
			}
		}
	...
	public function del_reasonOp() {
		...
		if ($state) {
		    ...
		    showMessage(Language::get('nc_common_del_succ'),'index.php?gct=refund&gp=reason');
		} else {
