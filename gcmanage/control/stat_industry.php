...
class stat_industryControl extends SystemControl{
    private $links = array(
        array('url'=>'gct=stat_industry&gp=scale','lang'=>'stat_industryscale'),
        array('url'=>'gct=stat_industry&gp=rank','lang'=>'stat_industryrank'),
        array('url'=>'gct=stat_industry&gp=price','lang'=>'stat_industryprice'),
        array('url'=>'gct=stat_industry&gp=general','lang'=>'stat_industrygeneral'),
    );
    ...
    public function __construct(){
		...
		//处理搜索时间
		if (in_array($this->search_arr['gp'],array('scale','rank','price'))){
		...
		if (in_array($this->search_arr['gp'],array('scale','general'))){//仅显示前两级分类
		    $show_depth = 2;
		}
	...
	public function general_listOp(){
    	...
    	Tpl::output('actionurl',"index.php?gct={$this->search_arr['gct']}&gp={$this->search_arr['gp']}&choose_gcid=".$this->choose_gcid);
