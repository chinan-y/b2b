...
class SystemControl{
	...
	protected function __construct(){
		...
		if (($_GET['branch']!='' || $_GET['gp']=='ajax') && strtoupper(CHARSET) == 'GBK'){
	...
	protected final function systemLogin(){
		...
		if (!key_exists('gid',(array)$user) || !isset($user['sp']) || (empty($user['name']) || empty($user['id']))){
			@header('Location: index.php?gct=login&gp=login');exit;
		}else {
	...
	protected final function checkPermission($link_nav = null){
		...
		$gct = $_GET['gct']?$_GET['gct']:$_POST['gct'];
		$gp = $_GET['gp']?$_GET['gp']:$_POST['gp'];
		...
		if (is_array($link_nav)){
			if (!in_array("{$link_nav['gct']}.{$link_nav['gp']}",$permission) && !in_array($link_nav['gct'],$permission)){
				return false;
			}else{
				return true;
			}
		...
		//以下几项不需要验证
		$tmp = array('index','dashboard','login','common','cms_base','live_modifly','live');
		if (in_array($gct,$tmp)) return true;
		if (in_array($gct,$permission) || in_array("$gct.$gp",$permission)){
			return true;
		}else{
			$extlimit = array('ajax','export_step1');
			if (in_array($gp,$extlimit) && (in_array($gct,$permission) || strpos(serialize($permission),'"'.$gct.'.'))){
				return true;
			}
			//带前缀的都通过
			foreach ($permission as $v) {
				if (!empty($v) && strpos("$gct.$gp",$v.'_') !== false) {
					return true;break;
				}
			}
		}
	...
	protected final function getNav($permission = '',&$top_nav,&$left_nav,&$map_nav){
		...
		foreach ($array['left'] as $k=>$v) {
			...
			foreach ($v['list'] as $key=>$value) {
				...
				$tmp_list .= str_ireplace(array('_args_','_text_','_opact_','_pkey_'),array($value['args'],$value['text'],$value['gp'].$value['gct'],$value['parent']),$model_list_parent.$model_list);
			}
	...
	protected final function sublink($links = array(), $actived = '', $file='index.php'){
		...
		foreach ($links as $k=>$v) {
			...
			$href = ($array['gp'] == $actived ? null : "href=\"{$file}?{$v['url']}\"");
			$class = ($array['gp'] == $actived ? "class=\"current\"" : null);
			$lang = L($v['lang']);
...
//			$state = $state ? L('nc_succ') : L('nc_fail');
		...
		$data['ip']			= getIp();
		$data['url']		= $_REQUEST['gct'].'&'.$_REQUEST['gp'];
