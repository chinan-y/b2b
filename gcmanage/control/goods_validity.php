...
class goods_validityControl extends SystemControl{
     ...
     * 商品管理
     */
    public function goods_validityOp() {
		...
		if ($_GET['search_serial'] !='') {
            $where['goods_serial'] = array('like', '%' . trim($_GET['search_serial']) . '%');
        }
		...
		if ((intval($_GET['search_days']) >=0) && ($_GET['search_days'] !='')) {
            $where['goods_valite_time']  = array('between',array(strtotime(date("Y-m-d")),strtotime(date("Y-m-d")) + (intval($_GET['search_days']) + 1)*3600*24));
        }
        if ($choose_gcid > 0){
		...
		if ($_GET['min_price'] !='' && $_GET['max_price'] !='') {
			$where = 'goods_price > '.$_GET['min_price'].' and goods_price < '.$_GET['max_price'];
		}
		if($_GET['type'] == 'validity'){
			$where['goods_valite_time']  = array('gt', 0);
			$order = 'goods_valite_time asc' ;
		}else{
			$order = 'goods_valite_time desc' ;
		}
        ...
        switch ($_GET['type']) {
            ...
            // 等待审核
            case 'waitverify':
                $goods_list = $model_goods->getGoodsCommonWaitVerifyList($where, '*', 10, ' goods_commonid desc');
                break;
            // 全部商品
            default:
    ...
    public function get_goods_list_ajaxOp() {
	  ...
	  public function update_goods_sortsOp() {
		  $Model = Model();
		  $update = array();
          $update['goods_sorts'] = $_GET['sorts'];
          $where = array();
          $where['goods_commonid'] = $_GET['commonid'];
		  $update_goods = $Model -> 	table('goods') -> where($where) -> update($update);
		  $update_goods = $Model -> 	table('goods_common') -> where($where) -> update($update);
	  }
