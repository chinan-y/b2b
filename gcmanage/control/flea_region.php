...
class flea_regionControl extends SystemControl{
	public function __construct(){
		...
		if ($GLOBALS['setting_config']['flea_isuse'] != 1 ){
			showMessage(Language::get('flea_index_unable'),'index.php?gct=dashboard&gp=welcome');
			// showMessage(Language::get('admin_ztc_unavailable'),'index.php?gct=dashboard&gp=welcome');
		}
	}
	/**
	 * 地区列表
	...
	public function flea_region_importOp(){
		...
		if ($_POST['form_submit'] == 'ok'){
			if (!empty($_FILES['csv'])){
				...
				showMessage($lang['region_import_succ'],'index.php?gct=flea_region&gp=flea_region');
			}else {
