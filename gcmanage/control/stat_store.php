...
class stat_storeControl extends SystemControl{
	private $links = array(
        array('url'=>'gct=stat_store&gp=newstore','lang'=>'stat_newstore'),
        array('url'=>'gct=stat_store&gp=hotrank','lang'=>'stat_storehotrank'),
        array('url'=>'gct=stat_store&gp=storesales','lang'=>'stat_storesales'),
        array('url'=>'gct=stat_store&gp=degree','lang'=>'stat_storedegree'),
        array('url'=>'gct=stat_store&gp=storearea','lang'=>'stat_storearea'),
    );
	...
	public function __construct(){
		...
		//处理搜索时间
		if (in_array($_REQUEST['gp'],array('hotrank','storesales'))){
    ...
    public function newstoreOp(){
        //导出excel连接地址
        $actionurl = 'index.php?gct=stat_store&gp=newstore';
    	$where = array();
    	//所属店铺分类
	...
	public function showstoreOp(){
		...
		if (in_array($_GET['type'],array('newbyday','newbyweek','newbymonth','storearea'))){
		    $actionurl = 'index.php?gct=stat_store&gp=showstore&type='.$_GET['type'].'&t='.$this->search_arr['t'];
    		$searchtime_arr_tmp = explode('|',$this->search_arr['t']);
    		foreach ((array)$searchtime_arr_tmp as $k=>$v){
    		    $searchtime_arr[] = intval($v);
    		}
		...
		$this->links[] = array('url'=>'gct=stat_store&gp=showstore','lang'=>'stat_storelist');
	...
	public function storesales_listOp(){
		...
		Tpl::output('actionurl',"index.php?gct={$this->search_arr['gct']}&gp={$this->search_arr['gp']}&t={$this->search_arr['t']}&search_sclass={$search_sclass}&search_sname={$_GET['search_sname']}");
		Tpl::output('show_page',$model->showpage(2));
	...
	public function storeareaOp(){
		...
		Tpl::output('stat_json',$stat_json);
		Tpl::output('statlist',$statlist);
		$actionurl = 'index.php?gct=stat_store&gp=showstore&type=storearea';
		if (trim($_GET['search_time'])){
