...
class messageControl extends SystemControl{
	private $links = array(
		array('url'=>'gct=message&gp=email','lang'=>'email_set'),
		array('url'=>'gct=message&gp=mobile','lang'=>'mobile_set'),
	    array('url'=>'gct=message&gp=seller_tpl', 'lang'=>'seller_tpl'),
	    array('url'=>'gct=message&gp=member_tpl', 'lang'=>'member_tpl'),
	    array('url'=>'gct=message&gp=email_tpl','lang'=>'email_tpl')
	);
		...
		if (chksubmit()){
			...
			if ($error != ''){
				...
				if ($result === true){
					...
					showMessage(L('mailtemplates_edit_succ'),'index.php?gct=message&gp=email_tpl');
				}else {
...
// 		}
    ...
    public function seller_tpl_editOp() {
        ...
        $this->links[] = array('url'=>'gct=message&gp=seller_tpl_edit','lang'=>'seller_tpl_edit');
    ...
    public function member_tpl_editOp() {
        ...
        $this->links[] = array('url'=>'gct=message&gp=member_tpl_edit','lang'=>'member_tpl_edit');
     ...
     * 商家消息模板更新微信消息模板
     */
    private function member_tpl_update_wechat() {
        $wechat_content = trim($_POST['wechat_content']);
        if (empty($wechat_content)) {
            showMessage('请填写微信模板内容。');
        }
        // 条件
        $where = array();
        $where['mmt_code'] = trim($_POST['code']);
        // 数据
        $update = array();
        $update['mmt_wechat_switch'] = intval($_POST['wechat_switch']);
        $update['mmt_wechat_content'] = $wechat_content;
        $result = Model('member_msg_tpl')->editMemberMsgTpl($where, $update);
        $this->member_tpl_update_showmessage($result);
    }
