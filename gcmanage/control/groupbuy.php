...
class groupbuyControl extends SystemControl{
    ...
    public function __construct(){
        ...
        if (C('groupbuy_allow') != 1){
 			$url = array(
 				array(
					'url'=>'index.php?gct=dashboard&gp=welcome',
					'msg'=>Language::get('close'),
				),
				array(
					'url'=>'index.php?gct=groupbuy&gp=groupbuy_template_list&groupbuy_open=1',
    ...
    public function class_saveOp() {
        ...
        if(empty($class_id)) {
            ...
            if($model_groupbuy_class->save($param)) {
                ...
                showMessage(Language::get('groupbuy_class_add_success'),'index.php?gct=groupbuy&gp=class_list');
            }
            else {
                showMessage(Language::get('groupbuy_class_add_fail'),'index.php?gct=groupbuy&gp=class_list');
            }
        }
        else {
            //编辑
            if($model_groupbuy_class->update($param,array('class_id'=>$class_id))) {
                ...
                showMessage(Language::get('groupbuy_class_edit_success'),'index.php?gct=groupbuy&gp=class_list');
            }
            else {
                showMessage(Language::get('groupbuy_class_edit_fail'),'index.php?gct=groupbuy&gp=class_list');
            }
        }
    ...
    public function price_saveOp() {
        ...
        if(empty($range_id)) {
            ...
            if($model->save($param)) {
                ...
                showMessage(Language::get('groupbuy_price_range_add_success'),'index.php?gct=groupbuy&gp=price_list');
            }
            else {
                showMessage(Language::get('groupbuy_price_range_add_fail'),'index.php?gct=groupbuy&gp=price_list');
            }
        }
        else {
            //编辑
            if($model->update($param,array('range_id'=>$range_id))) {
                ...
                showMessage(Language::get('groupbuy_price_range_edit_success'),'index.php?gct=groupbuy&gp=price_list');
            }
            else {
                showMessage(Language::get('groupbuy_price_range_edit_fail'),'index.php?gct=groupbuy&gp=price_list');
            }
        }
        ...
        if (chksubmit()) {
            ...
            if (!empty($_FILES['live_pic1']['name'])) {
				if(C(OSS_IS_STORAGE) == 0){
					...
					$upload->set('default_dir',ATTACH_GROUPBUY);
				...
				if(C(OSS_IS_STORAGE) == 1){
					...
					$extend 	= pathinfo($_FILES['live_pic1']['name']); 
					$extend 	= strtolower($extend["extension"]); 
					$imgname	= 'gs_'.date('His').rand(0,99);
					$imgfullname= $imgname . '.' . $extend;
					$objectname = DIR_UPLOAD.DS.ATTACH_GROUPBUY.DS.$imgfullname;
					$pathname 	= $_FILES['live_pic1']['tmp_name'];
					$result = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
					if ($result){
						$update['live_pic1'] = $imgfullname;
					}else{
						showMessage('GroupbyPic upload error!', '', '', 'error');
					}
				}
            }
            ...
            if (!empty($_FILES['live_pic2']['name'])) {
				...
				if(C(OSS_IS_STORAGE) == 1){
					...
					$oss = Logic('oss');
					$bucketname = IMAGE_BUCKET_NAME;
					$extend 	= pathinfo($_FILES['live_pic2']['name']); 
					$extend 	= strtolower($extend["extension"]); 
					$imgname	= 'gs_'.date('His').rand(0,99);
					$imgfullname= $imgname . '.' . $extend;
					$objectname = DIR_UPLOAD.DS.ATTACH_GROUPBUY.DS.$imgfullname;
					$pathname 	= $_FILES['live_pic2']['tmp_name'];
					$result = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
					if ($result){
						$update['live_pic2'] = $imgfullname;
					}else{
						showMessage('GroupbyPic upload error!', '', '', 'error');
					}
				}
            ...
            if (!empty($_FILES['live_pic3']['name'])) {
				...
				if(C(OSS_IS_STORAGE) == 1){
					...
					$oss = Logic('oss');
					$bucketname = IMAGE_BUCKET_NAME;
					$extend 	= pathinfo($_FILES['live_pic3']['name']); 
					$extend 	= strtolower($extend["extension"]); 
					$imgname	= 'gs_'.date('His').rand(0,99);
					$imgfullname= $imgname . '.' . $extend;
					$objectname = DIR_UPLOAD.DS.ATTACH_GROUPBUY.DS.$imgfullname;
					$pathname 	= $_FILES['live_pic3']['tmp_name'];
					$result = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
					if ($result){
						$update['live_pic3'] = $imgfullname;
					}else{
						showMessage('GroupbyPic upload error!', '', '', 'error');
					}
				}
            ...
            if (!empty($_FILES['live_pic4']['name'])) {
				...
				if(C(OSS_IS_STORAGE) == 1){
					...
					$oss = Logic('oss');
					$bucketname = IMAGE_BUCKET_NAME;
					$extend 	= pathinfo($_FILES['live_pic4']['name']); 
					$extend 	= strtolower($extend["extension"]); 
					$imgname	= 'gs_'.date('His').rand(0,99);
					$imgfullname= $imgname . '.' . $extend;
					$objectname = DIR_UPLOAD.DS.ATTACH_GROUPBUY.DS.$imgfullname;
					$pathname 	= $_FILES['live_pic4']['tmp_name'];
					$result = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
					if ($result){
						$update['live_pic4'] = $imgfullname;
					}else{
						showMessage('GroupbyPic upload error!', '', '', 'error');
					}
				}
            ...
            if ($result) {
				if(C(OSS_IS_STORAGE) == 0){
					if($list_setting['live_pic1'] != '' && isset($update['live_pic1'])){
						@unlink(BASE_UPLOAD_PATH.DS.ATTACH_GROUPBUY.DS.$list_setting['live_pic1']);
					}

					if($list_setting['live_pic2'] != '' && isset($update['live_pic2'])){
						@unlink(BASE_UPLOAD_PATH.DS.ATTACH_GROUPBUY.DS.$list_setting['live_pic2']);
					}

					if($list_setting['live_pic3'] != '' && isset($update['live_pic3'])){
						@unlink(BASE_UPLOAD_PATH.DS.ATTACH_GROUPBUY.DS.$list_setting['live_pic3']);
					}

					if($list_setting['live_pic4'] != '' && isset($update['live_pic4'])){
						@unlink(BASE_UPLOAD_PATH.DS.ATTACH_GROUPBUY.DS.$list_setting['live_pic4']);
					}
    ...
    private function show_menu($menu_key) {
        $menu_array = array(
            'groupbuy_list'=>array('menu_type'=>'link','menu_name'=>'抢购活动','menu_url'=>'index.php?gct=groupbuy&gp=groupbuy_list'),
            'groupbuy_quota'=>array('menu_type'=>'link','menu_name'=>'套餐管理','menu_url'=>'index.php?gct=groupbuy&gp=groupbuy_quota'),
            'class_list'=>array('menu_type'=>'link','menu_name'=>Language::get('groupbuy_class_list'),'menu_url'=>'index.php?gct=groupbuy&gp=class_list'),
            'class_add'=>array('menu_type'=>'link','menu_name'=>Language::get('groupbuy_class_add'),'menu_url'=>'index.php?gct=groupbuy&gp=class_add'),
            'price_list'=>array('menu_type'=>'link','menu_name'=>Language::get('groupbuy_price_list'),'menu_url'=>'index.php?gct=groupbuy&gp=price_list'),
            'price_add'=>array('menu_type'=>'link','menu_name'=>Language::get('groupbuy_price_add'),'menu_url'=>'index.php?gct=groupbuy&gp=price_add'),
            'price_edit'=>array('menu_type'=>'link','menu_name'=>Language::get('groupbuy_price_edit'),'menu_url'=>'index.php?gct=groupbuy&gp=price_edit'),
