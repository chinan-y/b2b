...
class store_joininControl extends SystemControl{
	...
	public function edit_infoOp() {
	    ...
	    if (chksubmit()) {
    	    ...
    	    showMessage(Language::get('nc_common_save_succ'),'index.php?gct=store_joinin&gp=edit_info');
	    }
	...
	public function edit_helpOp() {
		...
		if (chksubmit()) {
			...
			if ($state) {
				...
				showMessage(Language::get('nc_common_save_succ'),'index.php?gct=store_joinin&gp=help_list');
			} else {
	...
	public function upload_picOp() {
		...
		if (!empty($_FILES['fileupload']['name'])) {
			...
			if(C(OSS_IS_STORAGE) == 1){
				...
				$extend 	= strtolower($extend["extension"]); 
				$imgname	= 'sin_'.date('YmdHis').rand(0,999);
				$imgfullname= $imgname . '.' . $extend;
				$objectname = DIR_UPLOAD.DS.ATTACH_ARTICLE.DS.$imgfullname;
				$pathname 	= $_FILES['fileupload']['tmp_name'];
				$uploadresult = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
				$insert_array = array();
