...
class sns_malbumControl extends SystemControl{
		...
		if(chksubmit()){
			...
			foreach ($ap_list as $val){
				...
				if(C(OSS_IS_STORAGE) == 1){
					$oss = Logic('oss');
					$oss -> deleteObject(IMAGE_BUCKET_NAME, DIR_UPLOAD.DS.ATTACH_MALBUM.DS.$val['member_id'].DS.$val['ap_cover']);
				}
			}
	...
	public function del_picOp(){
		...
		if(!empty($ap_info)){
			...
			if(C(OSS_IS_STORAGE) == 1){
				$oss = Logic('oss');
				$oss -> deleteObject(IMAGE_BUCKET_NAME, DIR_UPLOAD.DS.ATTACH_MALBUM.DS.$ap_info['member_id'].DS.$ap_info['ap_cover']);
			}
