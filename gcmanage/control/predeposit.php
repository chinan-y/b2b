...
class predepositControl extends SystemControl{
	...
	public function recharge_editOp(){
		$id = intval($_GET['id']);
		if ($id <= 0){
			showMessage(Language::get('admin_predeposit_parameter_error'),'index.php?gct=predeposit&gp=predeposit','','error');
		}
		//查询充值信息
		$model_pd = Model('predeposit');
		$condition = array();
		...
		if (empty($info)){
			showMessage(Language::get('admin_predeposit_record_error'),'index.php?gct=predeposit&gp=predeposit','','error');
		}
		if (!chksubmit()) {
		    //显示支付接口列表
		...
		try {
		    ...
		    $model_pd->commit();
		    $this->log($log_msg,1);
		    showMessage(Language::get('admin_predeposit_recharge_edit_success'),'index.php?gct=predeposit&gp=predeposit');
		} catch (Exception $e) {
		    $model_pd->rollback();
		    $this->log($log_msg,0);
		    showMessage($e->getMessage(),'index.php?gct=predeposit&gp=predeposit','html','error');
		}
	}
	...
	public function recharge_delOp(){
		...
		if ($result){
			showMessage(Language::get('admin_predeposit_recharge_del_success'),'index.php?gct=predeposit&gp=predeposit');
		}else {
			showMessage(Language::get('admin_predeposit_recharge_del_fail'),'index.php?gct=predeposit&gp=predeposit','','error');
		}
	}
	...
	public function pd_log_listOp(){
		...
		if (!empty($_GET['lg_desc'])){
			$condition['lg_desc'] = trim($_GET['lg_desc']);
			$list_log = $model_pd->getPdLogList($condition,$page,'*','lg_id desc');
	...
	public function pd_cash_listOp(){
		...
		$if_pstart_date = preg_match('/^20\d{2}-\d{2}-\d{2}$/',$_GET['pstime']);
        $if_pend_date = preg_match('/^20\d{2}-\d{2}-\d{2}$/',$_GET['petime']);
        $pstart_unixtime = $if_pstart_date ? strtotime($_GET['pstime']) : null;
        $pend_unixtime = $if_pend_date ? strtotime($_GET['petime']): null;
		
        if ($start_unixtime || $end_unixtime) {
            $condition['pdc_add_time'] = array('time',array($start_unixtime,$end_unixtime));
        }
		if ($pstart_unixtime || $pend_unixtime) {
            $condition['pdc_payment_time'] = array('time',array($pstart_unixtime,$pend_unixtime));
        }
        if (!empty($_GET['mname'])){
		...
		$model_pd = Model('predeposit');
		$cash_list = $model_pd->getPdCashList($condition,20,'*','pdc_payment_state asc,pdc_id desc');
	...
	public function pd_cash_delOp(){
		...
		if ($pdc_id <= 0){
			showMessage(Language::get('admin_predeposit_parameter_error'),'index.php?gct=predeposit&gp=pd_cash_list','','error');
		}
		$model_pd = Model('predeposit');
		$condition = array();
		...
		try {
		    ...
		    $model_pd->commit();
			showMessage(Language::get('admin_predeposit_cash_del_success'),'index.php?gct=predeposit&gp=pd_cash_list');

		} catch (Exception $e) {
		    $model_pd->commit();
		    showMessage($e->getMessage(),'index.php?gct=predeposit&gp=pd_cash_list','html','error');
		}
	}
	 ...
	 * 更改提现为已审核状态
	 */
	public function pd_cash_auditOp(){
		$remark = $_POST['remark'];
	    $id = intval($_GET['id']);
	    if ($id <= 0){
	        showMessage(Language::get('admin_predeposit_parameter_error'),'index.php?gct=predeposit&gp=pd_cash_list','','error');
	    ...
	    if (!is_array($info) || count($info)<0){
	        showMessage(Language::get('admin_predeposit_record_error'),'index.php?gct=predeposit&gp=pd_cash_list','','error');
	    }
        ...
        $update = array();
        $admininfo = $this->getAdminInfo();
		$admininfo['name'] .= '&nbsp;|&nbsp;'.$remark;
        $update['pdc_payment_state'] = 2;
		$update['pdc_actual'] = $_POST['actual'];
	...
	public function export_step1Op(){
		...
		if (!is_numeric($_GET['curpage'])){
			...
			if ($count > self::EXPORT_SIZE ){	//显示下载链接
				...
				Tpl::output('murl','index.php?gct=predeposit&gp=predeposit');
	...
	public function export_cash_step1Op(){
		...
		if (!is_numeric($_GET['curpage'])){
			...
			if ($count > self::EXPORT_SIZE ){	//显示下载链接
				...
				Tpl::output('list',$array);
				Tpl::output('murl','index.php?gct=predeposit&gp=pd_cash_list');
				Tpl::showpage('export.excel');
			}else{	//如果数量小，直接下载
	...
	public function export_mx_step1Op(){
		...
		if (!is_numeric($_GET['curpage'])){
    		...
    		if ($count > self::EXPORT_SIZE ){	//显示下载链接
    			...
    			Tpl::output('list',$array);
    			Tpl::output('murl','index.php?gct=predeposit&gp=pd_log_list');
    			Tpl::showpage('export.excel');
    		}else{	//如果数量小，直接下载
	...
	public function add_money(){
			...
			if (!is_array($member_info) || count($member_info)<=0){
				showMessage(Language::get('admin_points_userrecord_error'),'index.php?gct=predeposit&gp=predeposit_add','','error');
			}
			...
			if ($_POST['operatetype'] == 2 && $money > $available_predeposit){
				showMessage(('预存款不足，会员当前预存款').$available_predeposit,'index.php?gct=predeposit&gp=predeposit_add','','error');
			}
			if ($_POST['operatetype'] == 3 && $money > $available_predeposit){
				showMessage(('可冻结预存款不足，会员当前预存款').$available_predeposit,'index.php?gct=predeposit&gp=predeposit_add','','error');
			}
			if ($_POST['operatetype'] == 4 && $money > $freeze_predeposit){
				showMessage(('可恢复冻结预存款不足，会员当前冻结预存款').$freeze_predeposit,'index.php?gct=predeposit&gp=predeposit_add','','error');
			}
			$model_pd = Model('predeposit');
			...
			switch ($_POST['operatetype'])
			{
				...
				case 4:
					...
					break;
				default:
					showMessage('操作失败','index.php?gct=predeposit&gp=pd_log_list');
					break;  
			}
			try {
				$model_pd->beginTransaction();
				...
				$model_pd->commit();
				$this->log($log_msg,1);
				showMessage('操作成功','index.php?gct=predeposit&gp=pd_log_list');
			} catch (Exception $e) {
				$model_pd->rollback();
				$this->log($log_msg,0);
				showMessage($e->getMessage(),'index.php?gct=predeposit&gp=pd_log_list','html','error');
			}
