...
class live_groupbuyControl extends SystemControl{
	...
	public function del_groupbuyOp(){
		if(isset($_POST)&&!empty($_POST)){
			...
			if($res){
				showMessage('删除抢购成功','index.php?gct=live_groupbuy','','succ');
			}else{
				showMessage('删除抢购失败','index.php?gct=live_groupbuy','','error');
			}
		}
	}
	...
	public function auditOp(){
		...
		if($res){
			...
			showMessage('审核成功','index.php?gct=live_groupbuy','','succ');
		}else{
			showMessage('审核失败','index.php?gct=live_groupbuy','','error');
		}
	}
	...
	public function cancelOp(){
		...
		if($res){
			...
			showMessage('操作成功','index.php?gct=live_groupbuy','','succ');
		}else{
			showMessage('操作失败','index.php?gct=live_groupbuy','','error');
		}
	...
	public function groupbuyvoucherOp(){
		...
		if(empty($groupbuy)){
			showMessage('抢购不存在','index.php?gct=live_groupbuy','','error');
		}
