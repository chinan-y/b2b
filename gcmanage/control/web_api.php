	    ...
	    if (!empty($file_name)) {
			if (!empty($_FILES['pic']['name'])) {
				...
				if(C(OSS_IS_STORAGE) == 1){
					...
					$newname	= 'b_'.date('YmdHis') . rand(0,9999) . '.' . $extend;
					$objectname = DIR_UPLOAD.DS.ATTACH_EDITOR.DS.$newname;
					...
					if($_FILES['pic']['type'] =='image/gif' || $_FILES['pic']['type'] =='image/jpeg' || $_FILES['pic']['type'] =='image/png'){
						...
						if ($result) {
							$pic_name = ATTACH_EDITOR."/".$newname.'?'.mt_rand(100,999);//加随机数防止浏览器缓存图片
						}
					}
				}
