...
class adminControl extends SystemControl{
	private $links = array(
		array('url'=>'gct=admin&gp=admin','lang'=>'limit_admin'),
		array('url'=>'gct=admin&gp=admin_add','lang'=>'admin_add_limit_admin'),
		array('url'=>'gct=admin&gp=gadmin','lang'=>'limit_gadmin'),
		array('url'=>'gct=admin&gp=gadmin_add','lang'=>'admin_add_limit_gadmin'),
	);
	...
	public function admin_addOp(){
		if (chksubmit()){
			...
			if ($rs){
				...
				showMessage(L('nc_common_save_succ'),'index.php?gct=admin&gp=admin');
			}else {
				showMessage(L('nc_common_save_fail'));
			}
		}
	...
	public function gadmin_setOp(){
		...
		if (chksubmit()){
			...
			if ($update){
				...
				showMessage(L('nc_common_save_succ'),'index.php?gct=admin&gp=gadmin');
			}else {
	...
	public function admin_editOp(){
		if (chksubmit()){
			...
			if ($result){
				...
				showMessage(Language::get('admin_edit_success'),'index.php?gct=admin&gp=admin');
			}else{
				showMessage(Language::get('admin_edit_fail'),'index.php?gct=admin&gp=admin');
			}
		}else{
			//查询用户信息
			$admin_model = Model('admin');
			...
			if (!is_array($admininfo) || count($admininfo)<=0){
				showMessage(Language::get('admin_edit_admin_error'),'index.php?gct=admin&gp=admin');
			}
			...
			foreach ($limit as $k=>$v) {
				if (is_array($v['child'])){
					...
					foreach ($v['child'] as $key => $value) {
						$gct = (!empty($value['gct'])) ? $value['gct'] : $v['gct'];
						if (strpos($gct,'|') == false){//act参数不带|
							$limit[$k]['child'][$key]['gp'] = rtrim($gct.'.'.str_replace('|','|'.$gct.'.',$value['gp']),'.');
						}else{//act参数带|
							$tmp_str = '';
							if (empty($value['gp'])){
								$limit[$k]['child'][$key]['gp'] = $gct;
							}elseif (strpos($value['gp'],'|') == false){//op参数不带|
								foreach (explode('|',$gct) as $v1) {
									$tmp_str .= "$v1.{$value['gp']}|";
								}
								$limit[$k]['child'][$key]['gp'] = rtrim($tmp_str,'|');
							}elseif (strpos($value['gp'],'|') != false && strpos($gct,'|') != false){//gp,act都带|，交差权限
								foreach (explode('|',$gct) as $v1) {
									foreach (explode('|',$value['gp']) as $v2) {
										$tmp_str .= "$v1.$v2|";
									}
								}
								$limit[$k]['child'][$key]['gp'] = rtrim($tmp_str,'|');
