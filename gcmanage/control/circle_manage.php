			...
			if($_POST['submit_type'] == 'batchdel'){
				...
				if(!empty($circle_list)){
					foreach ($circle_list as $val){
						...
						if(C(OSS_IS_STORAGE) == 1){
							$oss= Logic('oss');
							$oss ->deleteObject(IMAGE_BUCKET_NAME, DIR_UPLOAD.DS.ATTACH_CIRCLE.'/group/'.$circle_list['circle_id'].'.jpg');
						}
					}
				}
				// 删除附件
				...
				if(!empty($affix_list)){
					foreach ($affix_list as $val){
						...
						if(C(OSS_IS_STORAGE) == 1){
							$oss = Logic('oss');
							$oss -> deleteObject(IMAGE_BUCKET_NAME, themeImagePath($val['affix_filename']));
							$oss -> deleteObject(IMAGE_BUCKET_NAME, themeImagePath($val['affix_filethumb']));
						}
					}
		...
		if(chksubmit()){
			...
			if($error != ''){
				...
				if($rs){
					...
					showMessage(L('nc_common_op_succ'), 'index.php?gct=circle_manage&gp=circle_list');
				}else{
					showMessage(L('nc_common_op_fail'));
				}
			}
