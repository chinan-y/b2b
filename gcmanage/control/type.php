...
class typeControl extends SystemControl {
	...
	public function type_addOp(){
		...
		if (chksubmit()){
            ...
            $url = array(
                array(
                    'url'=>'index.php?gct=type&gp=type_add',
                    'msg'=>$lang['type_index_continue_to_dd']
                ),
                array(
                    'url'=>'index.php?gct=type&gp=type',
		...
		if($_POST['form_submit']){
			...
			if ($error != ''){
				...
				if($return){
					...
					showMessage($lang['type_edit_type_attr_edit_succ'],'index.php?gct=type&gp=type');
				}else{
	...
	public function export_step1Op(){
		...
		if (!is_numeric($_GET['curpage'])){
			...
			if ($count > self::EXPORT_SIZE ){	//显示下载链接
				...
				Tpl::output('murl','index.php?gct=type&gp=type');
