...
class microshopControl extends SystemControl{
    ...
    const MICROSHOP_CLASS_LIST = 'index.php?gct=microshop&gp=goodsclass_list';
    const GOODS_FLAG = 1;
...
//	}
	...
	public function manage_saveOp() {
        ...
        if(!empty($_FILES['microshop_logo']['name'])) {
			...
			if(C(OSS_IS_STORAGE) == 1){
				...
				$extend 	= pathinfo($_FILES['microshop_logo']['name']); 
				$extend 	= strtolower($extend["extension"]); 
				$imgname	= 'msl_'.date('His').rand(0,9999);
				$imgfullname= $imgname . '.' . $extend;
				$objectname = DIR_UPLOAD.DS.ATTACH_MICROSHOP.DS.$imgfullname;
				$pathname 	= $_FILES['microshop_logo']['tmp_name'];
				if($_FILES['microshop_logo']['type'] =='image/gif' || $_FILES['microshop_logo']['type'] =='image/jpeg' || $_FILES['microshop_logo']['type'] =='image/png'){
					$result = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
					if(!$result) {
						showMessage('Upload Error');
					}
					$update_array['microshop_logo'] = $imgfullname;	
				}
				$old_image[] = DIR_UPLOAD.DS.ATTACH_MICROSHOP.DS.C('microshop_logo');
			}
        }
        if(!empty($_FILES['microshop_header_pic']['name'])) {
			...
			if(C(OSS_IS_STORAGE) == 1){
				...
				$oss = Logic('oss');
				$bucketname = IMAGE_BUCKET_NAME;
				$extend 	= pathinfo($_FILES['microshop_header_pic']['name']); 
				$extend 	= strtolower($extend["extension"]); 
				$imgname	= 'msh_'.date('His').rand(0,9999);
				$imgfullname= $imgname . '.' . $extend;
				$objectname = DIR_UPLOAD.DS.ATTACH_MICROSHOP.DS.$imgfullname;
				$pathname 	= $_FILES['microshop_header_pic']['tmp_name'];
				if($_FILES['microshop_header_pic']['type'] =='image/gif' || $_FILES['microshop_header_pic']['type'] =='image/jpeg' || $_FILES['microshop_header_pic']['type'] =='image/png'){
					$result = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
					if(!$result) {
						showMessage('Upload Error');
					}
					$update_array['microshop_header_pic'] = $imgfullname;	
				}
				$old_image[] = DIR_UPLOAD.DS.ATTACH_MICROSHOP.DS.C('microshop_header_pic');
			}
        }
        ...
        if ($result === true){
            if(!empty($old_image)) {
                foreach ($old_image as $value) {
					...
					if(C(OSS_IS_STORAGE) == 1){
						$oss =Logic('oss');
						$oss->deleteObject(IMAGE_BUCKET_NAME, $value);
					}
                }
            }
    ...
    private function class_save($type) {
        ...
        if(!empty($_FILES['class_image']['name'])) {
			...
			if(C(OSS_IS_STORAGE) == 1){
				...
				$oss = Logic('oss');
				$bucketname = IMAGE_BUCKET_NAME;
				$extend 	= pathinfo($_FILES['class_image']['name']); 
				$extend 	= strtolower($extend["extension"]); 
				$imgname	= 'msh_'.date('His').rand(0,9999);
				$imgfullname= $imgname . '.' . $extend;
				$objectname = DIR_UPLOAD.DS.ATTACH_MICROSHOP.DS.$imgfullname;
				$pathname 	= $_FILES['class_image']['tmp_name'];
				if($_FILES['class_image']['type'] =='image/gif' || $_FILES['class_image']['type'] =='image/jpeg' || $_FILES['class_image']['type'] =='image/png'){
					$result = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
					if(!$result) {
						showMessage('Upload Error');
					}
					$param['class_image'] = $imgfullname;	
					//删除老图片
					if(!empty($_POST['old_class_image'])) {
						$old_image = DIR_UPLOAD.DS.ATTACH_MICROSHOP.DS.$_POST['old_class_image'];
						$oss = Logic('oss');
						$oss ->deleteObject(IMAGE_BUCKET_NAME, $old_image);
					}	
				}
			}
        }
        ...
        if($result) {
            showMessage(Language::get('class_add_success'),"index.php?gct=microshop&gp={$type}class_list");
        } else {
            showMessage(Language::get('class_add_fail'),"index.php?gct=microshop&gp={$type}class_list",'','error');
        }
        ...
        if(!empty($list)) {
            foreach ($list as $value) {
                if(!empty($value['class_image'])) {
					...
					if(C(OSS_IS_STORAGE) == 1){
						$old_image = DIR_UPLOAD.DS.ATTACH_MICROSHOP.DS.$value['class_image'];
						$oss =Logic('oss');
						$oss ->deleteObject(IMAGE_BUCKET_NAME, $old_image);
					}
                }
        ...
        if(!empty($list)) {
            foreach ($list as $personal_info) {
                ...
                foreach ($image_array as $value) {
					...
					if(C(OSS_IS_STORAGE) == 1){
						$image_name = DIR_UPLOAD.DS.ATTACH_MICROSHOP.DS.$personal_info['commend_member_id'].DS.$value;
						$oss =Logic('oss');
						$oss ->deleteObject(IMAGE_BUCKET_NAME, $image_name);
					}
                }
            }
        }
    ...
    public function adv_saveOp() {
        ...
        if(!empty($_FILES['adv_image']['name'])) {
			...
			if(C(OSS_IS_STORAGE) == 1){
				...
				$oss = Logic('oss');
				$bucketname = IMAGE_BUCKET_NAME;
				$extend 	= pathinfo($_FILES['adv_image']['name']); 
				$extend 	= strtolower($extend["extension"]); 
				$imgname	= 'msa_'.date('His').rand(0,9999);
				$imgfullname= $imgname . '.' . $extend;
				$objectname = DIR_UPLOAD.DS.ATTACH_MICROSHOP.DS.'adv'.DS.$imgfullname;
				$pathname 	= $_FILES['adv_image']['tmp_name'];
				if($_FILES['adv_image']['type'] =='image/gif' || $_FILES['adv_image']['type'] =='image/jpeg' || $_FILES['adv_image']['type'] =='image/png'){
					$result = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
					if(!$result) {
						showMessage('Upload Error');
					}
					$param['adv_image'] = $imgfullname;
				}
			}
            ...
            if(!empty($_POST['old_adv_image'])) {
				...
				if(C(OSS_IS_STORAGE) == 1){
					$old_image = DIR_UPLOAD.DS.ATTACH_MICROSHOP.DS.'adv'.DS.$_POST['old_adv_image'];
					$oss =Logic('oss');
					$oss ->deleteObject(IMAGE_BUCKET_NAME, $old_image);
				}
            }
        ...
        if($result) {
            showMessage(Language::get('nc_common_save_succ'),"index.php?gct=microshop&gp=adv_manage");
        } else {
            showMessage(Language::get('nc_common_save_fail'),"index.php?gct=microshop&gp=adv_manage",'','error');
        }
    }
    ...
    public function adv_dropOp() {
        ...
        if(!empty($list)) {
            foreach ($list as $adv_info) {
				...
				if(C(OSS_IS_STORAGE) == 1){
					$old_image = DIR_UPLOAD.DS.ATTACH_MICROSHOP.DS.'adv'.DS.$adv_info['adv_image'];
					$oss =Logic('oss');
					$oss ->deleteObject(IMAGE_BUCKET_NAME, $old_image);
				}
            }
    ...
    private function show_menu($menu_key) {
        $menu_array = array(
            "{$menu_key}"=>array('menu_type'=>'link','menu_name'=>Language::get('nc_manage'),'menu_url'=>'index.php?gct=microshop&gp='.$menu_key),
        );
    ...
    private function show_menu_goods_class($menu_key) {
        $menu_array = array(
            'goods_class_list'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_manage'),'menu_url'=>'index.php?gct=microshop&gp=goodsclass_list'),
            'goods_class_add'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_new'),'menu_url'=>'index.php?gct=microshop&gp=goodsclass_add'),
        );
    ...
    private function show_menu_personal_class($menu_key) {
        $menu_array = array(
            'personal_class_list'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_manage'),'menu_url'=>'index.php?gct=microshop&gp=personalclass_list'),
            'personal_class_add'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_new'),'menu_url'=>'index.php?gct=microshop&gp=personalclass_add'),
        );
    ...
    private function show_menu_store($menu_key) {
        $menu_array = array(
            'store_manage'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_manage'),'menu_url'=>'index.php?gct=microshop&gp=store_manage'),
            'store_add'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_new'),'menu_url'=>'index.php?gct=microshop&gp=store_add'),
        );
        $menu_array[$menu_key]['menu_type'] = 'text';
        Tpl::output('menu',$menu_array);
    }
    ...
    private function show_menu_adv($menu_key) {
        $menu_array = array(
            'adv_manage'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_manage'),'menu_url'=>'index.php?gct=microshop&gp=adv_manage'),
            'adv_add'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_new'),'menu_url'=>'index.php?gct=microshop&gp=adv_add'),
        );
