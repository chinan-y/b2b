...
class circle_informControl extends SystemControl{
	...
	public function spellInformUrl($param){
		if($param['reply_id'] == 0) return $url = 'index.php?gct=theme&gp=theme_detail&c_id='.$param['circle_id'].'&t_id='.$param['theme_id'];
		...
		$page = ceil($count/15);
		return $url = 'index.php?gct=theme&gp=theme_detail&c_id='.$param['circle_id'].'&t_id='.$param['theme_id'].'&curpage='.$page.'#f'.$param['reply_id'];
	}
	/**
	 * Inform state
	 */
