...
class cms_specialControl extends SystemControl{
    ...
    const LINK_CMS_SPECIAL = 'index.php?gct=cms_special&gp=cms_special_list';
    //专题状态草稿箱
    ...
    private function cms_special_image_upload($image) {
        if(!empty($_FILES[$image]['name'])) {
			...
			if(C(OSS_IS_STORAGE) == 1){
				...
				$imgname	= 'cmi_'.date('His').rand(0,9999);
				$imgfullname= $imgname . '.' . $extend;
				$objectname = DIR_UPLOAD.DS.ATTACH_CMS.DS.'special'.DS.$imgfullname;
    ...
    public function special_image_uploadOp() {
        ...
        if(!empty($_FILES['special_image_upload']['name'])) {
			...
			if(C(OSS_IS_STORAGE) == 1){
				...
				$oss = Logic('oss');
				$bucketname = IMAGE_BUCKET_NAME;
				$extend 	= pathinfo($_FILES['special_image_upload']['name']); 
				$extend 	= strtolower($extend["extension"]); 
				$imgname	= 'cms_'.date('His').rand(0,9999);
				$imgfullname= $imgname . '.' . $extend;
				$objectname = DIR_UPLOAD.DS.ATTACH_CMS.DS.'special'.DS.$imgfullname;
				$pathname 	= $_FILES['special_image_upload']['tmp_name'];
				$result = $oss->uploadFile($bucketname, $objectname, $pathname, $options);
				if(!$result) {
					$data['status'] = 'fail';
					$data['error'] = 'Upload Error';
				...
				$data['origin_file_name'] = $_FILES['special_image_upload']['name'];
				$data['file_url'] = getCMSSpecialImageUrl($imgfullname);
			}
    ...
    public function goods_infoOp() {
        if(isset($_POST['keyword']) && !isset($_GET['keyword'])){
            $_GET['keyword'] = trim($_POST['keyword']);
            $_SERVER['REQUEST_URI'] .= '&keyword=' . trim($_GET['keyword']);
        }
        $model_goods = Model('goods');
        $condition = array();
        //$condition['goods_promotion_type'] = $model_goods::PROMOTION_TYPE_NULL;
        $keyword = $_GET['keyword'];
        if($keyword){
            $condition['goods_name'] = array('like', '%' . $keyword . '%');
        }

        $goods_list = $model_goods->getGoodsOnlineList($condition, 'goods_id,goods_commonid,goods_name,goods_price,goods_rebate_rate,goods_promotion_price,goods_promotion_type,goods_marketprice,goods_image', 5);

        Tpl::output('goods_list', $goods_list);
        Tpl::output('show_page', $model_goods->showpage());
        Tpl::showpage('cms_special_widget.goods', 'null_layout');
    }
    ...
    private function show_menu($menu_key) {
        $menu_array = array(
            'special_list'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_manage'),'menu_url'=>'index.php?gct=cms_special&gp=cms_special_list'),
            'special_add'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_new'),'menu_url'=>'index.php?gct=cms_special&gp=cms_special_add'),
