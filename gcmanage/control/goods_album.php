...
class goods_albumControl extends SystemControl{
	...
	public function del_album_picOp(){
		list($goods_commonid,$apic_id,$filename) = @explode('|',$_GET['key']);
		$goods_commonid = intval($goods_commonid);
		if (!is_numeric($apic_id) || empty($filename)) exit('0');
		$this->del_file($filename, $goods_commonid);
	 ...
	 *
	 */
	private function del_file($filename, $goods_commonid){
		//取店铺ID
		...
		if(C(OSS_IS_STORAGE) == 0){
			if (!file_exists(BASE_UPLOAD_PATH.'/'.ATTACH_GOODS.'/'.$store_id.'/'.$goods_commonid.'/'.$filename)){
				$path = BASE_UPLOAD_PATH.'/'.ATTACH_GOODS.'/'.$store_id.'/'.$filename;
			}else{
				$path = BASE_UPLOAD_PATH.'/'.ATTACH_GOODS.'/'.$store_id.'/'.$goods_commonid.'/'.$filename;
			}
			
			if (is_file($path)) unlink($path);
			
			$image_ext = explode(',', GOODS_IMAGES_EXT);
			$path1 = substr($path,0,strpos($path, '.'));
			$ext = strrchr($path, '.');
			
			foreach ($image_ext as $v) {
				if (is_file($fpath = $path1.$v.$ext)){
					unlink($fpath);
				}
			}
		}
		if(C(OSS_IS_STORAGE) == 1){
			$oss = Logic('oss');
			$path = DIR_UPLOAD.DS.ATTACH_GOODS.DS.$store_id.DS.$goods_commonid.DS.$filename;
			$oss->deleteObject(IMAGE_BUCKET_NAME, $path);
