...
class mb_adControl extends SystemControl{
	...
	public function mb_ad_delOp(){
		...
		if ($link_id > 0){
			...
			showMessage(L('link_index_del_succ'),'index.php?gct=mb_ad&gp=mb_ad_list');
		}else {
			showMessage(L('link_index_choose_del'),'index.php?gct=mb_ad&gp=mb_ad_list');
		}
	}
		...
		if ($_POST['form_submit'] == 'ok'){
			...
			if ($error != ''){
				...
				if ($result){
					$url = array(
						array(
							'url'=>'index.php?gct=mb_ad&gp=mb_ad_list',
