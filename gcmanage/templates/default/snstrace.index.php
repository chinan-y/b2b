  ...
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base">
        ...
        <li><a href="index.php?gct=snstrace&gp=commentlist"><span><?php echo $lang['admin_snstrace_commentlist'];?></span></a></li>
      </ul>
    </div>
  </div>
  <div class="fixed-empty"></div>
  <form method="get" name="formSearch">
    <input type="hidden" name="gct" value="snstrace">
    <input type="hidden" name="gp" value="tracelist">
      ...
      <tbody>
        ...
        <tr class="hover edit">
          ...
          <td class="fd-list"><!-- 动态列表start -->
            ...
            <li>
              <div class="fd-aside"> <span class="thumb size60"><a href="<?php echo SHOP_SITE_URL.DS;?>index.php?gct=member_snshome&mid=<?php echo $v['trace_memberid'];?>" target="_blank"> <img src="<?php echo getMemberAvatarForID($v['trace_memberid']);?>" onload="javascript:DrawImage(this,60,60);"> </a> </span> </div>
              <dl class="fd-wrap">
                <dt>
                  <h3><a href="<?php echo SHOP_SITE_URL.DS;?>index.php?gct=member_snshome&mid=<?php echo $v['trace_memberid'];?>" target="_blank"><?php echo $v['trace_membername'];?></a><?php echo $lang['nc_colon'];?></h3>
                ...
                <dd>
                  ...
                  <div class="fd-forward">
                    ...
                    <div class="stat"> <span><?php echo $lang['admin_snstrace_forward'];?><?php echo $v['trace_orgcopycount']>0?"({$v['trace_orgcopycount']})":''; ?></span>&nbsp;&nbsp; <span><a href="index.php?gct=snstrace&gp=commentlist&tid=<?php echo $v['trace_originalid'];?>"><?php echo $lang['admin_snstrace_comment']; ?><?php echo $v['trace_orgcommentcount']>0?"({$v['trace_orgcommentcount']})":''; ?></a></span> </div>
                  	<?php }?>
                  </div>
                  <?php } else {?>
                  <?php echo parsesmiles($v['trace_content']);?>
                ...
                <dd> <span class="fc-time fl"><?php echo date('Y-m-d H:i',$v['trace_addtime']);?></span> <span class="fr"><?php echo $lang['admin_snstrace_forward'];?><?php echo $v['trace_copycount']>0?"({$v['trace_copycount']})":'';?>&nbsp;|&nbsp;<a href="index.php?gct=snstrace&gp=commentlist&tid=<?php echo $v['trace_id'];?>"><?php echo $lang['admin_snstrace_comment']; ?><?php echo $v['trace_commentcount']>0?"({$v['trace_commentcount']})":'';?></a></span> <span class="fl">&nbsp;&nbsp;<?php echo $lang['admin_snstrace_state'].$lang['nc_colon'];?><?php echo $v['trace_state']==1?"<font style='color:red;'>{$lang['admin_snstrace_statehide']}</font>":"{$lang['admin_snstrace_stateshow']}";?></span> <span class="fl">&nbsp;&nbsp;<?php echo $lang['admin_snstrace_privacytitle'].$lang['nc_colon'];?>
...
function submit_form(type){
	if(type=='del'){
		...
		$('#form_trace').attr('action','index.php?gct=snstrace&gp=tracedel');
	}else if(type=='hide'){
		$('#form_trace').attr('action','index.php?gct=snstrace&gp=traceedit&type=hide');
	}else{
		$('#form_trace').attr('action','index.php?gct=snstrace&gp=traceedit&type=show');
	}
	$('#form_trace').submit();
}
</script>