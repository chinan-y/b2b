...
$(document).ready(function(){
	...
	if($.cookie('now_location_act') != null){
		...
		$('#workspace').attr('src','index.php?gct=dashboard&gp=welcome');
	}
...
function openItem(args){
	...
	if($.cookie('<?php echo COOKIE_PRE?>sys_key') === null){
		location.href = 'index.php?gct=login&gp=login';
		return false;
	}
	...
	if (typeof(gct)=='undefined'){var nav = args;}
	...
	if (typeof(gct)=='undefined'){
		...
		$.cookie('now_location_act',gct);
		$.cookie('now_location_op',gp);
		$("a[name='item_"+gp+gct+"']").addClass('selected');
		//crumbs
		$('#crumbs').html('<span>'+$('#nav_'+nav+' > span').html()+'</span><span class="arrow">&nbsp;</span><span>'+$('#item_'+gp+gct).html()+'</span>');
	}
	src = 'index.php?gct='+gct+'&gp='+gp;
...
$(function(){
	...
	$('.lang dd').click(function(){
		var lang=$(this).attr('nctype');
		$.ajax({
			url:'index.php?gct=index&gp=setLang',
			type:'post',
			dataType:'json',
			data:{language:lang},
			success:function(re){
    ...
    <tr>
      <td colspan="2" height="90" class="mainhd"><div class="layout-header"> <!-- Title/Logo - can use text instead of image -->
          ...
          <div id="topnav" class="top-nav">
            <ul>
			<?php if(LANG_TYPE){ ?>
			  <li class='lang' style='margin-right:15px; cursor: pointer'><a>语言切换</a>
				<?php foreach($output['lang'] as $key=>$value){ ?>
				<?php if($value['status']){?>
				<dd style='display:none'  nctype='<?php echo $key; ?>'><?php echo $value['name']; ?></dd>
				<?php } ?>
				<?php } ?>
			  </li>
			<?php } ?>
          ...
          <div class="toolbar">
            ...
            <div class="toolmenu"><span class="bar-btn"></span>
              <ul class="bar-list">
                ...
                <!--//zmr>v30-->
                <li><a href="index.php?gct=setting&gp=exetarget" target="_blank">执行计划任务</a></li>
              </ul>
            </div>
          </div>
        </div>
