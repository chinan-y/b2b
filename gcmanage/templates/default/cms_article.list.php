...
$(document).ready(function(){
    //行内ajax编辑
    $('span[nc_type="article_sort"]').inline_edit({gct: 'cms_article',gp: 'update_article_sort'});
    $('span[nc_type="article_click"]').inline_edit({gct: 'cms_article',gp: 'update_article_click'});
...
function submit_delete(id){
    if(confirm('<?php echo $lang['nc_ensure_del'];?>')) {
        $('#list_form').attr('action','index.php?gct=cms_article&gp=cms_article_drop');
        $('#article_id').val(id);
        $('#list_form').submit();
    }
}
...
function submit_callback(id){
    if(confirm('<?php echo $lang['cms_ensure_callback'];?>')) {
        $('#list_form').attr('action','index.php?gct=cms_article&gp=cms_article_callback');
        $('#article_id').val(id);
        $('#list_form').submit();
    }
}
...
<div class="page">
  ...
  <form method="get" name="formSearch">
    <input type="hidden" value="cms_article" name="gct">
    <input type="hidden" value="<?php echo $_GET['gp'];?>" name="gp">
    <table class="tb-type1 noborder search">
      <tbody>
        <tr>
          ...
          <?php if($_GET['gp'] == 'cms_article_list') { ?>
      ...
      <tbody>
        ...
        <tr class="hover edit">
          ...
          <td class="w108 align-center">
              <a href="<?php echo CMS_SITE_URL.'/index.php?gct=article&gp=article_detail&article_id='.$val['article_id'];?>" target="_blank"><?php echo $lang['cms_text_see'];?></a>
...
<div id="dialog_verify" style="display:none;">
    <form id="verify_form" method='post' action="index.php?gct=cms_article&gp=cms_article_verify">
