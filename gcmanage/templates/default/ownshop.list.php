  ...
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base">
        ...
        <li><a href="index.php?gct=ownshop&gp=add"><span>新增</span></a></li>
      </ul>
    </div>
  </div>
  <div class="fixed-empty"></div>
  <form method="get" name="formSearch" id="formSearch">
  <input type="hidden" name="gct" value="ownshop" />
...
<?php if ($output['store_name'] != '') { ?>
        <a href="index.php?gct=ownshop&gp=list" class="btns" title="<?php echo $lang['nc_cancel_search'];?>"><span><?php echo $lang['nc_cancel_search'];?></span></a>
<?php } ?>
      </td>
    </tr>
  </tbody>
...
<?php foreach($output['store_list'] as $k => $v) { ?>
        <tr class="">
          ...
          <td class="align-center w200">
            <a href="index.php?gct=ownshop&gp=edit&id=<?php echo $v['store_id']; ?>">编辑</a>
<?php if (!$v['bind_all_gc']) { ?>
            |
            <a href="index.php?gct=ownshop&gp=bind_class&id=<?php echo $v['store_id']; ?>">经营类目</a>
<?php } ?>
<?php if (empty($output['storesWithGoods'][$v['store_id']])) { ?>
            |
            <a href="index.php?gct=ownshop&gp=del&id=<?php echo $v['store_id']; ?>" onclick="return confirm('此操作不可逆转！确定删除？');">删除</a>
<?php } ?>
          </td>
        </tr>
<?php } ?>
