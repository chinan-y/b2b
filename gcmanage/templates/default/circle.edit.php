...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base">
        <li><a href="index.php?gct=circle_manage&gp=circle_list"><span><?php echo $lang['nc_manage'];?></span></a></li>
        <li><a href="index.php?gct=circle_manage&gp=circle_verify"><span><?php echo $lang['circle_wait_verify'];?></span></a></li>
        <li><a href="index.php?gct=circle_manage&gp=circle_add"><span><?php echo $lang['nc_new'];?></span></a></li>
...
$(function(){
	...
	function ajaxFileUpload()
	{
		...
		(
			{
				url:'index.php?gct=common&gp=pic_upload&form_submit=ok&uploadpath=<?php echo ATTACH_CIRCLE;?>/group',
				secureuri:false,
				fileElementId:'_pic',
				dataType: 'json',
				...
				{
					if (data.status == 1){
						ajax_form('cutpic','<?php echo $lang['nc_cut'];?>','index.php?gct=common&gp=pic_cut&type=circle&x=120&y=120&resize=1&ratio=1&filename=<?php echo UPLOAD_SITE_URL.'/'.ATTACH_CIRCLE;?>/group/<?php echo $_GET['c_id'];?>.jpg&url='+data.url,690);
					}else{
						alert(data.msg);
					}
	...
	$('#mastername').focus(function(){
		ajax_form('choose_master', '<?php echo $lang['circle_choose_master'];?>', 'index.php?gct=circle_manage&gp=choose_master&c_id=<?php echo $output['circle_info']['circle_id'];?>', 320);
	});
	...
	$('#circle_form').validate({
        ...
        rules : {
        	c_name : {
        		...
        		maxlength : 12,
            	remote : {
            		url : 'index.php?gct=circle_manage&gp=check_circle_name&id=<?php echo $output['circle_info']['circle_id'];?>',
                    type: 'get',
                    data:{
                    	name : function(){
                            return $('#c_name').val();
            ...
            mastername : {
            	required : true,
                remote   : {
                    url :'index.php?gct=circle_manage&gp=check_member&c_id=<?php echo $output['circle_info']['circle_id'];?>&branch=ok',
                    type:'get',
                    data:{
                        name : function(){
