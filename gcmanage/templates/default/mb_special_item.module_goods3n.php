...
<table class="table tb-type2" id="prompt" style="background-color: #FFF; border-bottom: solid 1px #deeffb">
  ...
  <?php } ?>
<div class="index_block goods3n-list">
  <?php if($item_edit_flag) { ?>
  <h3>商品版块3*N</h3>
  <?php } ?>
  <div class="title">
  ...
  <div nctype="item_content" class="content">
	...
	<?php if(!is_array($item_value)) continue; ?>
    <div nctype="item_image" class="item">
      <div class="goods3n-pic"><img nctype="goods_image" src="<?php echo cthumb($item_value['goods_image'], 240, $item_value['store_id'], $item_value['goods_commonid']);?>" alt="<?php echo $item_value['goods_name'];?>"></div>
      <div class="goods3n-name" nctype="goods_name"><?php echo $item_value['goods_name'];?></div>
      <div class="goods3n-price" nctype="goods_price">￥<?php echo $item_value['goods_promotion_price'];?>
			<span class="goods3n-item-zi"><del>￥<?php echo $item_value['goods_marketprice'];?><del/><span/>
	  </div>
...
<script id="item_goods_template" type="text/html">
    <div nctype="item_image" class="item">
        <div class="goods-pic"><img nctype="image" src="<%=goods_image%>" alt="<%=goods_name%>"></div>
