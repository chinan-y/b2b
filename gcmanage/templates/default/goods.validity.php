...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base">
        <li><a href="index.php?gct=goods_validity&gp=goods_validity" ><span><?php echo $lang['goods_index_all_goods'];?></span></a></li>
		<li><a href="index.php?gct=goods_validity&gp=goods_validity&type=validity" ><span><?php echo '有效期商品';?></span></a></li>
      </ul>
    </div>
  </div>
  <div class="fixed-empty"></div>
  <form method="get" name="formSearch" id="formSearch">
    <input type="hidden" name="gct" value="goods_validity">
    <input type="hidden" name="gp" value="goods_validity">
    <table class="tb-type1 noborder search">
      <tbody>
        <tr>
		  ...
		  <th><label for="search_serial">商品货号</label></th>
          <td><input type="text" value="<?php echo $output['search']['search_serial']?>" name="search_serial" id="search_serial" class="txt" /></td>
		  ...
		  <th><label for="search_days"></label>有效天数</th>
		  <td><input type="text" value="<?php echo $output['search']['search_days']?>" name="search_days" id="search_days" class="txt" /></td>
		  <th><label></label>天以内</th>
        </tr>
        <tr>
          ...
          <td>
            <div id="ajax_brand" class="ncsc-brand-select w180">
                  ...
                  <div class="ncsc-brand-select-container">
                    <div class="brand-index" data-url="index.php?gct=common&gp=ajax_get_brand">
         ...
         <th><label>等待审核</label></th>
		  ...
		  <td><input type="text" value="<?php echo $output['search']['min_price']?>" name="min_price" id="min_price" style="width: 55px;"/>到<input type="text" value="<?php echo $output['search']['max_price']?>" name="max_price" id="max_price" style="width: 55px;"/></td>
    ...
    <table class="table tb-type2">
      <thead>
        <tr class="thead">
		  ...
		  <th class="w60 align-center">店铺名称</th>
          <th class="w60 align-center">商品货号</th>
          <th colspan="1"><?php echo $lang['goods_index_name'];?></th>
		  <th class="w72 align-center">有效日期</th>
		  <th class="w72 align-center">有效天数</th>
		  <th class="w72 align-center">预警时间</th>
      ...
      <tbody>
        ...
        <tr class="hover edit">
          ...
          <td class="align-left">
		  <?php if($v['goods_valite_time']==0) {echo "<font color=red>未设置</font>";}?>
		  <?php if($v['goods_valite_time']>0) {echo date('Y-m-d',$v['goods_valite_time']);}?>
		  </td>
		  <td class="align-center">
		  <?php if ($v['goods_valite_time']-time()>270*3600*24) {echo "<font color=green>".round(($v['goods_valite_time']-time())/3600/24)."天</font>";}?>
		  <?php if ($v['goods_valite_time']-time()>120*3600*24 && $v['goods_valite_time']-time()<=270*3600*24 ) {echo "<font color=orange>".round(($v['goods_valite_time']-time())/3600/24)."天</font>";}?>
		  <?php if ($v['goods_valite_time']-time()>90*3600*24 && $v['goods_valite_time']-time()<=120*3600*24 ) {echo "<font color=#f77>".round(($v['goods_valite_time']-time())/3600/24)."天</font>";}?>
		  <?php if ($v['goods_valite_time']-time()>60*3600*24 && $v['goods_valite_time']-time()<=90*3600*24 ) {echo "<font color=#f55>".round(($v['goods_valite_time']-time())/3600/24)."天</font>";}?>
		  <?php if ($v['goods_valite_time']-time()>30*3600*24 && $v['goods_valite_time']-time()<=60*3600*24 ) {echo "<font color=#f33>".round(($v['goods_valite_time']-time())/3600/24)."天</font>";}?>
		  <?php if ($v['goods_valite_time']-time()>=0 && $v['goods_valite_time']-time()<=30*3600*24 ) {echo "<font color=#f00>".round(($v['goods_valite_time']-time())/3600/24)."天</font>";}?>
		  <?php if ($v['goods_valite_time']-time()<0) {echo "<font color=#f00><b>已过期</b></font>";}?>
		  </td>
		  <td class="align-center">
		  <?php if($v['goods_alarm_time']==0) {echo "<font color=red>未设置</font>";}?>
		  <?php if($v['goods_alarm_time']>0) {echo date('Y-m-d H:i:s',$v['goods_alarm_time']);}?>
		  </td>
		  <td>
...
$(function(){
    ...
    $('#ncsubmit').click(function(){
        $('input[name="gp"]').val('goods_validity');$('#formSearch').submit();
    });
    ...
    $('i[nctype="ajaxGoodsList"]').toggle(
        function(){
            ...
            if (_div.html() == '') {
                $.getJSON('index.php?gct=goods&gp=get_goods_list_ajax' , {commonid : _commonid}, function(date){
                    if (date != 'false') {
                        var _ul = $('<ul class="ncsc-goods-sku-list"></ul>');
                        $.each(date, function(i, o){
                            $('<li><div class="goods-thumb" title="商品货号：' + o.goods_serial + '"><a href="' + o.url + '" target="_blank"><image src="' + o.goods_image + '" ></a></div>' + o.goods_spec + '<div class="goods-price">价格：<em title="￥' + o.goods_price + '">￥' + o.goods_price + '</em></div><div class="goods-storage">库存：<em title="' + o.goods_storage + '">' + o.goods_storage + '</em></div><a href="' + o.url + '" target="_blank" class="ncsc-btn-mini">查看商品详情</a></li>').appendTo(_ul);
                            });
                        _ul.appendTo(_div);
                        _parenttr.next().show();
                        // 计算div的宽度
...
function goods_lockup(ids) {
    _uri = "<?php echo ADMIN_SITE_URL;?>/index.php?gct=goods&gp=goods_lockup&id=" + ids;
...
<script type="text/javascript">
    function  getVal(sorts, commonid){
      $.getJSON('index.php?gct=goods&gp=update_goods_sorts' , {sorts : sorts, commonid:commonid}, function(date){
       });
    }
</script>