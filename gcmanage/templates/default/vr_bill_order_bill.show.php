...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      <h3>虚拟订单结算</h3>
		<ul class="tab-base">
		<li><a href="index.php?gct=vr_bill"><span>结算管理</span></a></li>
		<li><a href="index.php?gct=vr_bill&gp=show_statis&os_month=<?php echo $output['bill_info']['os_month'];?>"><span><?php echo $output['bill_info']['os_month'];?>期  商家账单列表</span></a></li>
...
<div class="fixed-empty"></div>  
    <table class="table tb-type2 noborder search">
      ...
      <a class="btns" onclick="if (confirm('审核后将无法撤销，进入下一步付款环节，确认审核吗?')){return true;}else{return false;}" href="index.php?gct=vr_bill&gp=bill_check&ob_no=<?php echo $_GET['ob_no'];?>"><span><?php echo $lang['nc_exdport'];?>审核</span></a>
       <?php }elseif ($output['bill_info']['ob_state'] == BILL_STATE_SYSTEM_CHECK){?>
		<a class="btns" href="index.php?gct=vr_bill&gp=bill_pay&ob_no=<?php echo $_GET['ob_no'];?>"><span><?php echo $lang['nc_exdport'];?>付款完成</span></a>
       <?php }elseif ($output['bill_info']['ob_state'] == BILL_STATE_SUCCESS){?>
      <a class="btns" target="_blank" href="index.php?gct=vr_bill&gp=bill_print&ob_no=<?php echo $_GET['ob_no'];?>"><span><?php echo $lang['nc_exposrt'];?>打印</span></a>
      <?php }?>
      </td></tr>
...
$(function(){
    $('#ncsubmit').click(function(){
    	$('input[name="gp"]').val('show_bill');$('#formSearch').submit();
    });
});
</script> 
