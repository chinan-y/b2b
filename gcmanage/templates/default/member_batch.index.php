...
<div class="page">
  ...
  <form method="get" name="formSearch" id="formSearch">
    <input type="hidden" value="member_batch" name="gct">
    <input type="hidden" value="member" id="gp" name="gp">
    <table class="tb-type1 noborder search">
      <tbody>
        <tr>
          ...
          <td><select name="search_saleplat" >
              <option value='-1'>选择注册来源</option>
              <?php if ($output['member_saleplat']){?>
                <?php foreach ($output['member_saleplat'] as $k=>$v){?>
                  <option <?php if(isset($_GET['search_saleplat']) && $_GET['search_saleplat'] == $k){ ?>selected='selected'<?php } ?> value="<?php echo $k;?>"><?php echo $v;?></option>
                <?php }?>
              <?php }?>
            </select></td>
        ...
        <tr>
          <td colspan="7">
            发送类型：
            <label><input type="radio" checked="checked" name="type" id="type" value="mail">邮件</label>
            <label><input type="radio" name="type" id="type" value="message">站内消息</label>
            <label><input type="radio" name="type" id="type" value="short">短信</label>&nbsp;&nbsp;&nbsp;&nbsp;
            模版(设置|消息设置|用户消息模版)：
            <select name="code" id="code">
              <?php foreach($output['mmtpl_list'] as $v){ ?>
                <option value="<?php echo $v['mmt_code']; ?>"><?php echo $v['mmt_name']; ?></option>
              <?php } ?>
            </select>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <a href="JavaScript:void(0);" class="btn" onclick="if(confirm('<?php echo $lang['batch_ensure_send']?>')){$('#gp').val('batch_result');$('#formSearch').submit();}"><span>批量发送到结果集</span></a>
          </td>
        </tr>
    ...
    <table class="table tb-type2 nobdb">
      <thead>
        <tr class="thead">
          ...
          <th class="align-center"><span fieldname="logins" nc_type="order_by">引荐人ID</span></th>
      ...
      <tbody>
        ...
        <tr class="hover member">
          ...
          <td class="align-center"><?php if($v['inviter_id']>0){echo $v['inviter_id']." | ";} ?><?php if($v['refer_id']>0){echo $v['refer_id'];} ?></td>
          ...
          <td class="align-center"><?php echo $output['member_saleplat'][$v['saleplat_id']];?></td>
      ...
      <tfoot class="tfoot">
        ...
        <td class="w24"><input type="checkbox" class="checkall" id="checkallBottom"></td>
          <td colspan="16">
            ...
            <a href="JavaScript:void(0);" class="btn" onclick="if(confirm('<?php echo $lang['batch_ensure_send']?>')){$('#form_member').submit();}"><span>勾选指定用户发送消息</span></a>
