...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base">
        <li><a href="index.php?gct=pointprod&gp=pointprod" ><span><?php echo $lang['admin_pointprod_list_title'];?></span></a></li>
        <li><a href="index.php?gct=pointprod&gp=prod_add" ><span><?php echo $lang['admin_pointprod_add_title'];?></span></a></li>
  ...
  <form method="get" name="formSearch">
    <input type="hidden" name="gct" value="pointorder">
    <input type="hidden" name="gp" value="pointorder_list">
  ...
  <form method='post' id="form_order" action="index.php">
    <input type="hidden" name="gct" value="pointorder">
    <input type="hidden" id="list_op" name="gp" value="order_dropall">
      ...
      <tbody>
        ...
        <tr class="hover">
          ...
          <td class="w150 align-center">
            <a href="index.php?gct=pointorder&gp=order_info&order_id=<?php echo $v['point_orderid']; ?>" class="edit"><?php echo $lang['nc_view']; ?></a>
            <?php if ($v['point_orderallowship']) {//发货（已确认付款，待发货）?>
            <a href="index.php?gct=pointorder&gp=order_ship&id=<?php echo $v['point_orderid']; ?>"><?php echo $lang['admin_pointorder_ship_title']; ?></a>
            <?php } ?>
            <?php if ($v['point_orderalloweditship']) {//修改物流（已发货，待收货）?>
            <a href="index.php?gct=pointorder&gp=order_ship&id=<?php echo $v['point_orderid']; ?>"><?php echo $lang['admin_pointorder_ship_modtip']; ?></a>
            <?php } ?>
            <!-- 取消订单 -->
            <?php if ($v['point_orderallowcancel']){//取消订单（未发货） ?>
            <a href="javascript:void(0)" onclick="confirmorder('index.php?gct=pointorder&gp=order_cancel&id=<?php echo $v['point_orderid']; ?>','<?php echo $lang['admin_pointorder_cancel_confirmtip']; ?>');"><?php echo $lang['admin_pointorder_cancel_title']; ?></a>
            <?php } ?>
            <br>
            <!-- 删除订单 -->
            <?php if ($v['point_orderallowdelete']) {?>
            <a href="javascript:void(0)" onclick="if(confirm('<?php echo $lang['nc_ensure_del']; ?>')){window.location='index.php?gct=pointorder&gp=order_drop&order_id=<?php echo $v['point_orderid']; ?>';}else{return false;}"><?php echo $lang['nc_del']; ?></a>
            <?php } ?></td>
        </tr>
        <?php } ?>
        <?php }else { ?>
