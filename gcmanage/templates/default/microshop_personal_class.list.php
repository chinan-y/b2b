...
$(document).ready(function(){
    //行内ajax编辑
    $('span[nc_type="class_sort"]').inline_edit({gct: 'microshop',gp: 'personalclass_sort_update'});
    $('span[nc_type="class_name"]').inline_edit({gct: 'microshop',gp: 'personalclass_name_update'});
});
...
function submit_delete(id){
    if(confirm('<?php echo $lang['nc_ensure_del'];?>')) {
        ...
        $('#list_form').attr('action','index.php?gct=microshop&gp=personalclass_drop');
        $('#class_id').val(id);
        $('#list_form').submit();
    }
}
    ...
    <table class="table tb-type2">
      ...
      <tbody>
        ...
        <tr class="hover edit">
          ...
          <td class="w72 align-center"><a href="index.php?gct=microshop&gp=personalclass_edit&class_id=<?php echo $val['class_id'];?>"><?php echo $lang['nc_edit'];?></a> <a href="javascript:submit_delete(<?php echo $val['class_id'];?>)"><?php echo $lang['nc_del'];?></a></td>
        </tr>
        <?php } ?>
        <?php }else { ?>
        <tr class="no_data">
