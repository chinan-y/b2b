...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base">
        <li><a href="JavaScript:void(0);" class="current"><span>销售员订单</span></a></li>
    ...
    <tbody>
      ...
      <tr>
        ...
        <ul>
			...
			<li style="color:#FF0000;"><?php echo '已入库是订单推到了仓库， 待收货是仓库的货物已出库';?></li>
          </ul></td>
      </tr>
    </tbody>
  </table>
  ...
  <table class="table tb-type2 nobdb">
    <thead>
      <tr class="thead">
		...
		<?php if(C('one_rank_rebate') == 1){?> <th class="align-center"><?php echo "UP1";?></th><th class="align-center"><?php echo "UP1推广奖励";?></th><?php } ?>
		<?php if(C('two_rank_rebate') == 1){?> <th class="align-center"><?php echo "UP2";?></th><th class="align-center"><?php echo "UP2推广奖励";?></th><?php } ?>
		<?php if(C('three_rank_rebate') == 1){?><th class="align-center"><?php echo "UP3";?></th><th class="align-center"><?php echo "UP3推广奖励";?></th><?php } ?>
    ...
    <?php foreach($output['order_list'] as $order){  ?>
      <tr class="hover">
		...
		<?php if(C('one_rank_rebate') == 1) { ?>   <td class="align-center"><?php echo $order['one_name'];?></td><td class="align-center"><?php echo $order['one_rebate'];?></td><?php } ?>
		<?php if(C('two_rank_rebate') == 1) { ?>  <td class="align-center"><?php echo $order['two_name'];?></td><td class="align-center"><?php echo $order['two_rebate'];?></td><?php } ?>
		<?php if(C('three_rank_rebate') == 1) { ?> <td class="align-center"><?php echo $order['three_name'];?></td><td class="align-center"><?php echo $order['three_rebate'];?></td><?php } ?>
		
        <td class="align-center"><?php echo orderPaymentName($order['payment_code']);?></td>
		<?php if($order['lib_state'] && $order['lib_state']['MAKE_CSV'] == 20 && $order['order_state'] != 40){ ?>
			<?php if($order['lib_state']['MAKE_CSV'] == 20 && $order['lib_state']['OIF_MEMO'] == null){?>
				<td class="align-center" style="color:#006F94;"><?php echo '已入库';?></td>
			<?php }else { ?>
				<td class="align-center" style="color:#FF0000;"><?php echo '待收货';?></td>
			<?php } ?>
		<?php }else{ ?>
        ...
        <!-- 取消订单 -->
    		...
    		<?php }?>
    			<?php if($order['goods_rebate_amount'] >1 && $order['order_state'] < 40 && orderState($order) != '已取消'){?>
