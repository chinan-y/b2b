...
$(document).ready(function(){
    //行内ajax编辑
    $('span[nc_type="picture_sort"]').inline_edit({gct: 'cms_picture',gp: 'update_picture_sort'});
    $('span[nc_type="picture_click"]').inline_edit({gct: 'cms_picture',gp: 'update_picture_click'});
...
function submit_delete(id){
    if(confirm('<?php echo $lang['nc_ensure_del'];?>')) {
        $('#list_form').attr('action','index.php?gct=cms_picture&gp=cms_picture_drop');
        $('#picture_id').val(id);
        $('#list_form').submit();
    }
}
...
function submit_callback(id){
    if(confirm('<?php echo $lang['cms_ensure_callback'];?>')) {
        $('#list_form').attr('action','index.php?gct=cms_picture&gp=cms_picture_callback');
        $('#picture_id').val(id);
        $('#list_form').submit();
    }
}
...
<div class="page">
  ...
  <form method="get" name="formSearch">
        <input type="hidden" value="cms_picture" name="gct">
        ...
        <table class="tb-type1 noborder search">
            <tbody>
                <tr>
                    ...
                    <?php if($_GET['gp'] == 'cms_picture_list') { ?>
      ...
      <tbody>
        ...
        <tr class="hover edit">
          ...
          <td class="w108 align-center">
              <a href="<?php echo CMS_SITE_URL.'/index.php?gct=picture&gp=picture_detail&picture_id='.$val['picture_id'];?>" target="_blank"><?php echo $lang['cms_text_see'];?></a>
...
<div id="dialog_verify" style="display:none;">
    <form id="verify_form" method='post' action="index.php?gct=cms_picture&gp=cms_picture_verify">
