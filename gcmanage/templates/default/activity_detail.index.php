...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base">
        <li><a href="index.php?gct=activity&gp=activity" ><span><?php echo $lang['nc_manage'];?></span></a></li>
        <li><a href="index.php?gct=activity&gp=new" ><span><?php echo $lang['nc_new'];?></span></a></li>
  ...
  <form method="get" name="formSearch">
    ...
    <input type="hidden" name="gct" value="activity">
    <input type="hidden" name="gp" value="detail">
    ...
    <table class="table tb-type2">
      ...
      <tbody id="treet1">
        ...
        <tr class="hover">
          ...
          <td><a target="_blank" href="<?php echo SHOP_SITE_URL;?>/index.php?gct=goods&goods_id=<?php echo $v['item_id']?>"><?php echo $v['item_name'];?></a></td>
          ...
          <td class="w150 align-center">
          	<?php if($v['activity_detail_state']!='1'){?>
            	<a href="index.php?gct=activity&gp=deal&activity_detail_id=<?php echo $v['activity_detail_id'];?>&state=1"><?php echo $lang['activity_detail_index_pass'];?></a>
            <?php }?>
            <?php if($v['activity_detail_state']=='0'){?>
            	&nbsp;|&nbsp;
            <?php }?>
            <?php if($v['activity_detail_state']!='2'){?>
            	<a href="index.php?gct=activity&gp=deal&activity_detail_id=<?php echo $v['activity_detail_id'];?>&state=2"><?php echo $lang['activity_detail_index_refuse'];?></a>
            <?php }?>
            <?php if ($v['activity_detail_state']=='0' || $v['activity_detail_state']=='2'){?>
            	&nbsp;|&nbsp;<a href="javascript:void(0)" onclick="if(confirm('<?php echo $lang['nc_ensure_del'];?>')){location.href='index.php?gct=activity&gp=del_detail&activity_detail_id=<?php echo $v['activity_detail_id'];?>';}"><?php echo $lang['nc_del'];?></a></td>
            <?php }?>
        </tr>
        <?php } ?>
        <?php }else { ?>
...
function submit_form(gp){
	if(gp=='del'){
		if(!confirm('<?php echo $lang['nc_ensure_del'];?>')){
			return false;
		}
		$('#listform').attr('action','index.php?gct=activity&gp=del_detail');
	}else if(gp=='pass'){
		if(!confirm('<?php echo $lang['activity_detail_index_pass_all'];?>')){
			return false;
		}
		$('#listform').attr('action','index.php?gct=activity&gp=deal&state=1');
	}else if(gp=='refuse'){
		if(!confirm('<?php echo $lang['activity_detail_index_refuse_all'];?>')){
			return false;
		}
		$('#listform').attr('action','index.php?gct=activity&gp=deal&state=2');
	}
	$('#listform').submit();
}
</script>
